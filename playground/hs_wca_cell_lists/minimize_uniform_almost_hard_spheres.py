...
class MinimizeUniformHardsSpheres(object):
    def __init__(
        ...
        self.hard_radii = np.random.normal(
            loc=1, scale=0.1, size=self.nr_particles
        )
        self.box_length = np.power(
            np.sum(
                np.asarray([4 * np.pi * r**3 / 3 for r in self.hard_radii])
            )
            / self.hard_volume_fraction,
            1 / 3,
        )
        ...
        print(
            "energy after:",
            self.potential.getEnergy(self.optimizer.get_result().coords),
        )
...
if __name__ == "__main__":
    ...
    for alpha in np.linspace(0, 0.5, 10):
        ...
        for phi in np.linspace(0.3, 0.6, 10):
            ...
            print("( sca:", alpha, ")")
            MinimizeUniformHardsSpheres(
                hard_volume_fraction=phi, use_hswca=use_hswca
            )
