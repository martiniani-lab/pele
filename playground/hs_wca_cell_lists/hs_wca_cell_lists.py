...
class Config2D(object):
    ...
    def optimize(self, nr_samples=1):
        ...
        self.optimizer_cells = ModifiedFireCPP(
            self.x_initial.copy(),
        ...
        self.optimizer_cells_ = LBFGS_CPP(
            self.x_initial.copy(), self.potential_cells_
        )
        print("initial E, x:", self.potential.getEnergy(self.x_initial.copy()))
        print(
            "initial E, x_:",
            self.potential_cells.getEnergy(self.x_initial.copy()),
        )
        ...
        print(
            "fire final E, x_cells:", self.optimizer_cells.get_result().energy
        )
        print("fire final E, plain: ", self.potential.getEnergy(self.x_final))
        print(
            "fire final E, cell: ",
            self.potential_cells.getEnergy(self.x_final_cells),
        )
        ...
        if not self.res_x_final.success or not self.res_x_final_cells.success:
            ...
            print(
                "self.res_x_final_cells.success", self.res_x_final_cells.success
            )
            print("-------------")
            plot_disks(self.x_initial, self.radii, self.boxvec, sca=self.sca)
            plot_disks(self.x_final, self.radii, self.boxvec, sca=self.sca)
            plot_disks(
                self.x_final_cells, self.radii, self.boxvec, sca=self.sca
            )
        ...
        print(
            "lbfgs final E, x_cells:", self.optimizer_cells_.get_result().energy
        )
        print(
            "lbfgs final E, plain: ", self.potential_.getEnergy(self.x_final_)
        )
        print(
            "lbfgs final E, cell: ",
            self.potential_cells_.getEnergy(self.x_final_cells_),
        )
        ...
        if (
            not self.res_x_final_.success
            or not self.res_x_final_cells_.success
            or not self.res_x_final.success
            or not self.res_x_final_cells.success
        ):
            print("-------------")
            ...
            print(
                "self.res_x_final_cells_.success",
                self.res_x_final_cells.success,
            )
            print("-------------")
            plot_disks(self.x_initial, self.radii, self.boxvec, sca=self.sca)
            plot_disks(self.x_final_, self.radii, self.boxvec, sca=self.sca)
            plot_disks(
                self.x_final_cells_, self.radii, self.boxvec, sca=self.sca
            )
...
class Config2DFrozenBoundary(object):
    def __init__(self, nparticles_x, amplitude):
        ...
        self.eps = 1.0
        max_edge = (
            np.amax([np.amax(self.x_initial), np.abs(np.amin(self.x_initial))])
            + 2 * self.amplitude
            + (1 + self.sca) * self.radius
        )
        ...
        self.nstepsmax = int(1e6)
        assert self.boxvec[0] == self.boxvec[1]
        self.x_initial_red = reduce_coordinates(
            self.x_initial, self.frozen_atoms, self.ndim
        )
        ...
        assert np.allclose(
            self.potential.getEnergy(self.x_initial_red),
            self.potential_cells.getEnergy(self.x_initial_red),
            rtol=1e-10,
        )
        print(self.boxvec)
    ...
    def optimize(self, nr_samples=1):
        ...
        self.optimizer_cells_ = LBFGS_CPP(
            self.x_initial_red.copy(), self.potential_cells
        )
        t0 = time.time()
        print("self.optimizer.run(self.nstepsmax)", self.nstepsmax)
        self.optimizer.run(self.nstepsmax)
        ...
        print(
            "lbfgs final E, cell: ",
            self.potential_cells.getEnergy(self.x_final_cells_),
        )
        print("lbfgs number of particles:", self.N)
        print("lbfgs time no cell lists:", t3 - t2, "sec")
        print("lbfgs time cell lists:", t4 - t3, "sec")
...
def measurement(nr_samples, LXmax, amplitude):
    ...
    time_ratio = []
    time_ratio_lbfgs = []
    for LX in range(3, LXmax + 1):
        print("---LX: ", LX)
        t_fire = 0
        t_lbfgs = 0
        nrp = 0
        for tmp in range(nr_samples):
    ...
    plt.plot(
        xr,
        np.array([ll(xi, popt[0], popt[1]) for xi in xr]) - 1,
        "k",
        label=r"Exponent: " + to_string(popt[1], 2),
    )
