...
from pele.optimize import LBFGS_CPP, ModifiedFireCPP
...
class Config2D(object):
    def __init__(self, nparticles_x, amplitude):
        ...
        self.x_initial = np.asarray(
            [xi + np.random.uniform(-self.amplitude, self.amplitude) for xi in self.x]
        )
        ...
        assert self.potential.getEnergy(self.x_initial) == self.potential_.getEnergy(
            self.x_initial
        )
    ...
    def optimize(self, nr_samples=1):
        ...
        print(
            "fire final E, cell: ", self.potential_cells.getEnergy(self.x_final_cells)
        )
...
class Config2DFrozenBoundary(object):
    ...
    def optimize(self, nr_samples=1):
        ...
        print(
            "lbfgs final E, cell: ", self.potential_cells.getEnergy(self.x_final_cells_)
        )
        print("lbfgs number of particles:", self.N)
        print("lbfgs time no cell lists:", t3 - t2, "sec")
        print("lbfgs time cell lists:", t4 - t3, "sec")
