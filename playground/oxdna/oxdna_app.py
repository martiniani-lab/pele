...
class AppOXDNA(AppBasinHopping):
    ...
    def create_takestep(self):
        ...
        step2 = takestep.AdaptiveStepsize(
            OXDNATakestep(displace=0.0, rotate=self.options.rotate),
            frequency=50,
        )

        step1 = takestep.AdaptiveStepsize(
            OXDNAScrewStep(rotate_backbone=0.5, rotate_base=3.0)
        )
        ...
        reseed = takestep.Reseeding(
            step2, genrandom, maxnoimprove=self.options.reseed
        )
        return reseed
    ...
    def create_basinhopping(self, add_minimum=None, potential=None):
        ...
        self.opt = AppBasinHopping.create_basinhopping(
    ...
    def check_converged(self, E, coords):
        if E < (self.target + EDIFF):
            ...
            print(
                "%d %d %f"
                % (self.opt.stepnum, self.potential.ncalls, timespent)
            )
            exit()
