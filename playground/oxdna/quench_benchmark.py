...
from pele.optimize.quench import fire, mylbfgs
from pele.potentials.gminpotential import GMINPotential
from pele.systems import oxdna

parser = OptionParser()
parser.add_option(
    "--tol", dest="tol", type=float, default=1e-6, help="tolerance for quench"
)
parser.add_option("-M", dest="M", type=int, default=5, help="M value for mylbfgs")
parser.add_option(
    "--maxErise",
    dest="maxErise",
    type=float,
...
parser.add_option(
    "-o", dest="outfile", default="results.txt", help="outfile for results"
)
