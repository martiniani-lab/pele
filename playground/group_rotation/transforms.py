...
class LinearTransform(Transform):
    ...
    def __init__(
        self, transform=np.identity(3), orthogonal=False, *args, **kwargs
    ):
...
class AffineTransform(Transform):
    ...
    def getI(self):
        inverse = AffineTransform(
            linear_transform=self.linear_transform.I,
...
def inversion(affine=False):
    ...
    """
    linear_inversion = LinearTransform(
        [[-1, 0, 0], [0, -1, 0], [0, 0, -1]], orthogonal=True
    )
    if affine == True:
...
def proper_rotation(axis, angle, affine=False, right_handed=True):
    ...
    lin_rotation_transform = LinearTransform(
        lin_rotation_matrix, orthogonal=True
    )
    if affine == False:
...
def reflection(plane_normal, affine=False):
    ...
    lin_reflection_transform = LinearTransform(
        reflection_matrix, orthogonal=True
    )
    if affine == False:
...
def improper_rotation(axis, angle, affine=False, right_handed=True):
    ...
    returns a LinearTransform object.
    """
    rot = proper_rotation(axis=axis, angle=angle, right_handed=right_handed)
