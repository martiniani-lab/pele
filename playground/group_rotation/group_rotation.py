...
class GroupRotation(TakestepInterface):
    ...
    def takeStep(self, coords, **kwargs):
        ...
        for group in self.parameters:
            ...
            if (
                np.random.uniform()
                > self.parameters[group]["selection_probability"]
            ):
            ...
            rotation = transforms.proper_rotation(bond_axis, angle, affine=True)
            ...
            # indexing.
            coords_indices = [
                3 * atom + k for atom in group_atoms for k in range(3)
            ]
            ...
            for group_atom_coord_index, coord_index in enumerate(
                coords_indices
            ):
...
def averages(parameters, coords, n_trials, attribute):
    ...
    for i in range(n_trials):
        ...
        for group in parameters:
            attribute_average[group] = (
                (i) * attribute_average[group] + step_result[group][attribute]
            ) / float(i + 1)
            diff = step_result[group][attribute] - attribute_average[group]
            attribute_variance[group] = (
                (i) * attribute_variance[group] + diff**2
            ) / float(i + 1)
