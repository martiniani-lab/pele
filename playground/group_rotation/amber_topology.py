...
class AmberTopology(object):
    ...
    def parse_file(self, file_object):
        ...
        for line in file_object:
            ...
            if line.startswith("%VERSION"):
                ...
                split_line = [
                    line[i : i + current_data_length]
                    for i in range(0, len(line), current_data_length)
                ]
                formatted_line = list(
                    map(data_casts[current_data_type], split_line)
                )
    ...
    def bonds(self, by_type):
        ...
        self.bonds = []
        topology_bonds_list = (
            topology_data["BONDS_INC_HYDROGEN"]
            + topology_data["BONDS_WITHOUT_HYDROGEN"]
        )
        if by_type is True:
    ...
    def angles(self, by_type):
        if not hasattr(self, "topology_data"):
            raise exc.ValueError("Topology file doesn't contain any data.")
        self.angles = []
        topology_angles_list = (
            topology_data["ANGLES_INC_HYDROGEN"]
            + topology_data["ANGLES_WITHOUT_HYDROGEN"]
        )
        if by_type is True:
    ...
    def dihedrals(self, by_type):
        if not hasattr(self, "topology_data"):
            raise exc.ValueError("Topology file doesn't contain any data.")
        self.dihedrals = []
        topology_dihedrals_list = (
            topology_data["DIHEDRALS_INC_HYDROGEN"]
            + topology_data["DIHEDRALS_WITHOUT_HYDROGEN"]
        )
        if by_type is True:
