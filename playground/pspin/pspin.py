...
def isclose(a, b, rel_tol=1e-9, abs_tol=0.0, method="weak"):
    ...
    if method not in ("asymmetric", "strong", "weak", "average"):
        raise ValueError(
            'method must be one of: "asymmetric",'
            ' "strong", "weak", "average"'
        )
...
def compare_exact(x1, x2, rel_tol=1e-9, abs_tol=0.0, method="weak"):
...
def func(pot, coords):
    ...
    results = lbfgs_cpp(
        coords, pot, nsteps=1e5, tol=1e-9, iprint=-1, maxstep=10
    )
...
def main():
    ...
    assert (
        p == 3
    ), "the interaction matrix setup at the moment requires that p==3"
    ...
    if False:
        ...
        out = Parallel(n_jobs=max(1, 4))(
            delayed(func)(pot, x) for x in coords_list
        )
        out = np.array(out)
    ...
    if False:
        ...
        nx.draw_networkx_edges(
            G, pos, edgelist=epos, width=2, alpha=0.3, edge_color="r", ax=ax
        )
        nx.draw_networkx_edges(
            G,
            pos,
        ...
        # rendering
        minimum = lbfgs_cpp(
            coords, pot, nsteps=1e5, tol=1e-5, iprint=10, maxstep=10
        ).coords
        ...
        nx.draw_networkx_edges(
            G, pos, edgelist=epos, width=2, alpha=0.3, edge_color="r", ax=ax2
        )
        nx.draw_networkx_edges(
            G,
            pos,
