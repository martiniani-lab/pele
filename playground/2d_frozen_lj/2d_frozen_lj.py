...
class LJClusterFrozen2D(LJCluster):
    def __init__(self, natoms):
        ...
        if len(self.frozen_dof) <= 1:
            print(
                "warning: Dealing properly with the rotational and translational degrees of freedom and reflection symmetry in clusters with 1 or 2 frozen atoms is not implemented yet"
            )

    def get_potential(self):
        pot = LJCluster.get_potential(self)
        frozen_pot = FrozenPotWrapper(
    ...
    def get_random_configuration(self):
        ...
        boxl = 0.7 * float(self.natoms) ** (1.0 / 2)
