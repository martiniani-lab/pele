...
from pele.mindist import ExactMatchCluster, MinPermDistCluster
from pele.mindist._minpermdist_policies import (MeasureAtomicCluster,
                                                TransformAtomicCluster)
from pele.mindist.permutational_alignment import (_find_permutations,
                                                  _make_cost_matrix,
...
def find_best_permutation_molecular(
    X1,
    X2,
    permlist=None,
    user_algorithm=None,
    ...
    for atomlists in permlist:
        ...
        if user_algorithm is not None:
            dist, perm = user_algorithm(
                X1[atomlist], X2[atomlist2], make_cost_matrix=user_cost_matrix, **kwargs
            )
        elif user_cost_matrix is not _make_cost_matrix:
            dist, perm = find_permutations_hungarian(
                X1[atomlist], X2[atomlist2], make_cost_matrix=user_cost_matrix, **kwargs
            )
        else:
...
def permlist_water(nmol):
    ...
    permlist = [
        [
            list(range(0, nmol * 3, 3)),
            list(range(1, nmol * 3, 3)),
            list(range(2, nmol * 3, 3)),
        ]
    ]
    permlist += [
        list(x) for x in zip(list(range(1, nmol * 3, 3)), list(range(2, nmol * 3, 3)))
    ]
