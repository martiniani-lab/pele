...
class XYModlelSystem(BaseSystem):
    ...
    def setup_params(self, params):
        ...
        params.double_ended_connect.local_connect_params.NEBparams.iter_density = (
            50.0
        )
        params.double_ended_connect.local_connect_params.NEBparams.reinterpolate = (
            50
        )
        params.double_ended_connect.local_connect_params.NEBparams.adaptive_nimages = (
            True
        )
        params.double_ended_connect.local_connect_params.NEBparams.adaptive_niter = (
            False
        )
    ...
    def get_potential(self, phases=None):
        try:
            ...
            base_pot = XYModel(
                dim=self.dim, phi=self.phi_disorder, phases=phases
            )
            ...
            print(
                "making frozen spin at",
                self.node2xyz(base_pot.index2node[frozen_index]),
            )
    ...
    def get_compare_exact(self):
        ...
        return lambda x1, x2: mindist(x1, x2)[0] < 0.1
    ...
    def get_random_configuration(self):
        if self.one_frozen:
            ...
            n = self.nspins
        return np.random.uniform(0, 2.0 * np.pi, n)
    ...
    def get_takestep(self, **kwargs):
        ...
        kwargs = dict(
            list(self.params["takestep"].items()) + list(kwargs.items())
        )
        try:
