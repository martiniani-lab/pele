...
class SoftSphereSystem(BaseSystem):
    ...
    def __init__(self, radii, boxvec, power=2):
        ...
        self.params.double_ended_connect.local_connect_params.NEBparams.interpolator = (
            InterpolateLinearMeasure(MeasurePeriodic(self.boxvec))
        )
    ...
    def set_params(self, params):
        ...
        params.double_ended_connect.local_connect_params.tsSearchParams.hessian_diagonalization = (
            True
        )

        params.takestep.stepsize = 0.4
    ...
    def get_random_configuration(self):
        ...
        for i in range(self.boxvec.size):
            x[:, i] = np.random.uniform(
                -self.boxvec[i] / 2.0, self.boxvec[i] / 2.0, self.natoms
            )
        return x.ravel()

    def draw(self, coordslinear, index):
        from pele.systems._opengl_tools import (
            draw_atomic_binary_polydisperse, draw_box)

        put_in_box(coordslinear, self.boxvec)
        draw_atomic_binary_polydisperse(
            coordslinear, index, bdim=self.boxvec.size, radii=self.radii
        )
...
def plot_potential():
    ...
    elist = [
        pot.getEnergy(np.array([0, 0.0, 0.0, r, 0.0, 0, 2.5, 0, 0])) for r in rlist
    ]
