...
def spin3d_distance(xa, xb, distance=True, grad=True):
    ...
    sys.stderr.write(
        "Warning, this distance function (spin3d_distance) is wrong.  The NEB will be messed up\n"
    )
...
class HeisenbergSystem(BaseSystem):
    def __init__(self, dims=[4, 4], field_disorder=1.0, disorder=False):
        ...
        if field_disorder == 0.0 or not disorder:
    ...
    def setup_params(self, params):
        ...
        params.double_ended_connect.local_connect_params.NEBparams.interpolator = (
            interpolate_spins
        )
        params.double_ended_connect.local_connect_params.NEBparams.image_density = (
            10
        )
        params.double_ended_connect.local_connect_params.NEBparams.reinterpolate = (
            50
        )
        params.double_ended_connect.local_connect_params.NEBparams.distance = (
            spin3d_distance
        )
    ...
    def get_potential(self):
        try:
            ...
            if self.one_frozen:
                ...
                base_pot = HeisenbergModel(dim=self.dims, field_disorder=0.0)
                ...
                self.pot = FrozenPotWrapper(
                    base_pot, reference_coords, frozen_dof
                )
                ...
                return HeisenbergModelRA(
                    dim=self.dims, field_disorder=self.field_disorder
                )
...
def test_pot():
    ...
    system = HeisenbergSystem(field_disorder=3.0, disorder=True)
    pot = system.get_potential()
    x = system.get_random_configuration()
