...
def water():
    ...
    water.add_atom("O", np.array([0.0, 0.0, 0.0]), 16.0)
    water.add_atom("H", rho * np.array([0.0, sin(0.5 * theta), cos(0.5 * theta)]), 1.0)
    water.add_atom("H", rho * np.array([0.0, -sin(0.5 * theta), cos(0.5 * theta)]), 1.0)
...
def align(system, coords1, coords2):
    ...
    for p1, p2 in zip(c1.rotRigid, c2.rotRigid):
        ...
        p2n = rotations.rotate_aa(np.array([0.0, 0.0, pi]), p2)
        theta2 = np.linalg.norm(rotations.rotate_aa(p2n, -p1))
        theta1 -= int(theta1 / 2.0 / pi) * 2.0 * pi
        theta2 -= int(theta2 / 2.0 / pi) * 2.0 * pi
        if theta2 < theta1:
            p2[:] = p2n
...
def get_path(system, coords1, coords2, nimages):
    ...
    path = [
        x
        for x in InterpolatedPath(
            coords2, coords1, nimages, interpolator=system.interpolate
        )
    ]
