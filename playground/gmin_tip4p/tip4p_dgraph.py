...
for ts in db.transition_states():
    ...
    ts.free_energy = (
        ts.energy + kbT * 0.5 * ts.fvib + kbT * np.log(ts.pgorder) + kbT * np.log(kbT)
    )
    if (
        ts.free_energy > ts.minimum1.free_energy
        or ts.free_energy > ts.minimum2.free_energy
    ):
...
dg_F = DisconnectivityGraph(
    graph.graph, db.minima(), energy_attribute="free_energy", subgraph_size=20
)
dg_F.calculate()
dg_F.plot()
