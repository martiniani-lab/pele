...
from pele.angleaxis import (RBPotentialWrapper, RBSystem, RBTopology,
                            RigidFragment)
    ...
    for i in range(aps - 1):
        for j in range(aps - 1):
            if i + j >= aps - 1:
                break
            xnew = v1 * i + v2 * j
            if i == 0 and j == 0 or i == 0 and j == aps - 2 or i == aps - 2 and j == 0:
                atomtype = OTHER_TYPE
            elif i == 0:
                atomtype = EDGE1_TYPE
            elif j == 0:
...
class PlateFolder(RBSystem):
    ...
    def get_mindist(self, **kwargs):
        from pele.angleaxis import (MeasureAngleAxisCluster,
        ...
        return MinPermDistAACluster(
            self.aasystem, measure=measure, transform=transform, accuracy=0.1, **kwargs
        )
