...
class PlateFolder(RBSystem):
    ...
    def get_random_configuration(self):
        ...
        coords = 10.0 * np.random.random(6 * self.nrigid)
    ...
    def setup_aatopology(self):
        ...
        topology = RBTopology()
        topology.add_sites(
            [make_triangular_plate() for i in range(self.nrigid)]
        )
    ...
    def _create_potential(self):
        ...
        plate_pot = PlatePotential(
            harmonic_atoms1, harmonic_atoms2, lj_atoms, k=10
        )
    ...
    def get_mindist(self, **kwargs):
        ...
        from pele.angleaxis import (
            ...
            MeasureAngleAxisCluster,
        )

        transform = TransformAngleAxisCluster(self.aatopology)
        measure = MeasureAngleAxisCluster(
            self.aatopology, transform=transform, permlist=[]
        )
        return MinPermDistAACluster(
            self.aasystem,
            measure=measure,
            transform=transform,
            accuracy=0.1,
