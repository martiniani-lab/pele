...
print("time for cpp lbfgs with fortran lj", t3-t2, "speedup",  old_div((t2-t1),(t3-t2)))
print("time for cpp lbfgs with old lj", t4-t3, "speedup",  old_div((t2-t1),(t4-t3)))
...
for name, pot in potentials.items():
    ...
    label = "%15s" % name
    print(label, "getEnergy", pot.getEnergy(x), "getEnergyGradient E", e, "rms", old_div(np.linalg.norm(x), np.sqrt(x.size)))
