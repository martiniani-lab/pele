...
class PTMC(object):
    ...
    ****This is still in testing, and definitely not ready for any production runs****
    ...
    def tryExchangeNoParallel(self):
        ...
        deltabeta = (
            1.0 / self.replicas[k].temperature
            - 1.0 / self.replicas[k + 1].temperature
        )
        w = min(1.0, np.exp(deltaE * deltabeta))
        rand = np.random.rand()
        if w > rand:
...
class PTExchangeIndependent(object):
    ...
    def run(self):
        ...
        # print self.replicas
        print(
            "accept ratio", float(self.naccept) / nsteps, nsteps, self.naccept
        )
        print(self.acccount)
    ...
    def mcstep(self):
        ...
        w = min(1.0, np.exp((e1 - e2) * (beta1 - beta2)))
        rand = np.random.rand()
        if w > rand:
            # accept step
            # print "accept step"
