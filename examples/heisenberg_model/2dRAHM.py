...
from pele.potentials.heisenberg_spin import (coords2ToCoords3,
                                             coords3ToCoords2, grad3ToGrad2,
                                             make2dVector, make3dVector)
...
def printspins(fout, pot, coords2):
    ...
    for node in pot.G.nodes():
        ...
        fout.write(
            "%g %g %g %g %g %g %g %g\n"
            % (node[0], node[1], s[i, 0], s[i, 1], s[i, 2], h[i, 0], h[i, 1], h[i, 2])
        )
...
pot = HeisenbergModelRA(dim=[L, L], field_disorder=2.0)
