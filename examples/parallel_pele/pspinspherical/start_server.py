...
def write_server_uri(server_name, hostname, port, nspins, p):
    ...
    with open(
        "server_uri_pspin_spherical_p{}_N{}.uri".format(p, nspins), "w"
    ) as out_server_uri:
...
def get_database_params_worker(nspins, p):
    db, (db_nspins, db_p, interactions) = _get_database_params(
        "pspin_spherical_p{}_N{}.sqlite".format(p, nspins)
    )
...
def main():
    ...
    parser.add_argument(
        "--server-name", type=str, help="name of the dispatcher", default=None
    )
    parser.add_argument(
        "--host",
        type=str,
    ...
    try:
        ...
        print(
            "Warning: database {} already exists, using the already existing database"
            .format(dbname)
        )
    except IOError:
        db = None
