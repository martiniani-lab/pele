...
struct mypot_interaction {
  ...
  mypot_interaction(double sig, double eps)
      : _sig12(pow(sig, 12)), _sig24(_sig12 * _sig12), _eps(eps) {}
   ...
   * |g|/|rij| */
  double inline energy_gradient(double r2, double *gij, size_t atom_i,
                                size_t atom_j) const {
    double ir2 = 1.0 / r2;
    double ir6 = ir2 * ir2 * ir2;
    double ir12 = ir6 * ir6;
  ...
  double inline energy_gradient_hessian(double r2, double *gij, double *hij,
    ...
    double energy = 4. * _eps * (_sig24 * ir24 - _sig12 * ir12);
    *gij = 4. * _eps * (24. * _sig24 * ir24 - 12. * _sig12 * ir12) * ir2;
    *hij = 4. * _eps * (24. * 25. * _sig24 * ir24 - 12. * 13 * _sig12 * ir12) *
           ir2;
    return energy;
  }
};
