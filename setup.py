...
with_cvode_default = int(os.environ.get("PELE_WITH_CVODE", 1))
parser.add_argument(
    "--with-cvode",
    type=int,
    default=with_cvode_default,
    help="Build with CVODE. Needed for Attractor identification. Default: True. Can also be set with PELE_WITH_CVODE=0 or 1.",
)
...
# Try to get number of jobs from MAKEFLAGS, then from a custom env var.
default_jobs = 4
makeflags = os.environ.get("MAKEFLAGS")
if makeflags:
    m = re.search(r"-j\s*(\d+)", makeflags)
    if m:
        default_jobs = int(m.group(1))

default_jobs = int(os.environ.get("PELE_BUILD_JOBS", default_jobs))

if jargs.j is None:
    jargs.j = default_jobs
...
def get_ldflags(opt="--ldflags"):
    ...
    try:
        ...
        # LINKFORSHARED contains required linker flags for extensions
        linkforshared = getvar("LINKFORSHARED")
        if linkforshared:
            libs.extend(linkforshared.split())

        if not sys.platform.startswith("darwin"):
            # On Linux, only link python if shared libraries are enabled
            if getvar("Py_ENABLE_SHARED"):
                ...
                # We do NOT link against a static libpython.a, as it often
                # doesn't exist or causes other issues.
                pass # Don't add -lpython
        else:
            # On macOS, -undefined dynamic_lookup is handled by CMake.
            # We don't need to explicitly link python here.
            pass
        ...
        # LINKFORSHARED contains required linker flags for extensions
        linkforshared = std_sysconfig.get_config_var("LINKFORSHARED")
        if linkforshared:
            libs.extend(linkforshared.split())

        if not sys.platform.startswith("darwin"):
            if std_sysconfig.get_config_var("Py_ENABLE_SHARED"):
                pyver = std_sysconfig.get_config_var("VERSION")
                libs.append(f"-lpython{pyver}")
            else:
                pass # Don't add -lpython on static builds
        else:
            pass # No explicit linking on macOS

        libdir = std_sysconfig.get_config_var("LIBDIR")
        if libdir:
            libs.insert(0, f"-L{libdir}")

        return " ".join(list(dict.fromkeys(libs))) # Remove duplicates
