...
TEST(EXTENDED_MXD, TEST_256_RUN_DENSE) {
  ...
  radii = pele::generate_bidisperse_radii(n_1, n_2, r_1, r_2, r_std1, r_std2, 0);
...
// required to plot trajectory
  ...
  pele::ExtendedMixedOptimizer optimizer_mxopt(
      pot, x, extension_potential, 1e-9, 60, 1, 1e-1, 1e-4, 1e-4, false);
...
TEST(EXTENDED_MXD, TEST_256_RUN_ITERATIVE) {
  static const size_t _ndim = 2;
  size_t n_particles;
  size_t n_dof;
