...
TEST_F(LJTest, Energy_Works) { test_energy(); }

TEST_F(LJTest, EnergyGradient_AgreesWithNumerical) { test_energy_gradient(); }
...
class LJNeighborListTest : public LJTest {
  virtual void setup_potential() {
    ...
    pot = std::shared_ptr<pele::BasePotential>(
        new pele::LJNeighborList(ilist, c6, c12));
  }
};

TEST_F(LJNeighborListTest, Energy_Works) { test_energy(); }
...
TEST_F(LJCutTest, Energy_Works) { test_energy(); }
...
public:
  virtual void setup_potential() {
    ...
    pot = std::shared_ptr<pele::BasePotential>(
        new pele::LJCutAtomList(c6, c12, rcut, atoms));
  }
};

TEST_F(LJCutAtomListTest, Energy_Works) { test_energy(); }
...
public:
  virtual void setup_potential() {
    ...
    pot = std::shared_ptr<pele::BasePotential>(
        new pele::LJCutPeriodicAtomList(c6, c12, rcut, boxvec, atoms));
  }
};

TEST_F(LJCutPeriodicAtomListTest, Energy_Works) { test_energy(); }
