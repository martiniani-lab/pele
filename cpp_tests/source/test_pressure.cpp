...
TEST(Pressure, BasicComponents_Work) {
  ...
  std::shared_ptr<pele::BasePotential> pot =
      std::make_shared<pele::InverseIntPower<nr_dim, power>>(eps, radii);
  ...
  const double e_true =
      r < 2 * radius ? eps / power * std::pow((1 - r / (2 * radius)), power)
                     : 0;
  EXPECT_DOUBLE_EQ(e_true, e);
  double dr[nr_dim];
  dynamic_cast<pele::PairwisePotentialInterface *>(pot.get())->get_rij(
      dr, x.data(), x.data() + nr_dim);
  for (size_t i = 0; i < nr_dim; ++i) {
    EXPECT_DOUBLE_EQ(delta_x[i], dr[i]);
  }
  double gij;
  const double e_i = dynamic_cast<pele::PairwisePotentialInterface *>(pot.get())
                         ->get_interaction_energy_gradient(
                             pele::pos_int_pow<2>(r), &gij, 0, 1);
  EXPECT_DOUBLE_EQ(e_true, e_i);
  const double sigma = 2 * radius;
  const double gij_true =
      eps / (sigma * r) * std::pow(1 - r / sigma, power - 1);
  ...
  for (size_t i = 0; i < ptensor.size(); ++i) {
    // std::cout << "ptensor[i] / ptensor_true[i]: " << ptensor[i] /
    // ptensor_true[i] << "\n";
