...
public:
  ...
  void SetUp() {
    ...
    for (size_t k = 0; k < ndof; ++k) {
      x[k] = double(k + 1) / double(10) +
             0.2 * (k / nparticles) * (1 + distribution(generator));
    }
    ...
    const double L =
        2 * std::max<double>(
                fabs(*std::max_element(x.data(), x.data() + ndof)),
...
TEST_F(CellListsTestMoreHS_WCA, Number_of_neighbors_LeesEdwards) {
  for (double shear = 0.0; shear <= 1.0; shear += 0.01) {
    pele::CellLists<pele::leesedwards_distance<3>> cell(
        std::make_shared<pele::leesedwards_distance<3>>(boxvec, shear), boxvec,
        boxvec[0]);
    pele::CellLists<pele::leesedwards_distance<3>> cell2(
        std::make_shared<pele::leesedwards_distance<3>>(boxvec, shear), boxvec,
        boxvec[0], 1);
    pele::CellLists<pele::leesedwards_distance<3>> cell3(
        std::make_shared<pele::leesedwards_distance<3>>(boxvec, shear), boxvec,
        boxvec[0], 2);
    pele::CellLists<pele::leesedwards_distance<3>> cell4(
        std::make_shared<pele::leesedwards_distance<3>>(boxvec, shear), boxvec,
        boxvec[0], 4);
    size_t count = nparticles * (nparticles - 1) / 2;
    ASSERT_EQ(count, get_nr_unique_pairs(x, cell));
...
TEST_F(CellListsTestMoreHS_WCA, EnergyMoreParticles_Works) {
  ...
  pele::InversePowerPeriodicCellLists<3> pot_cell2_(pow, eps, radii, boxvec,
                                                    .2);
  pele::InversePowerPeriodicCellLists<3> pot_cell3_(pow, eps, radii, boxvec,
                                                    .3);
...
//         EXPECT_LE(-0.5 * boxvec[0], x[ii]);
//     }
//     pele::InversePowerCellLists<3> pot_cell(pow, eps, radii, boxvec, rcut,
//     .1); pele::InversePowerCellLists<3> pot_cell2(pow, eps, radii, boxvec,
//     rcut, .2); pele::InversePowerCellLists<3> pot_cell3(pow, eps, radii,
//     boxvec, rcut, .3); pele::InversePowerCellLists<3> pot_cell_(pow, eps,
//     radii, boxvec, boxvec[0], 1); pele::InversePowerCellLists<3>
//     pot_cell2_(pow, eps, radii, boxvec, boxvec[0], 2);
//     pele::InversePowerCellLists<3> pot_cell3_(pow, eps, radii, boxvec,
//     boxvec[0], 3); pele::InversePower<3> pot(pow, eps, radii); const double
//     ecell = pot_cell.get_energy(x); const double ecell2 =
...
TEST_F(CellListsTestMoreHS_WCA, HSWCAEnergy_Works) {
  ...
  for (size_t factor = 1; factor < 2; ++factor) {
    pele::HS_WCAPeriodicCellLists<3> pot_cellA(eps, sca, radii, boxvec,
                                               factor * 0.01);
    pele::HS_WCAPeriodicCellLists<3> pot_cellB(eps, sca, radii, boxvec,
                                               (factor + 0.01) * 0.01);
...
TEST_F(CellListsTestMoreHS_WCA, HSWCAEnergyLeesEdwards_Works) {
  for (double shear = 0.0; shear <= 1.0; shear += 0.01) {
    ...
    for (size_t factor = 1; factor < 2; ++factor) {
      ...
      pele::HS_WCALeesEdwardsCellLists<3> pot_cell1(eps, sca, radii, boxvec,
                                                    shear);
      const double e_cell1 = pot_cell1.get_energy(x);
      pele::HS_WCALeesEdwardsCellLists<3> pot_cellA(eps, sca, radii, boxvec,
                                                    shear, factor);
      pele::HS_WCALeesEdwardsCellLists<3> pot_cellB(eps, sca, radii, boxvec,
                                                    shear);
      const double e_cellA = pot_cellA.get_energy(x);
      const double e_cellB = pot_cellB.get_energy(x);
...
TEST_F(CellListsTestMoreHS_WCA, HSWCAEnergyGradientLeesEdwards_Works) {
  for (double shear = 0.0; shear <= 1.0; shear += 0.01) {
    ...
    for (size_t factor = 1; factor < 3; ++factor) {
      pele::HS_WCALeesEdwardsCellLists<3> pot_cellA(eps, sca, radii, boxvec,
                                                    shear, (factor)*0.2);
      pele::HS_WCALeesEdwardsCellLists<3> pot_cellB(
          eps, sca, radii, boxvec, shear, (factor + 0.2) * 0.2);
      pele::Array<double> g_no_cells(x.size());
      pele::Array<double> g_cellA(x.size());
      pele::Array<double> g_cellB(x.size());
...
TEST_F(CellListsTestMoreHS_WCA, HSWCAEnergyGradientHessian_Works) {
  ...
  for (size_t factor = 1; factor < 3; ++factor) {
    ...
    const double egh_cellA =
        pot_cellA.get_energy_gradient_hessian(x, g_cellA, h_cellA);
    const double egh_cellB =
        pot_cellB.get_energy_gradient_hessian(x, g_cellB, h_cellB);
...
TEST_F(CellListsTestMoreHS_WCA, HSWCAMinimization_Works) {
  ...
  auto pot_no_cells =
      std::make_shared<pele::HS_WCAPeriodic<3>>(eps, sca, radii, boxvec);
  auto pot_cells = std::make_shared<pele::HS_WCAPeriodicCellLists<3>>(
      eps, sca, radii, boxvec, 0.2);
...
TEST_F(CellListsTestMoreHS_WCA, HSWCAMinimizationLeesEdwards_Works) {
  for (double shear = 0.0; shear <= 1.0; shear += 0.01) {
    ...
    auto pot_no_cells = std::make_shared<pele::HS_WCALeesEdwards<3>>(
        eps, sca, radii, boxvec, shear);
    auto pot_cells = std::make_shared<pele::HS_WCALeesEdwardsCellLists<3>>(
        eps, sca, radii, boxvec, shear, 0.2);
    pele::MODIFIED_FIRE opt_no_cells(pot_no_cells, x, .1, 1, 1);
    pele::MODIFIED_FIRE opt_cells(pot_cells, x, .1, 1, 1);
    opt_no_cells.run();
    opt_cells.run();
...
TEST_F(CellListsTestMoreHS_WCA, HSWCAMinimizationCartesian_Works) {
  ...
  auto pot_cells =
      std::make_shared<pele::HS_WCACellLists<3>>(eps, sca, radii, boxvec, 0.2);
  pele::MODIFIED_FIRE opt_no_cells(pot_no_cells, x, .1, 1, 1);
  pele::MODIFIED_FIRE opt_cells(pot_cells, x, .1, 1, 1);
  opt_no_cells.run();
  opt_cells.run();
...
public:
  ...
  virtual void SetUp() {
    ...
    for (size_t k = 0; k < ndof; ++k) {
      x[k] = double(k + 1) / double(10) +
             0.3 * (k / nparticles) * (1 + distribution(generator));
...
TEST_F(CellListsTestMoreHS_WCA2D, Number_of_neighbors) {
  ...
  pele::CellLists<pele::periodic_distance<2>> cell2(
      std::make_shared<pele::periodic_distance<2>>(boxvec), boxvec, boxvec[0],
      1);
  pele::CellLists<pele::periodic_distance<2>> cell3(
      std::make_shared<pele::periodic_distance<2>>(boxvec), boxvec, boxvec[0],
      3);
  pele::CellLists<pele::periodic_distance<2>> cell4(
      std::make_shared<pele::periodic_distance<2>>(boxvec), boxvec, boxvec[0],
      5);
  size_t count = (nparticles * (nparticles - 1) / 2);
  ASSERT_EQ(count, get_nr_unique_pairs(x, cell));
...
TEST_F(CellListsTestMoreHS_WCA2D, HSWCAEnergy_Works) {
  ...
  for (size_t factor = 1; factor < 3; ++factor) {
    ...
    pele::HS_WCAPeriodicCellLists<2> pot_cellB(eps, sca, radii, boxvec,
                                               factor + 0.2);
    const double e_cellA = pot_cellA.get_energy(x);
    const double e_cellB = pot_cellB.get_energy(x);
    EXPECT_DOUBLE_EQ(e_no_cells, e_cellA);
...
TEST_F(CellListsTestMoreHS_WCA2D, HSWCAEnergyLeesEdwards_Works) {
  for (double shear = 0.0; shear <= 1.0; shear += 0.01) {
    ...
    const double e_no_cells = pot_no_cells.get_energy(x);
    for (size_t factor = 1; factor < 3; ++factor) {
      pele::HS_WCALeesEdwardsCellLists<2> pot_cellA(eps, sca, radii, boxvec,
                                                    shear, factor);
      pele::HS_WCALeesEdwardsCellLists<2> pot_cellB(eps, sca, radii, boxvec,
                                                    shear, factor + 0.2);
      const double e_cellA = pot_cellA.get_energy(x);
      const double e_cellB = pot_cellB.get_energy(x);
      EXPECT_DOUBLE_EQ(e_no_cells, e_cellA);
...
TEST_F(CellListsTestMoreHS_WCA2D, HSWCAMinimzation_Works) {
  auto pot_no_cells =
      std::make_shared<pele::HS_WCAPeriodic<2>>(eps, sca, radii, boxvec);
  auto pot_cells = std::make_shared<pele::HS_WCAPeriodicCellLists<2>>(
      eps, sca, radii, boxvec, 1);
  pele::MODIFIED_FIRE opt_no_cells(pot_no_cells, x, .1, 1, 1);
  pele::MODIFIED_FIRE opt_cells(pot_cells, x, .1, 1, 1);
  opt_no_cells.run();
  opt_cells.run();
