...
TEST_F(HarmonicTest, Works) {
  ...
  for (size_t k = 0; k < 100; ++k) {
    ...
    const double e_pair =
        pele::harmonic_interaction(k).energy(pos_int_pow<2>(displ), 0);
...
TEST_F(HarmonicTest, COM_Works) {
  ...
  const double e_ini = pot->get_energy(xd);
  const double e_ini_true =
      0.5 * k * nr_particles * box_dimension * pos_int_pow<2>(displ);
  EXPECT_NEAR_RELATIVE(e_ini, e_ini_true, 1e-10);
}
...
public:
  ...
  virtual void setup_potential() {
    ...
    pot = std::shared_ptr<pele::BasePotential>(
        new pele::HarmonicAtomList(k, atoms));
  }
...
TEST_F(HarmonicAtomListTest, Energy_Works) { test_energy(); }
...
public:
  virtual void setup_potential() {
    ...
    pot = std::shared_ptr<pele::BasePotential>(
        new pele::HarmonicNeighborList(k, nlist));
  }
};

TEST_F(HarmonicNeighborListTest, Energy_Works) { test_energy(); }
