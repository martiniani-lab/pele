...
size_t get_nr_unique_pairs(Array<double> coords,
...
TEST_F(CellListsTest, Number_of_neighbors) {
  pele::CellLists<> cell(std::make_shared<pele::periodic_distance<3>>(boxvec),
                         boxvec, boxvec[0]);
  pele::CellLists<> cell2(std::make_shared<pele::periodic_distance<3>>(boxvec),
                          boxvec, boxvec[0], 1);
  pele::CellLists<> cell3(std::make_shared<pele::periodic_distance<3>>(boxvec),
                          boxvec, boxvec[0], 4.2);
  pele::CellLists<> cell4(std::make_shared<pele::periodic_distance<3>>(boxvec),
                          boxvec, boxvec[0], 5);
  size_t count = 3u;
...
TEST_F(CellListsTest, Number_of_neighbors_Cartesian) {
  pele::CellLists<pele::cartesian_distance<3>> cell(
      std::make_shared<pele::cartesian_distance<3>>(), boxvec, boxvec[0]);
  pele::CellLists<pele::cartesian_distance<3>> cell2(
      std::make_shared<pele::cartesian_distance<3>>(), boxvec, boxvec[0], 1);
  pele::CellLists<pele::cartesian_distance<3>> cell3(
      std::make_shared<pele::cartesian_distance<3>>(), boxvec, boxvec[0], 4.2);
  pele::CellLists<pele::cartesian_distance<3>> cell4(
      std::make_shared<pele::cartesian_distance<3>>(), boxvec, boxvec[0], 5);
  size_t count = 3u;
  ASSERT_EQ(count, get_nr_unique_pairs(x, cell));
  ASSERT_EQ(count, get_nr_unique_pairs(x, cell2));
  ASSERT_EQ(count, get_nr_unique_pairs(x, cell3));
...
TEST_F(CellListsTest, NumberNeighborsDifferentRcut_WorksLeesEdwards) {
  for (double shear = 0.0; shear <= 1.0; shear += 0.01) {
    ...
    pele::CellLists<pele::leesedwards_distance<3>> cell(dist, boxvec,
                                                        boxvec[0]);
    pele::CellLists<pele::leesedwards_distance<3>> cell2(dist, boxvec,
                                                         boxvec[0], 1);
    pele::CellLists<pele::leesedwards_distance<3>> cell3(dist, boxvec,
                                                         boxvec[0], 4.2);
    pele::CellLists<pele::leesedwards_distance<3>> cell4(dist, boxvec,
                                                         boxvec[0], 5);
    size_t count_ref = get_direct_nr_unique_pairs(dist, boxvec[0], x);
    size_t count = get_nr_unique_pairs(x, cell);
    size_t count2 = get_nr_unique_pairs(x, cell2);
...
TEST_F(CellListsTest, NumberNeighborsDifferentRcut_WorksCartesian) {
  ...
  pele::CellLists<pele::cartesian_distance<3>> cell2(dist, boxvec, boxvec[0],
                                                     1);
  pele::CellLists<pele::cartesian_distance<3>> cell3(dist, boxvec, boxvec[0],
                                                     4.2);
  pele::CellLists<pele::cartesian_distance<3>> cell4(dist, boxvec, boxvec[0],
                                                     5);
...
TEST_F(CellListsTest, getEnergyGradient_nan) {
  ...
  ASSERT_TRUE(isnan(e));
  ASSERT_TRUE(
      std::all_of(g.begin(), g.end(), [](double elem) { return isnan(elem); }));
}
...
TEST_F(CellListsTest, getEnergyGradientHessian_nan) {
  ...
  ASSERT_TRUE(
      std::all_of(g.begin(), g.end(), [](double elem) { return isnan(elem); }));
  ASSERT_TRUE(
      std::all_of(h.begin(), h.end(), [](double elem) { return isnan(elem); }));
}
...
TEST_F(CellListsTest, Energy_Works) {
  pele::InversePowerPeriodicCellLists<3> pot_cell(pow, eps, radii, boxvec, 1.0);
  pele::InversePowerPeriodicCellLists<3> pot_cell2(pow, eps, radii, boxvec,
                                                   2.0);
  pele::InversePowerPeriodicCellLists<3> pot_cell3(pow, eps, radii, boxvec,
                                                   3.0);
  pele::InversePowerPeriodicCellLists<3> pot_cell4(pow, eps, radii, boxvec,
                                                   4.0);
...
//     pele::InversePowerCellLists<3> pot_cell(pow, eps, radii, boxvec,
//     rcut, 1.0); pele::InversePowerCellLists<3> pot_cell2(pow, eps, radii,
//     boxvec, rcut, 2.0); pele::InversePowerCellLists<3> pot_cell3(pow, eps,
//     radii, boxvec, rcut, 3.0); pele::InversePowerCellLists<3> pot_cell4(pow,
//     eps, radii, boxvec, rcut, 4.0); pele::InversePower<3> pot(pow, eps,
//     radii); const double ecell = pot_cell.get_energy(x); const double ecell2
//     = pot_cell2.get_energy(x); const double ecell3 = pot_cell3.get_energy(x);
//     const double ecell4 = pot_cell4.get_energy(x);
//     const double etrue = pot.get_energy(x);
//     ASSERT_NEAR(ecell, etrue, 1e-10);
//     ASSERT_NEAR(ecell2, etrue, 1e-10);
...
TEST_F(CellListsTest, EnergyGradientHessian_AgreesWithNumerical) {
  ...
  for (size_t i = 0; i < 3; ++i) {
    pele::InversePowerPeriodicCellLists<3> pot(pow, eps, radii, boxvec,
                                               1.0 + i);
...
//     pot_no_cells.get_energy_gradient_hessian(x, g_no_cells, h_no_cells);
//     for (size_t i = 0; i < 3; ++i) {
//         pele::InversePowerCellLists<3> pot(pow, eps, radii, boxvec, rcut, 1.0
//         + i); Array<double> h(x.size() * x.size()); Array<double>
//         hnum(h.size()); const double e = pot.get_energy_gradient_hessian(x,
//         g, h); const double ecomp = pot.get_energy(x);
//         pot.numerical_gradient(x, gnum);
//         pot.numerical_hessian(x, hnum);
//         EXPECT_NEAR(e, ecomp, 1e-10);
//         EXPECT_NEAR(etrue, ecomp, 1e-10);
...
TEST_F(CellListsTest, HS_WCAEnergyLeesEdwards_Works) {
  for (double shear = 0.0; shear <= 1.0; shear += 0.01) {
    pele::HS_WCALeesEdwardsCellLists<3> pot_cell(eps, sca, radii, boxvec, shear,
                                                 1);
    pele::HS_WCALeesEdwardsCellLists<3> pot_cell2(eps, sca, radii, boxvec,
                                                  shear, 1.1);
    pele::HS_WCALeesEdwardsCellLists<3> pot_cell3(eps, sca, radii, boxvec,
                                                  shear, 1.2);
    pele::HS_WCALeesEdwardsCellLists<3> pot_cell4(eps, sca, radii, boxvec,
                                                  shear, 1.3);
...
TEST_F(CellListsTest, Norm_SimpleTest) {
  ...
  pele::Array<double> x = {0.0, 1.0, 1.0, 3.0, 4.0, 0.0, 2.0, 2.0, 2.0,
                           3.0, 0.0, 2.0, 4.0, 1.0, 2.0, 3.0, 3.0, 2.0};
...
public:
  ...
  void SetUp() {
    ...
    distribution = std::uniform_real_distribution<double>(-0.5 * boxlength,
                                                          0.5 * boxlength);
...
TEST_F(CellListsTestHomogeneous3D, GridAndSpacing_Works) {
  pele::CellLists<> cell_one(
      std::make_shared<pele::periodic_distance<3>>(boxvec), boxvec, boxvec[0]);
  cell_one.update(x);
  ...
  // << "\n";
  pele::CellLists<> cell_two(
      std::make_shared<pele::periodic_distance<3>>(boxvec), boxvec,
      boxvec[0] / 2);
  cell_two.update(x);
...
TEST_F(CellListsTestHomogeneous3D, GridAndSpacingLeesEdwards_Works) {
  for (double shear = 0.0; shear <= 1.0; shear += 0.01) {
    pele::CellLists<pele::leesedwards_distance<3>> cell_one(
        std::make_shared<pele::leesedwards_distance<3>>(boxvec, shear), boxvec,
        boxvec[0]);
    cell_one.update(x);
    EXPECT_EQ(cell_one.get_nr_cells(), 1u);
    EXPECT_EQ(cell_one.get_nr_cellsx(), 1u);
    // std::cout << "nr_unique_pairs: one:\n" << get_nr_unique_pairs(x,
    // cell_one) << "\n";
...
TEST_F(CellListsTestHomogeneous3D, GridAndSpacingCartesian_Works) {
  pele::CellLists<pele::cartesian_distance<3>> cell_one(
      std::make_shared<pele::cartesian_distance<3>>(), boxvec, boxvec[0]);
  EXPECT_EQ(cell_one.get_nr_cells(), 1u);
  EXPECT_EQ(cell_one.get_nr_cellsx(), 1u);
  // std::cout << "nr_unique_pairs: one:\n" << get_nr_unique_pairs(x, cell_one)
  // << "\n";
  pele::CellLists<pele::cartesian_distance<3>> cell_two(
      std::make_shared<pele::cartesian_distance<3>>(), boxvec, boxvec[0] / 2);
  EXPECT_EQ(cell_two.get_nr_cells(), 8u);
  EXPECT_EQ(cell_two.get_nr_cellsx(), 2u);
  // std::cout << "nr_unique_pairs: two:\n" << get_nr_unique_pairs(x, cell_two)
  // << "\n";
  pele::CellLists<pele::cartesian_distance<3>> cell_three(
      std::make_shared<pele::cartesian_distance<3>>(), boxvec, boxvec[0] / 3);
  EXPECT_EQ(cell_three.get_nr_cells(), 27u);
  EXPECT_EQ(cell_three.get_nr_cellsx(), 3u);
}
...
TEST_F(CellListsTestHomogeneous2D, GridAndSpacing_Works) {
  pele::CellLists<pele::periodic_distance<2>> cell_one(
      std::make_shared<pele::periodic_distance<2>>(boxvec), boxvec, boxvec[0]);
  EXPECT_EQ(cell_one.get_nr_cells(), 1u);
  EXPECT_EQ(cell_one.get_nr_cellsx(), 1u);
  // std::cout << "nr_unique_pairs: one:\n" << get_nr_unique_pairs(x, cell_one)
  // << "\n";
  pele::CellLists<pele::periodic_distance<2>> cell_two(
      std::make_shared<pele::periodic_distance<2>>(boxvec), boxvec,
      boxvec[0] / 2);
  EXPECT_EQ(cell_two.get_nr_cells(), 4u);
  EXPECT_EQ(cell_two.get_nr_cellsx(), 2u);
  // std::cout << "nr_unique_pairs: two:\n" << get_nr_unique_pairs(x, cell_two)
  // << "\n";
  pele::CellLists<pele::periodic_distance<2>> cell_three(
      std::make_shared<pele::periodic_distance<2>>(boxvec), boxvec,
      boxvec[0] / 3);
...
TEST_F(CellListsTestHomogeneous2D, GridAndSpacingCartesian_Works) {
  pele::CellLists<pele::cartesian_distance<2>> cell_one(
      std::make_shared<pele::cartesian_distance<2>>(), boxvec, boxvec[0]);
  EXPECT_EQ(cell_one.get_nr_cells(), 1u);
  EXPECT_EQ(cell_one.get_nr_cellsx(), 1u);
  // std::cout << "nr_unique_pairs: one:\n" << get_nr_unique_pairs(x, cell_one)
  // << "\n";
  pele::CellLists<pele::cartesian_distance<2>> cell_two(
      std::make_shared<pele::cartesian_distance<2>>(), boxvec, boxvec[0] / 2);
  EXPECT_EQ(cell_two.get_nr_cells(), 4u);
  EXPECT_EQ(cell_two.get_nr_cellsx(), 2u);
  // std::cout << "nr_unique_pairs: two:\n" << get_nr_unique_pairs(x, cell_two)
  // << "\n";
  pele::CellLists<pele::cartesian_distance<2>> cell_three(
      std::make_shared<pele::cartesian_distance<2>>(), boxvec, boxvec[0] / 3);
  EXPECT_EQ(cell_three.get_nr_cells(), 9u);
  EXPECT_EQ(cell_three.get_nr_cellsx(), 3u);
}
...
TEST_F(LatticeNeighborsTest, LargeRcut_Works) {
  ...
  for (size_t isubdom = 0; isubdom < lattice.m_nsubdoms; isubdom++) {
    count_neighbors +=
        pairs_inner[isubdom].size() + pairs_boundary[isubdom].size();
  }
...
TEST_F(LatticeNeighborsTest, SmallRcut_Works2) {
  ...
  ASSERT_EQ(count_neighbors,
            (neibs.size() - 1) * lattice.m_ncells / 2 + lattice.m_ncells);
}
...
TEST_F(LatticeNeighborsTest, positionToCellVec_BoxBoundaryWorks) {
  ...
  for (int i = -20; i <= 20; i++) {
    for (int j = -20; j <= 20; j++) {
      ...
      pele::VecN<ndim, size_t> cell_vec =
          lattice.position_to_cell_vec(coords.data());
...
template <size_t ndim>
void test_rectangular_cell_lists(const double rcut, Array<double> boxvec,
                                 Array<double> x) {
  periodic_distance<ndim> dist(boxvec);
  std::shared_ptr<pele::BasePotential> pot_c =
      std::make_shared<pele::LJCutPeriodicCellLists<ndim>>(4., 4., rcut, boxvec,
                                                           1);
  std::shared_ptr<pele::BasePotential> pot_p =
      std::make_shared<LJCutPeriodicN<ndim>>(4., 4., rcut, boxvec);
  ...
  std::shared_ptr<pele::GradientOptimizer> optimizer_c =
      std::make_shared<pele::MODIFIED_FIRE>(pot_c, x, 1, 1, .1);
  std::shared_ptr<pele::GradientOptimizer> optimizer_p =
      std::make_shared<pele::MODIFIED_FIRE>(pot_p, x, 1, 1, .1);
  optimizer_c->run();
  optimizer_p->run();
...
template <size_t ndim>
void get_boxvec_x0L(const double rcut, const size_t N, Array<double> &boxvec,
                    Array<double> &x) {
  ...
  for (size_t i = 0; i < N; ++i) {
    for (size_t k = 0; k < ndim; ++k) {
      x[i * ndim + k] =
          std::uniform_real_distribution<double>{0, boxvec[k]}(gen);
    }
  }
}

template <size_t ndim>
void get_boxvec_x05(const double rcut, const size_t N, Array<double> &boxvec,
                    Array<double> &x) {
  get_boxvec(rcut, ndim, boxvec);
  x = Array<double>(ndim * N);
  ...
  for (size_t i = 0; i < N; ++i) {
    for (size_t k = 0; k < ndim; ++k) {
      x[i * ndim + k] = std::uniform_real_distribution<double>{
          -0.5 * boxvec[k], 0.5 * boxvec[k]}(gen);
    }
  }
}

template <size_t ndim>
void get_boxvec_x_all(const double rcut, const size_t N, Array<double> &boxvec,
                      Array<double> &x) {
  get_boxvec(rcut, ndim, boxvec);
  x = Array<double>(ndim * N);
...
public:
  ...
  virtual void SetUp() {
    ...
    throw std::runtime_error(
        "This test only works with OpenMP enabled compilation");
#endif
    seed = 42;
    generator = std::mt19937_64(seed);
  ...
  void create_coords() {
    ...
    boxvec = Array<double>(
        ndim,
        std::max<double>(fabs(*std::max_element(x.data(), x.data() + ndof)),
                         fabs(*std::min_element(x.data(), x.data() + ndof))) +
            rcut);
  }
};
...
TEST_F(OpenMPCellListsTest, HSWCAEnergyLeesEdwards_Works) {
  for (size_t nthreads = 2; nthreads <= 4; nthreads++) {
    ...
    for (double shear = 0.0; shear <= 0.2; shear += 0.1) {
      ...
      pele::HS_WCALeesEdwardsCellLists<2> pot_cell(eps, sca, radii, boxvec,
                                                   shear, 1.0);
  ...
  for (size_t nthreads = 2; nthreads <= 4; nthreads++) {
    ...
    for (double shear = 0.0; shear <= 0.2; shear += 0.1) {
      ...
      for (size_t new_coords = 0; new_coords < 10; new_coords++) {
        ...
        const double egh_no_cells =
            pot_no_cells.get_energy_gradient_hessian(x, g_no_cells, h_no_cells);
        for (size_t rep_same = 0; rep_same < 100; rep_same++) {
          const double egh_cell =
              pot_cell.get_energy_gradient_hessian(x, g_cell, h_cell);
 ...
 *  Relaxation Dynamics in the Energy Landscape of Glass-Forming Liquids,
 *  PHYSICAL REVIEW X 12, 021001 (2022) (The soft sphere potential in the paper)
 *  This is in 2D, for the 3d version we don't really need non additivit
 */
TEST(CellLists, EnergyGradientHessianCalculationsWithNonAdditivityHarmonic) {
  constexpr size_t n_particles = 32;
  constexpr int ndim = 2;
  constexpr int pow = 2;
  constexpr int eps = 1.0;
  constexpr double non_additivity = 0.2;
  ...
  auto potential_with_cell_lists =
      std::make_shared<InverseIntPowerPeriodic<ndim, pow>>(
  ...
  double energy_with_cell_lists =
      potential_with_cell_lists->get_energy(quenched_coordinates);
  double energy_without_cell_lists = potential_without_cell_lists->get_energy(
