...
TEST_F(TestGaussianPot, OneGaussWorks) {
  ...
  std::shared_ptr<pele::GaussianPot> gauss =
      std::make_shared<pele::GaussianPot>(mean, cov);
  std::shared_ptr<pele::MODIFIED_FIRE> opt =
      std::make_shared<pele::MODIFIED_FIRE>(gauss, initial_coords, .1, 1, 1);
  opt->run(1000);
...
void energy_test(std::initializer_list<double> mean_,
                 std::initializer_list<double> cov_,
                 std::initializer_list<double> x_, const double e_true) {
...
void gradient_test(std::initializer_list<double> x_,
                   std::initializer_list<double> grad_true_) {
  pele::Array<double> x(x_);
...
TEST_F(TestGaussianPot, OneGaussWorksNonZero) {
  ...
  energy_test({0.1, 0.1, 0.1, 0.2}, {5, 4, 3, 2},
              {1. / 10., 2. / 10., 3. / 10., 4. / 10.}, -0.9822428825213733);
  gradient_test({0, 0, 0, 0}, {0, 0, 0, 0});
  gradient_test({0, 0.1, 0.2, 0.3}, {0, 0.09323938199059482, 0.1864787639811896,
                                     0.2797181459717845});
  gradient_test({1, 1, 1, 1}, {0.1353352832366127, 0.1353352832366127,
...
TEST_F(TestGaussianPotAuto, Energy_Works) { test_energy(); }
