...
TEST_F(CheckInitializerTest, FullRunPeriodic) {
  ...
  // We're going to only test for a single element assuming it applies to the
  // others
...
// {
//   double H0 = 0.1;              // Parameters match the defaults for the
//   python interface double maxstep = 0.1; double max_f_rise = 0.1; long
//   max_iter = 10000; int verbosity = 0; int iprint = -1;
//   std::shared_ptr<pele::HS_WCACellLists<_ndim> > potcell =
//   std::make_shared<pele::HS_WCACellLists<_ndim> >(eps, sca, hs_radii,
//   boxvec, 1.0); std::shared_ptr<pele::HS_WCA<_ndim> > pot =
//   std::make_shared<pele::HS_WCA<_ndim> >(eps, sca, hs_radii); pele::LBFGS
//   optimizer_lbfgs(potcell, x); optimizer_lbfgs.set_H0(H0);
//   optimizer_lbfgs.set_maxstep(maxstep);
//   optimizer_lbfgs.set_max_f_rise(max_f_rise);
//   optimizer_lbfgs.set_max_iter(max_iter);
//   optimizer_lbfgs.set_verbosity(verbosity);
...
//   double H0 = 0.1;              // Parameters match the defaults for the
//   python interface double maxstep = 0.1; double max_f_rise = 0.1; long
//   max_iter = 2; int verbosity = 0; int iprint = -1;

//   std::shared_ptr<pele::HS_WCAPeriodicCellLists<_ndim> > potcell =
//   std::make_shared<pele::HS_WCAPeriodicCellLists<_ndim> >(eps, sca, hs_radii,
//   boxvec, 1.0); std::shared_ptr<pele::HS_WCAPeriodic<_ndim> > pot =
//   std::make_shared<pele::HS_WCAPeriodic<_ndim> >(eps, sca, hs_radii, boxvec);
