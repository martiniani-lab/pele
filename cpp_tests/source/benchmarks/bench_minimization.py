...
from pele.optimize._quench import lbfgs_cpp, lbfgs_py, lbfgs_scipy, mylbfgs
...
potentials = {LJcpp: "cpp LJ", LJfortran: "fortran LJ"}
...
def do_benchmarks(natoms=1000, prog="./benchmarks/bench_minimization"):
    coords = np.random.uniform(-1, 1, 3 * natoms) * 1.5 * natoms ** (1.0 / 3)
    lj = LJcpp()
    ...
    print("\n")
    for pot, pot_name in potentials.items():
        for opt, opt_name in optimizers.items():
            p = pot()
            t0 = time.time()
            opt(coords.copy(), p, **opt_kwargs)
            t1 = time.time()
            print(
                "{pot_name:10}: {opt_name:13}: time {time}".format(
                    pot_name=pot_name, opt_name=opt_name, time=t1 - t0
                )
            )
            sys.stdout.flush()
