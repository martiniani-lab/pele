...
TEST_F(LowestEigPotentialTest, lowesteigtest_works) {
  ...
  _x = _lbfgs->get_x();
  _g = _lbfgs->get_g();
  _lowesteigpot = std::shared_ptr<pele::LowestEigPotential>(
      new pele::LowestEigPotential(_potential, _x, _bdim));
  std::shared_ptr<pele::LBFGS> lbfgs = std::shared_ptr<pele::LBFGS>(
      new pele::LBFGS(_lowesteigpot, _ranvec, _tol, _M));
...
public:
  ...
  virtual void SetUp() {
    ...
    _bdim = 3;
    _ndim = _natoms * _bdim;
    _orthog = std::shared_ptr<pele::Orthogonalize>(
        new pele::OrthogonalizeTranslational(_natoms, _bdim));
    ev0 = Array<double>(_ndim);
    ev1 = Array<double>(_ndim);
    ev2 = Array<double>(_ndim);
    ev0.assign(0.0);
