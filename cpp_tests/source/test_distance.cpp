...
TEST_F(DistanceTest, CartesianDistanceNorm_Works) {
  for (size_t i_repeat = 0; i_repeat < TEST_REPEAT; i_repeat++) {
    ...
    double dx_p_3[3];
    double dx_p_42[42];
    cartesian_distance<2>().get_rij(dx_p_2, x2[i_repeat].data(),
                                    y2[i_repeat].data());
    cartesian_distance<3>().get_rij(dx_p_3, x3[i_repeat].data(),
                                    y3[i_repeat].data());
    cartesian_distance<42>().get_rij(dx_p_42, x42[i_repeat].data(),
                                     y42[i_repeat].data());
    double ds_p_2 = 0;
    double ds_p_3 = 0;
    double ds_p_42 = 0;
...
TEST_F(DistanceTest, NearestImageConvention_Works) {
  ...
  periodic_distance<42>(pele::Array<double>(42, L))
      .get_rij(delta42, &*ones.begin(), x_out_of_box42);
  const double d2_42_before =
      std::inner_product(delta42, delta42 + 42, delta42, double(0));
  ...
  periodic_distance<42>(pele::Array<double>(42, L))
      .get_rij(delta42, &*ones.begin(), x_boxed_per42);
  const double d2_42_after =
      std::inner_product(delta42, delta42 + 42, delta42, double(0));
...
TEST_F(DistanceTest, PeriodicPutAtomInBox_BoxBoundaryWorks) {
  ...
  for (int i = -20; i <= 20; i++) {
    for (int j = -20; j <= 20; j++) {
      ...
      periodic_distance<2>(boxvec).put_atom_in_box(new_coords.data(),
                                                   coords.data());
      ...
      for (int i_y = -21; i_y <= 21; i_y += 2) {
        for (int j_y = -20; j_y <= 20; j_y++) {
          ...
          coords[0] = i_x * boxboundary +
                      j_x * std::numeric_limits<double>::epsilon() + shear_corr;
          coords[1] =
              i_y * boxboundary + j_y * std::numeric_limits<double>::epsilon();
          leesedwards_distance<2>(boxvec, shear)
              .put_atom_in_box(new_coords.data(), coords.data());
...
TEST_F(DistanceTest, SimplePeriodicNorm_Works) {
  ...
  for (size_t i_repeat = 0; i_repeat < TEST_REPEAT; i_repeat++) {
    ...
    periodic_distance<2>(bv2).get_rij(dx_periodic_2d, x2[i_repeat].data(),
                                      y2[i_repeat].data());
    periodic_distance<3>(bv3).get_rij(dx_periodic_3d, x3[i_repeat].data(),
                                      y3[i_repeat].data());
    periodic_distance<42>(bv42).get_rij(dx_periodic_42d, x42[i_repeat].data(),
                                        y42[i_repeat].data());
...
/** Check the Lees Edward-image convention via the put_atom_in_box method using
 * simple examples.
 */
TEST_F(DistanceTest, LeesEdwards_Image_Y) {
  ...
  double r_test[][2] = {
      {3.7, 6.5}, {-2.2, 8.5}, {3.8, -7.7}, {-2.2, -6.9}, {4.8, -8.5}};
  double r_new[5][2];
  double r_exp[][2] = {
      {2.7, -3.5}, {-3.2, -1.5}, {4.8, 2.3}, {-1.2, 3.1}, {-4.2, 1.5}};
...
/** Check the Lees Edward-image convention via the put_in_box method using
 * simple examples.
 */
TEST_F(DistanceTest, LeesEdwards_Image_NotY) {
  ...
  for (size_t i_repeat = 0; i_repeat < TEST_REPEAT; i_repeat++) {
    ...
    leesedwards_distance<2>(bv2, 0).put_atom_in_box(x2_new.data(),
                                                    x2[i_repeat].data());
    leesedwards_distance<3>(bv3, 0).put_atom_in_box(x3_new.data(),
                                                    x3[i_repeat].data());
    leesedwards_distance<42>(bv42, 0).put_atom_in_box(x42_new.data(),
                                                      x42[i_repeat].data());
    ...
    periodic_distance<2>(bv2).put_atom_in_box(x_periodic_2d_new.data(),
                                              x_periodic_2d.data());
    periodic_distance<3>(bv3).put_atom_in_box(x_periodic_3d_new.data(),
                                              x_periodic_3d.data());
    periodic_distance<42>(bv42).put_atom_in_box(x_periodic_42d_new.data(),
                                                x_periodic_42d.data());
...
TEST_F(DistanceTest, LeesEdwards_NoShear) {
  ...
  for (size_t i_repeat = 0; i_repeat < TEST_REPEAT; i_repeat++) {
    ...
    leesedwards_distance<2>(bv2, 0).get_rij(
        dx_leesedwards_2d, x2[i_repeat].data(), y2[i_repeat].data());
    leesedwards_distance<3>(bv3, 0).get_rij(
        dx_leesedwards_3d, x3[i_repeat].data(), y3[i_repeat].data());
    leesedwards_distance<42>(bv42, 0).get_rij(
        dx_leesedwards_42d, x42[i_repeat].data(), y42[i_repeat].data());
...
TEST_F(DistanceTest, LeesEdwards_ShearPeriodic) {
  ...
  for (int shear = 0; shear < 10; shear++) {
    for (size_t i_repeat = 0; i_repeat < TEST_REPEAT; i_repeat++) {
      ...
      double dx_leesedwards_3d[3];
      double dx_leesedwards_42d[42];
      leesedwards_distance<2>(bv2, shear)
          .get_rij(dx_leesedwards_2d, x2[i_repeat].data(), y2[i_repeat].data());
      leesedwards_distance<3>(bv3, shear)
          .get_rij(dx_leesedwards_3d, x3[i_repeat].data(), y3[i_repeat].data());
      leesedwards_distance<42>(bv42, shear)
          .get_rij(dx_leesedwards_42d, x42[i_repeat].data(),
                   y42[i_repeat].data());
      ...
      /* Compare distances:
       * Can't assert to full precision, since we loose precision when adding
       * dx, which is a multiple of BOX_LENGTH and might therefore be far larger
       * than the distance in x-direction.
       */
      for (size_t i = 0; i < 2; i++) {
...
TEST_F(DistanceTest, LeesEdwards_Shear) {
  ...
  for (double shear = 0.1; shear <= 1.0; shear += 0.1) {
    for (size_t i_repeat = 0; i_repeat < TEST_REPEAT; i_repeat++) {
      ...
      cartesian_distance<2>().get_rij(dx_cartesian_2d, x2[i_repeat].data(),
                                      y2[i_repeat].data());
      cartesian_distance<3>().get_rij(dx_cartesian_3d, x3[i_repeat].data(),
                                      y3[i_repeat].data());
      cartesian_distance<42>().get_rij(dx_cartesian_42d, x42[i_repeat].data(),
                                       y42[i_repeat].data());
      ...
      dx_final_2d[0] =
          dx_periodic_2d[0] -
          round(dx_cartesian_2d[1] / BOX_LENGTH) * shear * BOX_LENGTH;
      dx_final_2d[1] = dx_cartesian_2d[1] -
                       round(dx_cartesian_2d[1] / BOX_LENGTH) * BOX_LENGTH;
      dx_final_2d[0] =
          dx_final_2d[0] - round(dx_final_2d[0] / BOX_LENGTH) * BOX_LENGTH;
      double *dx_final_3d;
      dx_final_3d = dx_periodic_3d;
      dx_final_3d[0] =
          dx_periodic_3d[0] -
          round(dx_cartesian_3d[1] / BOX_LENGTH) * shear * BOX_LENGTH;
      dx_final_3d[1] = dx_cartesian_3d[1] -
                       round(dx_cartesian_3d[1] / BOX_LENGTH) * BOX_LENGTH;
      dx_final_3d[0] =
          dx_final_3d[0] - round(dx_final_3d[0] / BOX_LENGTH) * BOX_LENGTH;
      double *dx_final_42d;
      dx_final_42d = dx_periodic_42d;
      dx_final_42d[0] =
          dx_periodic_42d[0] -
          round(dx_cartesian_42d[1] / BOX_LENGTH) * shear * BOX_LENGTH;
      dx_final_42d[1] = dx_cartesian_42d[1] -
                        round(dx_cartesian_42d[1] / BOX_LENGTH) * BOX_LENGTH;
      dx_final_42d[0] =
          dx_final_42d[0] - round(dx_final_42d[0] / BOX_LENGTH) * BOX_LENGTH;
