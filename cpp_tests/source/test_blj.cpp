...
public:
  ...
  virtual void setup_potential() {
    ...
    compot->add_potential(
        std::make_shared<pele::LJCutAtomList>(c6, c12, rcut, atomsA));
    compot->add_potential(
        std::make_shared<pele::LJCutAtomList>(c6, c12, rcut, atomsA, atomsB));
    compot->add_potential(
        std::make_shared<pele::LJCutAtomList>(c6, c12, rcut, atomsB));
