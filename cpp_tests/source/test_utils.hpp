...
template <typename T>
::testing::AssertionResult almostEqual(const T a, const T b,
                                       int maxULPsDiff = 4) {
  ...
  if (std::abs(max_frac - scaled_min_frac) <=
      maxULPsDiff * limits::epsilon() * 0.5)
    return ::testing::AssertionSuccess();

  double ulp_diff =
      std::abs(max_frac - scaled_min_frac) / (limits::epsilon() * 0.5);
  return ::testing::AssertionFailure()
         << a << " != " << b << " (" << ulp_diff << " ULPs difference)";
}
