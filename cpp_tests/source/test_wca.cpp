...
TEST_F(WCATest, Energy_Works) { test_energy(); }
TEST_F(WCATest, EnergyGradient_AgreesWithNumerical) { test_energy_gradient(); }
...
public:
  virtual void setup_potential() {
    ...
    pot = std::shared_ptr<pele::BasePotential>(
        new pele::WCAAtomList(sig, eps, atoms));
  }
};

TEST_F(WCAAtomListTest, Energy_Works) { test_energy(); }
...
public:
  ...
  double operator()(const double r) const {
    ...
    return std::max<double>(
        0,
        epsilon * (4 * (pos_int_pow<12>(ratio) - pos_int_pow<6>(ratio)) + 1));
  }
  double grad(const double r) const {
    ...
    return (-8 * epsilon * r * sigma()) /
           pos_int_pow<2>(pos_int_pow<2>(r) - pos_int_pow<2>(r_sum)) *
           (12 * pos_int_pow<11>(g(r)) - 6 * pos_int_pow<5>(g(r)));
  }
  // This is not the gradient. Rather grad(r) / (-r).
  double scaled_grad(const double r) const { return grad(r) / (-r); }
  double sigma() const {
    return (2 * alpha + pos_int_pow<2>(alpha)) * pos_int_pow<2>(r_sum) *
           std::pow(2, -static_cast<double>(1) / static_cast<double>(6));
  }
  double g(const double r) const {
...
TEST_F(HS_WCATest, ExtendedEnergyTest_Works) {
  ...
  for (size_t i = 0; i < nr_points; ++i) {
    ...
    const double e_pair_pot_f_energy =
        pair_pot.energy(pos_int_pow<2>(r), r_sum);
    double gab;
    const double e_pair_pot_f_energy_gradient =
        pair_pot.energy_gradient(pos_int_pow<2>(r), &gab, r_sum);
    double hab;
    const double e_pair_pot_f_energy_gradient_hessian =
        pair_pot.energy_gradient_hessian(pos_int_pow<2>(r), &gab, &hab, r_sum);
    EXPECT_LE(0, e_pair_pot_f_energy);
    EXPECT_TRUE(
        almostEqual(e_pair_pot_f_energy, e_pair_pot_f_energy_gradient, 45));
    EXPECT_LE(0, e_pair_pot_f_energy_gradient);
    EXPECT_TRUE(almostEqual(e_pair_pot_f_energy,
                            e_pair_pot_f_energy_gradient_hessian, 45));
  ...
  //pele::sf_HS_WCA_interaction<>(eps, sca).evaluate_pair_potential(rmin, rmax,
  nr_points, atom_a, atom_b, x_, y_); pele::sf_HS_WCA_interaction<>(eps,
  sca).evaluate_pair_potential(0.70348, 0.70352, scale * nr_points, atom_a,
  atom_b, x_, y_);
  ...
  */
  ...
  for (size_t i = 0; i < nr_points; ++i) {
    const double r = rmin + i * rdelta;
    double pair_pot_gab;
    const double pair_pot_e =
        pair_pot.energy_gradient(pos_int_pow<2>(r), &pair_pot_gab, r_sum);
    double sf_pair_pot_gab;
    const double sf_pair_pot_e =
        sf_pair_pot.energy_gradient(pos_int_pow<2>(r), &sf_pair_pot_gab, r_sum);
...
TEST_F(HS_WCATest, EnergyGradient_AgreesWithNumerical) {
  ...
  for (int exp_ind = 0; exp_ind < 6; exp_ind++) {
    double e = ((struct pele::BasePotential *)pots[exp_ind])
                   ->get_energy_gradient(x, g);
    double ecomp = ((struct pele::BasePotential *)pots[exp_ind])->get_energy(x);
    ASSERT_NEAR(e, ecomp, 1e-10);
    ((struct pele::BasePotential *)pots[exp_ind])
        ->numerical_gradient(x, gnum, 1e-6);
...
TEST_F(HS_WCATest, EnergyGradientHessian_AgreesWithNumerical) {
  ...
  for (int exp_ind = 0; exp_ind < 6; exp_ind++) {
    ...
    double e = ((struct pele::BasePotential *)pots[exp_ind])
                   ->get_energy_gradient_hessian(x, g, h);
    double ecomp = ((struct pele::BasePotential *)pots[exp_ind])->get_energy(x);
...
TEST_F(HS_WCATest, Norm_SimpleTest) {
  ...
  pele::Array<double> x = {0.0, 1.0, 3.0, 4.0, 2.0, 2.0,
                           3.0, 0.0, 4.0, 1.0, 3.0, 3.0};
