...
class MonteCarlo(object):
    ...
    def __init__(
        ...
        if confCheck is None:
            confCheck = []
        if event_after_step is None:
            event_after_step = []
    ...
    def _mcStep(self):
        ...
        #########################################################################
        if self.acceptstep:
            self.acceptstep = self.acceptTest(
                self.markovE, self.trial_energy, self.coords, self.trial_coords
            )
    ...
    def takeOneStep(self):
        ...
        if (
            self.storage
            and (self.insert_rejected or acceptstep)
            and self.config_ok
        ):
        ...
        if self.stepnum % self.printfrq == 0:
            if self.outstream != None:
                self.outstream.write(
                    "MCstep    %12d  E= %20.12g  markov E= %20.12g accepted= %s\n"
                    % (
                        self.stepnum,
                        self.trial_energy,
                        self.markovE_old,
...
if __name__ == "__main__":
    ...
    takestep = sys.get_takestep(stepsize=0.1)
