...
def make3dVector(u):
    ...
    if np.abs(np.linalg.norm(vec) - 1) > 1e-5:
        print(
            "make3dVector: vector not normalized", u, vec, np.linalg.norm(vec)
        )
    return vec
...
def makeGrad2(vec2, grad3):
    ...
    grad2[
        0
    ] *= s1  # I need this to agree with the numerical gradient, but I think it shouldn't be there
    return grad2
...
class HeisenbergModel(BasePotential):
    ...
    def __init__(self, dim=None, field_disorder=1.0, fields=None):
        if dim is None:
            dim = [4, 4]
        self.dim = copy(dim)
