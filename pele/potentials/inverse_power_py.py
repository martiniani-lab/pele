...
class PyInversePower(BasePotential):
    ...
    def getEnergy(self, coords):
        ...
        for i, j in itertools.combinations(range(len(coords)), 2):
            ...
            for box_index_x, box_index_y in itertools.product(
                [-1, 0, 1], repeat=self.ndim
            ):
                image_x = box_index_x * self.boxvec[0]
                image_y = box_index_y * self.boxvec[1]
                image_displacement = np.array([image_x, image_y])
                distance = np.linalg.norm(
                    coords_i - coords_j + image_displacement
                )
                if distance < radii_i + radii_j:
                    # calculate energy
                    energy += (
                        (1 - distance / (radii_i + radii_j)) ** self.pow
                        * self.eps
                        / self.pow
                    )
        return energy
...
if __name__ == "__main__":
    coords = np.array(
        [1.7299, 1.82661, 1.46248, 3.65475, 3.60927, 1.0818, 3.26551, 2.89476]
    )
    pot = PyInversePower(
        pow=2.5,
        eps=1.0,
