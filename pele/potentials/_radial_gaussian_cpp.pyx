...
cdef extern from "pele/radial_gaussian.hpp" namespace "pele":
    cdef cppclass cBaseRadialGaussian "pele::BaseRadialGaussian":
        cBaseRadialGaussian(_pele.Array[double] coords, double k, double l0, double log_prefactor, size_t ndim) except +
    ...
    cdef cppclass cRadialGaussian "pele::RadialGaussian":
        cRadialGaussian(_pele.Array[double] coords, double k, double l0, double log_prefactor, size_t ndim) except +
    cdef cppclass cRadialGaussianCOM "pele::RadialGaussianCOM":
        cRadialGaussianCOM(_pele.Array[double] coords, double k, double l0, double log_prefactor, size_t ndim) except +
...
cdef class RadialGaussian(_pele.BasePotential):
    ...
    def __cinit__(self, coords, k, l0, log_prefactor = 1.0, bdim=3, com=False):
        ...
        if self.com is True:
            self.thisptr = shared_ptr[_pele.cBasePotential]( <_pele.cBasePotential*>new cRadialGaussianCOM(_pele.Array[double](<double*> corigin.data, corigin.size), 
                                                                   self.k, self.l0, self.log_prefactor, self.bdim) )
        else:
