...
fields = np.array(
    [
        [-0.18602174, -0.9495507, 0.25248638],
...
class TestHeisenbergModel(_base_test.BaseTestCases._TestConfiguration):
    def setUp(self):
        self.pot = HeisenbergModel(dim=[L, L], field_disorder=0.0)
        self.x0 = coords
...
class TestXYModelDisorder(_base_test.BaseTestCases._TestConfiguration):
    def setUp(self):
        self.pot = XYModel(dim=[L, L], phi=1.0, periodic=False, phases=phases)
        self.x0 = _x0_xy
...
class TestHeisenbergModelRADisorder(
    _base_test.BaseTestCases._TestConfiguration
):
    def setUp(self):
