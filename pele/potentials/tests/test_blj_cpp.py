...
class TestBLJ_CPP(_base_test._BaseTest):
    def setUp(self):
        ...
        ntypeA, self.Emin, rcut, epsAA, sigAA, epsBB, sigBB, epsAB, sigAB = list(
            map(float, xyz.title.split()[1::2])
        )
        ntypeA = int(ntypeA)
        self.rcut = rcut
        ...
        self.xrandom = np.random.uniform(-1, 1, self.xmin.size) * 5.0
