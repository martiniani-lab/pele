...
class TestLJCutCellLists(_base_test._TestConfiguration):
    def setUp(self):
        ...
        self.pot = _lj_cpp.LJCutCellLists(boxvec=boxv, rcut=rcut, ncellx_scale=1.0)
        from pele.optimize import lbfgs_cpp

        ret = lbfgs_cpp(x, self.pot_true, tol=10.0)
        self.x0 = ret.coords
    ...
    def check_cell_density(self, ncellx_scale=1.0):
        # note: 11/2014: this becomes extremely slow for ncellx_scale > 2.
        self.pot = _lj_cpp.LJCutCellLists(
            boxvec=self.boxvec, rcut=self.rcut, ncellx_scale=ncellx_scale
        )
