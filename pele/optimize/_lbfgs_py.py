...
class LBFGS(object):
    ...
    def __init__(
        ...
        self.rel_energy = (
            rel_energy  # use relative energy comparison for maxErise
        )
    ...
    def _get_LBFGS_step_cython(self, G):
        ...
        return _cython_lbfgs._compute_LBFGS_step(
            G, self.s, self.y, self.rho, self.k, self.H0
        )

    def _get_LBFGS_step_fortran(self, G):
        from . import mylbfgs_updatestep

        ret = mylbfgs_updatestep.lbfgs_get_step_wrapper(
            G,
            self.s.reshape(-1),
            self.y.reshape(-1),
            self.rho,
    ...
    def adjustStepSize(self, X, E, G, stp):
        ...
        if np.dot(G, stp) > 0:
            if self.debug:
                overlap = (
                    np.dot(G, stp) / np.linalg.norm(G) / np.linalg.norm(stp)
                )
                self.logger.warn(
                    "LBFGS returned uphill step, reversing step direction: overlap {}".format(
                        overlap
                    )
                )
            stp = -stp
        ...
        if nincrease > 10:
            ...
            self.logger.warning(
                "lbfgs: having trouble finding a good step size. %s %s, resetting the minimizer",
                f * stepsize,
                stepsize,
            )
            self.reset()
    ...
    def _armijo_condition(self, Enew, Eold, Gold, step, return_overlap=False):
        ...
        if not armijo and self.debug:
            ...
            print(
                self.iter_number,
                "rejecting step due to energy",
                Enew,
                Enew - Eold,
                overlap_old * self._wolfe1,
                "stepsize",
                stepsize,
            )
        if return_overlap:
    ...
    def _wolfe_conditions(self, Enew, Eold, Gnew, Gold, step, strong=False):
        ...
        """
        armijo, overlap_old = self._armijo_condition(
            Enew, Eold, Gold, step, return_overlap=True
        )
        if not armijo:
            return False
        ...
        if not wolfe2 and self.debug:
            ...
            print(
                "wolfe:",
                self.iter_number,
                "rejecting step due to gradient",
                overlap_new,
                overlap_old,
                self._wolfe2,
                "stepsize",
    ...
    def one_iteration(self):
        ...
        Xnew, self.energy, Gnew = self.adjustStepSize(
            self.X, self.energy, self.G, stp
        )
    ...
    def stop_criterion_satisfied(self):
        ...
        if self.alternate_stop_criterion is None:
            ...
            return self.alternate_stop_criterion(
                energy=self.energy, gradient=self.G, tol=self.tol, coords=self.X
            )
    ...
    def run(self):
        ...
        """
        while (
            self.iter_number < self.nsteps
            and not self.stop_criterion_satisfied()
        ):
            try:
                self.one_iteration()
