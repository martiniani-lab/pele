...
from scipy.integrate import BDF, LSODA, RK23, RK45, Radau
from scipy.integrate.quadpack import quad_explain

from pele.optimize import (LBFGS, LBFGS_CPP, MYLBFGS, Fire, ModifiedFireCPP,
                           Result)

from .rk import RK23_LS, RK45_LS
...
def steepest_descent(
    x0, pot, iprint=-1, dx=1e-4, nsteps=10000000, tol=1e-4, maxstep=-1.0, events=None
):
    ...
    for k in range(nsteps):
        ...
        if iprint > 0:
            if funcalls % iprint == 0:
                print(
                    "step %8d energy %20.12g rms gradient %20.12g" % (funcalls, E, rms)
                )
