...
from OpenGL.GL import glEnable, glLightfv, glMaterialfv
...
class Show3D(QGLWidget):
    ...
    def __init__(self, parent):
        ...
        self.rotation = rot.aa2mx(np.array([0.0, 0.0, 0.0]))  # np.array([0., 0.])
        self.zoom = 1.0
    ...
    def mouseMoveEvent(self, event):
        ...
        if event.buttons() == Qt.LeftButton:
            drot = rot.aa2mx(-np.array([delta.y(), delta.x(), 0.0]))
            self.rotation = np.dot(self.rotation, drot)
        elif event.buttons() == Qt.RightButton:
            drot = rot.aa2mx(np.array([0.0, 0.0, delta.x()]))
            self.rotation = np.dot(self.rotation, drot)
    ...
    def initializeGL(self):
        ...
        lightZeroPosition = [-800.0, 500.0, 1500.0, 1.0]
        ...
        glLightfv(GL.GL_LIGHT1, GL.GL_POSITION, [100.0, 200.0, -20.0, 1.0])
        glLightfv(GL.GL_LIGHT1, GL.GL_DIFFUSE, [1.0, 1.0, 1.0, 1.0])
        glLightfv(GL.GL_LIGHT1, GL.GL_SPECULAR, [1.0, 1.0, 1.0, 1.0])
        glLightfv(GL.GL_LIGHT1, GL.GL_AMBIENT, [0.0, 0.0, 0.0, 1.0])
        ...
        GL.glShadeModel(GL.GL_SMOOTH)
        GL.glMatrixMode(GL.GL_PROJECTION)
        GLU.gluPerspective(40.0, 1.0, 1.0, 40.0)
        GL.glMatrixMode(GL.GL_MODELVIEW)
