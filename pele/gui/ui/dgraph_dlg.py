...
class LabelMinimumAction(QtGui.QAction):
    ...
    def __call__(self, val):
        ...
        #         dialog.setLabelText("")
        dialog.setLabelText(
            "set label for minimum: " + str(self.minimum.energy)
        )
...
class DGraphWidget(QWidget):
    ...
    def _get_input_parameters(self):
        ...
        params[
            "order_by_basin_size"
        ] = self.ui.chkbx_order_by_basin_size.isChecked()
    ...
    def _color_minimum_energy_path(self, m1, m2):
        ...
        for tree in all_trees:
            tree.data["colour"] = (1.0, 0.0, 0.0)
        self.redraw_disconnectivity_graph()

    def _color_by_mfpt(self, min1, T=1.0):
        print(
            "coloring by the mean first passage time to get to minimum",
            min1._id,
        )
    ...
    def _color_by_committor(self, min1, min2, T=1.0):
        print(
            "coloring by the probability that a trajectory gets to minimum",
            min1._id,
            "before",
            min2._id,
        )
    ...
    def _layout_by_committor(self, min1, min2, T=1.0):
        print(
            "coloring by the probability that a trajectory gets to minimum",
            min1._id,
            "before",
        ...
        print(
            "number of committors near 1",
            len([v for v in list(committors.values()) if v > 1.0 - 1e-4]),
        )
        print(
            "number of committors equal to 1",
            len([v for v in list(committors.values()) if v == 1.0]),
        )
    ...
    def _on_left_click_minimum(self, minimum):
        print(
            "you clicked on minimum with id",
            minimum._id,
            "and energy",
            minimum.energy,
        )
        ...
        self.ui.label_selected_minimum.setText(
            "%g (%d)" % (minimum.energy, minimum._id)
        )
    ...
    def _on_right_click_minimum(self, minimum):
        ...
        if self._selected_minimum is not None:
            action2 = ColorPathAction(
                minimum, self._selected_minimum, parent=self
            )
            menu.addAction(action2)

            menu.addAction(
                ColorCommittorAction(
                    minimum, self._selected_minimum, parent=self
                )
            )
            menu.addAction(
                LayoutByCommittorAction(
                    minimum, self._selected_minimum, parent=self
                )
            )
    ...
    def _draw_disconnectivity_graph(self, show_minima=True, show_trees=False):
        ...
        if show_trees:
            ...
            treepoints = ax.scatter(
                x_pos, energies, picker=5, color="red", alpha=0.5
            )
            ...
            self._tree_cid = self.canvas.mpl_connect(
                "pick_event", self._on_pick_tree
            )
        ...
        if show_minima:
            ...
            self._minima_cid = self.canvas.mpl_connect(
                "pick_event", self._on_pick_minimum
            )
