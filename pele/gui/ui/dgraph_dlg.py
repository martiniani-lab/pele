...
from PyQt4.QtGui import QApplication, QColorDialog, QInputDialog, QWidget
...
class DGraphWidget(QWidget):
    ...
    def _on_left_click_minimum(self, minimum):
        print(
            "you clicked on minimum with id", minimum._id, "and energy", minimum.energy
        )
    ...
    def _on_right_click_minimum(self, minimum):
        ...
        if self._selected_minimum is not None:
            ...
            menu.addAction(
                ColorCommittorAction(minimum, self._selected_minimum, parent=self)
            )
            menu.addAction(
                LayoutByCommittorAction(minimum, self._selected_minimum, parent=self)
            )
    ...
    def _draw_disconnectivity_graph(self, show_minima=True, show_trees=False):
        ...
        dg.plot(axes=ax, show_minima=False, linewidth=self.line_width, title=self.title)
