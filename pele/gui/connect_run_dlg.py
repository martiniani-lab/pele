    ...
    def __init__(self, system, database, min1=None, min2=None, parent=None, app=None):
        ...
        if min1 is not None and min2 is not None:
            self.decrunner = DECRunner(
                system, database, min1, min2, outstream=self.textEdit_writer
            )
        ...
        self.view_energies = self.new_view(
            "Energies", self.wgt_energies, QtCore.Qt.TopDockWidgetArea
        )
        self.ui.actionEnergy.setChecked(True)

        self.wgt_graphview = GraphViewWidget(
            database=self.database, parent=self, app=app
        )
        self.view_graphview = self.new_view(
            "Graph View", self.wgt_graphview, QtCore.Qt.TopDockWidgetArea
        )
    ...
    def make_graph(self):
        if self.wgt_graphview.isVisible() and self.decrunner.success:
            self.wgt_graphview.make_graph(
                database=self.decrunner.database, minima=self.decrunner.newminima
            )
