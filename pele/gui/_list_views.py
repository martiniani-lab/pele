    ...
    def addMinima(self, mlist):
        if self.nmax is not None:
            if len(mlist) > self.nmax:
                ...
                print(
                    "warning: limiting the number of minima displayed in the gui to",
                    self.nmax,
                )
        for m in mlist:
            self.addMinimum(m)
        ...
        if self.nmax is not None:
            ...
            if nrows > self.nmax:
                if not self.issued_warning:
                    print(
                        "warning: limiting the number of minima displayed in the"
                        " gui to",
                        self.nmax,
                    )
    ...
    def addTransitionStates(self, tslist):
        if self.nmax is not None:
            if len(tslist) > self.nmax:
                ...
                print(
                    "warning: limiting the number of transition states displayed in the"
                    " gui to",
                    self.nmax,
                )
        for ts in tslist:
...
class ListViewManager(object):
    def __init__(self, parent):
        ...
        self.ui.list_minima_main.selectionModel().selectionChanged.connect(
            self.on_list_minima_main_selectionChanged
        )
        self.ui.listMinima1.selectionModel().selectionChanged.connect(
            self.on_listMinima1_selectionChanged
        )
        self.ui.listMinima2.selectionModel().selectionChanged.connect(
            self.on_listMinima2_selectionChanged
        )
        ...
        self.ui.list_TS.selectionModel().selectionChanged.connect(
            self.on_list_TS_selectionChanged
        )
