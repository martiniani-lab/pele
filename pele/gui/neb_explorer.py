...
class NEBRunner(object):
    ...
    def create_neb(self, coords1, coords2):
        ...
        min1 = throwaway_db.addMinimum(0.0, coords1)
        min2 = throwaway_db.addMinimum(1.0, coords2)
        # use the functions in DoubleEndedConnect to set up the NEB in the proper way
        double_ended = system.get_double_ended_connect(
            min1, min2, throwaway_db, fresh_connect=True
        )
        local_connect = double_ended._getLocalConnectObject()

        self.local_connect = local_connect

        return local_connect.create_neb(
            system.get_potential(), coords1, coords2, **local_connect.NEBparams
        )
...
class NEBEnergyWidget(MPLWidget):
    ...
    def update_gui(self, nebrunner):
        ...
        for distances, energies, stepnum in zip(
            nebrunner.distances[-nplots:],
            nebrunner.energies[-nplots:],
            nebrunner.stepnum[-nplots:],
        ):
            acc_dist = [0.0]
            acc = 0.0
            for d in distances:
            ...
            self.axes.plot(
                acc_dist, energies, "o-", label="step %d" % stepnum, **kwargs
            )
...
class NEBExplorer(QtGui.QMainWindow):
    def __init__(self, parent=None, system=None, app=None):
        ...
        self.view_energies = self.new_view(
            "Energies", self.energies, QtCore.Qt.TopDockWidgetArea
        )
        self.view_distances = self.new_view(
            "Distances", NEBDistanceWidget(), QtCore.Qt.TopDockWidgetArea
        )
        self.view_k = self.new_view(
            "k", NEBTimeseries(attrname="k"), QtCore.Qt.BottomDockWidgetArea
        )
        self.view_nimages = self.new_view(
            "nimages", NEBTimeseries(attrname="nimages"), QtCore.Qt.BottomDockWidgetArea
        )
        self.view_rms = self.new_view(
            "rms",
            NEBTimeseries(attrname="rms", yscale="log"),
    ...
    def on_actionTS_triggered(self, checked=None):
        ...
        if not hasattr(self, "local_connect_explorer"):
            self.local_connect_explorer = ConnectExplorerDialog(
                self.system, self.app, parent=self
            )
...
if __name__ == "__main__":
    ...
    system.params.double_ended_connect.local_connect_params.NEBparams.iter_density = 5.0
