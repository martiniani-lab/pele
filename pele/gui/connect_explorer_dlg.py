...
class ConnectExplorerDialog(QDialog):
    ...
    def on_refine_all_ts(self):
        ...
        climbing_images = [
            i
            for i in range(len(self.nebrunner.path))
            if self.nebrunner.neb.neb.isclimbing[i]
        ]
    ...
    def refine_transition_state(self, nebindex=None):
        ...
        def findTS_callback(
            coords=None, energy=None, rms=None, eigenval=None, **kwargs
        ):
        ...
        success, tsret, min1ret, min2ret = _refineTS(
            self.system.get_potential(),
            coords,
            tsSearchParams=tsSearchParams,
            pushoff_params=pushoff_params,
        )

        # process the results of the transition state search
        self.ts_coordspath = np.array(tscoordslist)
        self.ts_labels = [
            "TS path: energy=%g, rms=%g, eigenval=%g" % vals for vals in tsdata
        ]
        ...
        if success:
            ...
            pushoff_labels1 = [
                "Pushoff left: energy=%g, rms=%g" % vals for vals in data1
            ]
            pushoff_labels2 = [
                "Pushoff right: energy=%g, rms=%g" % vals for vals in data2
            ]
            # combine them together with one in reversed order.
            self.pushoff_coordspath = (
                list(reversed(pushoff_coordspath1)) + pushoff_coordspath2
            )
            self.pushoff_labels = (
                list(reversed(pushoff_labels1)) + pushoff_labels2
            )
    ...
    def show_neb_path(self, frame=0):
        self.oglwgt.setCoordsPath(
            self.nebrunner.path, frame=frame, labels=self.neb_labels
        )
        self.oglview = "neb"
...
if __name__ == "__main__":
    ...
    system.params.double_ended_connect.local_connect_params.NEBparams.image_density = (
        3.0
    )
    #    system.params.double_ended_connect.local_connect_params.NEBparams.adaptive_nimages = 5.
    system.params.double_ended_connect.local_connect_params.NEBparams.reinterpolate = (
        400
    )
    system.params.double_ended_connect.local_connect_params.NEBparams.max_images = (
        40
    )
