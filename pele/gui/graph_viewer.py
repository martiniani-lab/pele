...
class GraphViewWidget(QWidget):
    ...
    def make_graph_from(self, minima, cutoff=1):
        ...
        outer_layer = set()
        for m in minima:
            nodesdir = nx.single_source_shortest_path(
                self.full_graph, m, cutoff=cutoff
            )
    ...
    def show_graph(self, fixed=False, show_ids=True):
        ...
        layoutlist = [
            nxy for nxy in list(layout.items()) if nxy[0] in interior_nodes
        ]
        ...
        if self.boundary_nodes:
            ...
            boundary_layout_list = [
                nxy
                for nxy in list(layout.items())
                if nxy[0] in self.boundary_nodes
            ]
