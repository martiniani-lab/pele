...
from pele.gui.ui.dgraph_dlg import (check_thermodynamic_info,
                                    minimum_energy_path)
...
class GraphViewWidget(QWidget):
    ...
    def _color_by_committor(self, min1, min2, T=1.0):
        ...
        if not check_thermodynamic_info(transition_states):
            raise Exception(
                "The thermodynamic information is not yet computed.  Use the main menu"
                " of the gui under 'Actions'"
            )
    ...
    def _on_right_click_minimum(self, minimum):
        ...
        print(
            "you right clicked on minimum with id",
            minimum._id,
            "and energy",
            minimum.energy,
        )
        ...
        if self._selected_minimum is not None:
            ...
            menu.addAction(
                ColorByCommittorAction(minimum, self._selected_minimum, parent=self)
            )
    ...
    def show_graph(self, fixed=False, show_ids=True):
        ...
        layoutlist = [nxy for nxy in list(layout.items()) if nxy[0] in interior_nodes]
        ...
        # plot the nodes
        self._minima_points = ax.scatter(
            xypos[:, 0],
            xypos[:, 1],
            picker=5,
            s=markersize,
        ...
        if self.boundary_nodes:
            ...
            boundary_layout_list = [
                nxy for nxy in list(layout.items()) if nxy[0] in self.boundary_nodes
            ]
            ...
            self._boundary_points = ax.scatter(
                xypos[:, 0],
                xypos[:, 1],
                picker=5,
                s=markersize,
        ...
        if self._mpl_cid is not None:
            ...
            self._mpl_cid = None
        self._mpl_cid = self.fig.canvas.mpl_connect(
            "pick_event", self._on_mpl_pick_event
        )
