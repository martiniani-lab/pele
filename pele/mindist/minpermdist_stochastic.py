...
class MinPermDistCluster(object):
    ...
    def _standard_alignments(self, x1, x2):
        ...
        return StandardClusterAlignment(
            x1, x2, accuracy=self.accuracy, can_invert=self.transform.can_invert()
        )
...
def test(
    X1, X2, lj, atomtypes=None, fname="lj.xyz", minPermDist=MinPermDistCluster()
):  # pragma: no cover
    if not atomtypes:
        atomtypes = ["LA"]
    import copy
