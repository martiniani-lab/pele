...
def makeBLJNeighborListPotFreeze(
    natoms, frozenlist, ntypeA=None, rcut=2.5, boxl=None
):
    """
    ...
    ljAA = LJCut(
        eps=blj.AA.eps, sig=blj.AA.sig, rcut=rcut * blj.AA.sig, boxl=boxl
    )
    ljBB = LJCut(
        eps=blj.BB.eps, sig=blj.BB.sig, rcut=rcut * blj.BB.sig, boxl=boxl
    )
    ljAB = LJCut(
        eps=blj.AB.eps, sig=blj.AB.sig, rcut=rcut * blj.AB.sig, boxl=boxl
    )
    ...
    nlAA_mf = NeighborListSubsetBuild(
        natoms, rcut, mobileA, Blist=frozenA, boxl=boxl
    )
    ...
    nlBB_mf = NeighborListSubsetBuild(
        natoms, rcut, mobileB, Blist=frozenB, boxl=boxl
    )

    nlAB_ff = NeighborListSubsetBuild(
        natoms, rcut, frozenA, Blist=frozenB, boxl=boxl
    )
    nlAB_mm = NeighborListSubsetBuild(
        natoms, rcut, mobileA, Blist=mobileB, boxl=boxl
    )
    nlAB_mf = NeighborListSubsetBuild(
        natoms, rcut, mobileA, Blist=frozenB, boxl=boxl
    )
    nlAB_fm = NeighborListSubsetBuild(
        natoms, rcut, mobileB, Blist=frozenA, boxl=boxl
    )
    ...
    mobile_pot = NeighborListPotentialMulti(
        potlist_mobile, natoms, rcut, boxl=boxl
    )
...
class FrozenPotWrapper(object):  # pragma: no cover (obsolete)
    def __init__(self, potential, reference_coords, frozen_dof):
        ...
        self.coords_converter = FrozenCoordsConverter(
...
def test2():  # pragma: no cover
    ...
    print("starting coords:", reference_coords)
    print(
        "minimized coords:", fpot.coords_converter.get_full_coords(ret.coords)
    )
