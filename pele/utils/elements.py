...
def lookup_element_by_mass(mass):
    mass = float(mass)
    closest_mass = min((value["mass"] for value in list(elements.values())), key = lambda x: abs(x - mass))
    return next((value["symbol"] for value in list(elements.values()) if value["mass"] == closest_mass))
