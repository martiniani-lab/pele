...
from pele.utils.frozen_atoms import (
    FrozenCoordsConverter,
    FrozenPotWrapper,
...
class TestFrozenCoordsConverter(unittest.TestCase):
    def setUp(self):
        ...
        self.converter = FrozenCoordsConverter(
            self.reference_coords, self.frozen_dof
        )

    def test1(self):
        reduced_coords = self.converter.get_reduced_coords(
            self.reference_coords
        )
        self.assertEqual(self.ndof - len(self.frozen_dof), len(reduced_coords))

    def test2(self):
        full_coords = self.converter.get_full_coords(
            self.converter.get_reduced_coords(self.reference_coords)
        )
...
class TestBLJNeighborListFreeze(_base_test.BaseTestCases._TestConfiguration):
    def setUp(self):
        ...
        rcut = 2.5
        boxl = 5.5
        self.pot = makeBLJNeighborListPotFreeze(
