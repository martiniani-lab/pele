...
class TestWritePathsampleDB(unittest.TestCase):
    def test1(self):
        ...
        d1 = np.genfromtxt(os.path.join(current_dir, "ts.data")).reshape(-1, 8)[
            :, :5
        ]
        ...
        self.assertEqual(
            sha1_of_file(os.path.join(current_dir, "points.min")),
            sha1_of_file(pm.name),
        )
        self.assertEqual(
            sha1_of_file(os.path.join(current_dir, "points.ts")),
            sha1_of_file(pts.name),
        )

    def test2(self):
        from pele.utils.tests.test_disconnectivity_graph import (
