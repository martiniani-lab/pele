...
from pele.utils.disconnectivity_graph import (DisconnectivityGraph, Tree,
...
class TestDisconnectivityGraph(unittest.TestCase):
    ...
    def test_color_value(self):
        ...
        dgraph.plot(linewidth=2.0)
        dgraph.label_minima(
            {self.db.minima()[0]: "gmin", self.db.minima()[1]: "2nd lowest"}
        )
        if _show:
            from matplotlib import pyplot as plt
    ...
    def test_Emax(self):
        ...
        m1 = self.db.addMinimum(emax - 5.1, [0.0])
        m2 = self.db.addMinimum(emax - 5.2, [0.0])
        self.db.addTransitionState(emax - 5, [0.0], m1, m2)
        self.db.addMinimum(emax - 5.3, [0.0])  # so this will need to be removed
        dgraph = DisconnectivityGraph(
            self.tsgraph, Emax=emax - 2, subgraph_size=1, order_by_energy=True
        )
        dgraph.calculate()
        dgraph.plot()
    ...
    def test_gmin_not_connected(self):
        ...
        self.db.addMinimum(m0.energy - 10.0, m0.coords)
...
class TestTreeLeastCommonAncestor(unittest.TestCase):
    def test(self):
        ...
        self.assertEqual(
            TreeLeastCommonAncestor([t3_1, t3_2, t3_3]).least_common_ancestor, t1
        )
        self.assertEqual(
        ...
        self.assertItemsEqual(
            lca.get_all_paths_to_common_ancestor(), [t2_1, t3_1, t3_2]
        )
