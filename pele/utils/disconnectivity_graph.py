    ...
    def tree_get_colors(self, tree):
        ...
        try:
            ...
            if tree.is_leaf():
                ...
                colors_list = [
                    self.tree_get_colors(subtree) for subtree in tree.get_branches()
                ]
...
class ColorDGraphByValue(object):
    ...
    def __init__(
        self, tree_graph, minimum_to_value, colormap=None, normalize_values=True
    ):
        self.tree_graph = tree_graph
    ...
    def tree_get_value(self, tree):
        ...
        try:
            ...
            if tree.is_leaf():
                ...
                values = [
                    self.tree_get_value(subtree) for subtree in tree.get_branches()
                ]
...
class DisconnectivityGraph(object):
    ...
    def _make_tree(self, graph, energy_levels):
        ...
        minima = graph.nodes()
        maketree = _MakeTree(
            minima, transition_states, energy_levels, get_energy=self._getEnergy
        )
    ...
    def _order_trees_by_value(self, trees):
        ...
        def get_min_val(tree):
            return min(
                [self.get_value(leaf.data["minimum"]) for leaf in tree.leaf_iterator()]
            )
    ...
    def plot(
        self, show_minima=False, show_trees=False, linewidth=0.5, axes=None, title=None
    ):
