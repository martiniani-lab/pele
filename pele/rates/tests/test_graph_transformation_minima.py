    ...
    if db.number_of_transition_states() < nts:
        ...
        for i in range(nts):
            try:
                ...
                if (
                    not "couldn't find any random minima pair to connect"
                    in str(e)
                ):
                    raise

            connect = system.get_double_ended_connect(
                min1, min2, db, verbosity=0
            )
            connect.connect()

    connect = system.get_double_ended_connect(
        db.minima()[0], db.minima()[-1], db, verbosity=0
    )
    connect.connect()
...
class TestGraphRatesLJ(unittest.TestCase):
    def setUp(self):
        ...
        dbfname = os.path.join(
            current_dir, "lj15.{}.sqlite".format(sys.version_info.major)
        )
        print(dbfname)
...
class TestOptimCollagen(unittest.TestCase):
    ...
    def setUp(self):
        ...
        converter = OptimDBConverter(
            self.db,
            ndof=ndof,
            mindata=current_dir + "/collagen.min.data",
            tsdata=current_dir + "/collagen.ts.data",
            assert_coords=False,
        )
    ...
    def test1(self):
        ...
        rcalc = RateCalculation(
            self.db.transition_states(), [m1], [m2], T=0.592
        )
        rcalc.compute_rates()
        self.assertAlmostEqual(rcalc.get_rate_AB(), 7106337458.0, delta=1e4)
        self.assertAlmostEqual(rcalc.get_rate_BA(), 1955395816.0, delta=1e4)

        rcalc = RateCalculation(
            self.db.transition_states(), [m1, m3], [m2, m4], T=0.592
        )
        rcalc.compute_rates()
        self.assertAlmostEqual(rcalc.get_rate_AB(), 8638736600.0, delta=1e4)
        self.assertAlmostEqual(rcalc.get_rate_BA(), 3499625167.0, delta=1e4)

        rla = RatesLinalg(self.db.transition_states(), [m1], [m2], T=0.592)
        rAB = rla.compute_rates()
        self.assertAlmostEqual(rAB, 7106337458.0, delta=1e4)

        rla = RatesLinalg(
            self.db.transition_states(), [m1, m3], [m2, m4], T=0.592
        )
        rAB = rla.compute_rates()
        self.assertAlmostEqual(rAB, 8638736600.0, delta=1e4)
