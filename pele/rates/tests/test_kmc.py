...
class TestKMC(unittest.TestCase):
    ...
    def test_mfp(self):
        ...
        self.assertAlmostEqual(time, 1.0, delta=0.1)


class TestKMC_GraphReduction(unittest.TestCase):
    def compare(self, A, B, nnodes=10, nedges=20, weights=None, x=1):
        print("")
        maker = _MakeRandomGraph(
            nnodes=nnodes, nedges=nedges, node_set=A + B + [x]
        )
        graph = maker.run()
        ...
        self.assertAlmostEqual(PxB, PxB_kmc, delta=0.1)
        ...
        print(
            "graph reduction committor prob",
            reducer.get_committor_probabilityAB(A[0]),
        )
        self.assertAlmostEqual(
            paB, reducer.get_committor_probabilityAB(A[0]), delta=0.1
        )
    ...
    def test_committor_probabilities(self, nnodes=10, nedges=20):
        ...
        for x in nodes:
            PxB_kmc = kmc.committor_probability(x, A, B, niter=1000)
            self.assertAlmostEqual(PxB[x], PxB_kmc, delta=0.1)
