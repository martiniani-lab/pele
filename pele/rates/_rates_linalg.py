...
def reduce_rates(rates, B, A=None):
    ...
    if len(connected_nodes) != len(all_nodes):
        print(
            "removing",
            len(all_nodes) - len(connected_nodes),
...
class CommittorLinalg(object):
    ...
    def compute_committors(self):
        ...
        if self.right_side.size == 1:
            ...
            committors = scipy.sparse.linalg.spsolve(
                self.matrix, self.right_side
            )
...
class MfptLinalgSparse(object):
    ...
    def _make_subgroups(self):
        ...
        graph.add_edges_from(
            [
                uv
                for uv in iter(self.rates.keys())
                if uv[0] not in self.B and uv[1] not in self.B
            ]
        )
        ...
        if len(self.subgroups) <= 10:
            ...
            print(
                "subgroup sizes", [len(c) for c in self.subgroups[:10]], "..."
            )
    ...
    def compute_mfpt(self, use_umfpack=True, cg=False, mfpt_estimate=None):
        ...
        if cg:
            ...
            times, info = scipy.sparse.linalg.cgs(
                self.matrix, -np.ones(self.matrix.shape[0]), x0=x0
            )
            print(
                "time to solve using conjugate gradient",
                time.process_time() - t0,
            )
        else:
            times = scipy.sparse.linalg.spsolve(
                self.matrix,
...
class TwoStateRates(object):
    ...
    def compute_rates(
        ...
        if mfpt_estimates is not None and not cg:
            self.mfptimes = self.mfpt_computer.compute_mfpt_from_estimate(
                mfpt_estimates
            )
        elif symmetric:
            ...
            self.mfptimes = self.mfpt_computer.compute_mfpt_subgroups(
                use_umfpack=use_umfpack
            )
        else:
            self.mfptimes = self.mfpt_computer.compute_mfpt(
                use_umfpack=use_umfpack, cg=cg, mfpt_estimate=mfpt_estimates
            )

    def compute_committors(self):
        """compute the committors"""
        self.committor_computer = CommittorLinalg(
            self.rate_constants, self.A, self.B
        )
