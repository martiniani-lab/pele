...
from pele.rates._rates_linalg import LinalgError, TwoStateRates, reduce_rates
...
def log_equilibrium_occupation_probability(minimum, T):
    ...
    return old_div(-minimum.energy, T) - np.log(minimum.pgorder) - 0.5 * minimum.fvib
...
class _Minima2Rates(object):
    ...
    def _get_local_log_rate(self, min1, min2, ts):
        ...
        """
        if self.use_fvib:
            sigma = float(min1.pgorder) / (2.0 * np.pi * ts.pgorder)
            return (
                np.log(sigma)
                + (min1.fvib - ts.fvib) / 2.0
    ...
    def _compute_rate_constants(self):
        ...
        for ts in self.transition_states:
            if not self._transition_state_ok(ts):
                print(
                    "excluding invalid transition state from rate graph",
                    ts.energy,
                    ts.id(),
                )
                continue
        ...
        if nts_skip_same > 0:
            print(
                "warning: not using",
                nts_skip_same,
                "transition states because they connect a minimum with itself",
            )
        ...
        if True:
            ...
            print(
                "time scale need to be multiplied by", 1.0 / np.exp(self.max_log_rate)
            )
            rates = dict(
                (
    ...
    def _log_equilibrium_occupation_probability(self, minimum):
        ...
        # warning, this has not been checked, there might be a bug
        return (
            -self.beta * minimum.energy - np.log(minimum.pgorder) - 0.5 * minimum.fvib
        )
...
class RateCalculation(object):
    ...
    def __init__(self, transition_states, A, B, T=1.0, use_fvib=True):
        self.minima2rates = _Minima2Rates(
            transition_states, A, B, T=T, use_fvib=use_fvib
        )
    ...
    def get_rate_AB_SS(self):
        return old_div(self.reducer.get_rate_AB_SS(), self.minima2rates.rate_norm)

    def get_rate_BA_SS(self):
        return old_div(self.reducer.get_rate_BA_SS(), self.minima2rates.rate_norm)
...
class RatesLinalg(object):
    ...
    def compute_rates(self):
        ...
        return old_div(self.two_state_rates.get_rate_AB(), self.minima2rates.rate_norm)
...
def compute_committors(transition_states, A, B, T=1.0):
