...
class GraphReduction(object):
    ...
    def get_committor_probabilityAB(self, x):
        ...
        try:
            ...
            if x not in self.A and x not in self.B:
                raise ValueError(
                    "x is not in A or in B.  Use compute_committor_probability() if x"
                    " is an intermediate"
                )
    ...
    def _phase_two_group(self, full_graph, group):
        ...
        for a in self._reduce_all_iterator(group):
            ...
            if Paa > 0.999:
                print(
                    "warning, Pxx is very large (%s), numerical precision problems"
                    " might be in your future" % Paa
                )
    ...
    def _remove_node(self, x):
        ...
        if Pxx > 0.999:
            print(
                "warning, Pxx is very large (%s), numerical precision problems might be"
                " in your future" % Pxx
            )
