...
class GraphReduction(object):
    ...
    def get_committor_probabilityAB(self, x):
        ...
        if len(self._final_Pxx) == 0:
            raise RuntimeError(
                "you must call compute_rates before calling this function"
            )
        try:
            return 1.0 - self._final_Pxx[x]
        except KeyError:
            if x not in self.A and x not in self.B:
                raise ValueError(
                    "x is not in A or in B.  Use compute_committor_probability() if x is an intermediate"
                )
    ...
    def get_rate_AB_SS(self):
        ...
        for a in self.A:
            PaB = sum(
                (
                    data["P"]
                    for x, b, data in self.graph.out_edges(a, data=True)
                    if b in self.B
                )
            )
    ...
    def _phase_two_group(self, full_graph, group):
        ...
        for a in self._reduce_all_iterator(group):
            ...
            if Paa > 0.999:
                print(
                    "warning, Pxx is very large (%s), numerical precision problems might be in your future"
                    % Paa
                )
    ...
    def _add_edge(self, u, v):
        ...
        if self.debug:
            print("  creating edge", u, v)
        self.graph.add_edge(u, v, P=0.0)
    ...
    def _check_A_B_connected(self, connected_components):
        ...
        if not AB_connected:
            raise ValueError(
                "product and reactant sets (A and B) are not connected"
            )
        ...
        if unconnected_nodes:
            print(
                ...
                "nodes from the graph because they're not connected to A or to B",
            )
            self.graph.remove_nodes_from(unconnected_nodes)

        if len(remaining_components) > 1:
            print(
                "warning, graph is not fully connected.  There are",
    ...
    def initial_check_graph(self):
        for a in self.A:
            if not self.graph.has_node(a):
                raise ValueError(
                    "an element in the reactant set (A) is not in the graph"
                )
        for b in self.B:
            if not self.graph.has_node(b):
                raise ValueError(
                    "an element in the product set (B) is not in the graph"
                )
    ...
    def _get_committor_probability(self, x):
        PxA = sum(
            [
                data["P"]
                for (u, v, data) in self.graph.out_edges([x], data=True)
                if v in self.A
            ]
        )
        PxB = sum(
    ...
    def compute_committor_probabilities(self, nodes):
        ...
        if len(not_in_graph) > 0:
            raise ValueError(
                "At least on of the nodes is not in the graph."
        ...
        assert (
            len(set(self.graph.nodes()).difference(self.A).difference(self.B))
            == 0
        )
