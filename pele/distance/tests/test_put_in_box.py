...
class TestPutAtomInBox(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.rs = np.random.uniform(
            -boxlength, boxlength, (self.test_repeat, 3)
        )
    ...
    def test_leesedwards(self):
        ...
        for dim in [2, 3]:
            origin = np.zeros(dim)
            for r in self.rs:
                r_boxed = put_atom_in_box(
                    r, dim, dist_method, self.boxv[:dim], shear
                )
                for i in range(dim):
    ...
    def test_periodic(self):
        ...
        for dim in [2, 3]:
            ...
            rs_boxed_linear = put_in_box(
                rs_linear, dim, dist_method, self.boxv[:dim]
            )
