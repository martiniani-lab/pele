    ...
    ***orthogZeroEigs is system dependent, don't forget to set it***
    ...
    def __init__(
        ...
        if "maxstep" in self.tangent_space_quench_params:
            ...
            self.maxstep_tangent = (
                0.1  # this should be determined in a better way
            )
    ...
    def params(cls, obj=None):
        ...
        ] = obj.tangent_space_quench_params.copy()
        params[
        ...
        if "nsteps" in self.lowestEigenvectorQuenchParams:
            ...
            if self.verbosity > 3:
                print(
                    "Using default of",
                    niter,
                    "steps for finding lowest eigenvalue",
                )
    ...
    def _get_lowest_eigenvector_diagonalization(self, coords, **kwargs):
        ...
        if self.verbosity > 3:
            print(
                "computing the lowest eigenvector by diagonalizing the Hessian"
            )
    ...
    def _minimizeTangentSpace(self, coords, energy=None, gradient=None):
        ...
        if self._transverse_walker is None:
            ...
            self._transverse_walker.update_eigenvec(
                self.eigenvec, self.eigenval
            )
    ...
    def _stepUphill(self, coords):
        ...
        if self.eigenval > 0 and self.verbosity >= 2:
            logger.warn(
                "eigenvalue is positive, but stepping uphill along the lowest curvature mode anyway"
            )
