...
def InterpolatedPathDensity(initial, final, distance, density=10.0, **kwargs):
    """
...
class InterpolatedPath(object):
    ...
    def __getitem__(self, index):
        ...
        return self.interpolator(
            self.initial, self.final, float(index) / float(self.nimages - 1)
        )
...
if __name__ == "__main__":
    path = InterpolatedPath(0.0, 1.0, 10)
    print(len(path))
    print(path[5])
