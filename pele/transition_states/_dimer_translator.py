...
class _DimerPotential(object):
    ...
    def __init__(self, potential, eigenvec0, leig_kwargs=None):
    ...
    def projected_energy_gradient(self, energy, gradient):
        ...
        """
        projgrad = gradient - 2.0 * np.dot(gradient, self.eigenvec) * self.eigenvec
    ...
    def get_true_energy_gradient(self, coords):
        ...
        if (coords == self._true_coords).all():
            ...
            print(
                "warning: get_true_gradient should only be used to access precomputed"
