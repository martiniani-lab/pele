...
class _HybridEigenvectorWalker(object):
    ...
    def __init__(
        ...
        **transverse_kwargs
        ...
        self.tol = tol
        if self.transverse_kwargs is None:
            self.transverse_kwargs = dict()
        if "tol" not in self.transverse_kwargs:
            self.transverse_kwargs["tol"] = self.tol / 2.0
    ...
    def _update_coords(self, coords, transverse_energy=None, transverse_gradient=None):
        self.coords = coords
        if transverse_energy is None or transverse_gradient is None:
            (
                self._transverse_energy,
                self._transverse_gradient,
            ) = self.transverse_potential.getEnergyGradient(coords)
        else:
    ...
    def _step_uphill(self, coords, gradient, eigenvec, eigenval):
        ...
        stepsize = (
            ...
            * F
            ...
            / (1.0 + np.sqrt(1.0 + 4.0 * F**2 / eigenval**2))
        )
        ...
        if self.debug:
            ne, ng = self.transverse_potential.pot.getEnergyGradient(
                coords + stepsize * eigenvec
            )
            print(
                "uphill step",
                stepsize,
                ...
                ne - self.energy,
                "gradpar old new",
                np.dot(self.gradient, self.get_eigenvector()),
                np.dot(ng, self.get_eigenvector()),
            )

        return coords + stepsize * eigenvec

    def _minimize_transverse(
        self, nsteps, transverse_energy=None, transverse_gradient=None
    ):
        # must update
        minimizer = MYLBFGS(
            self.coords,
    ...
    def _print_status(self):
        rmsnorm = 1.0 / np.sqrt(self.coords.size)
    ...
    def one_iteration(self):
        coords = self._step_uphill(
            self.coords, self.gradient, self.get_eigenvector(), self.eigenval
        )
        self._update_coords(coords)
        ret = self._minimize_transverse(
            self.nsteps_tangent,
            transverse_energy=self._transverse_energy,
...
def test():  # pragma: no cover
    ...
    from pele.transition_states import FindTransitionState, GeneralizedDimer
    ...
    searcher = FindTransitionState(
        x.copy(), system.get_potential(), iprint=1, verbosity=10
    )
    # print ret
    ret = searcher.run()
    print(ret)
