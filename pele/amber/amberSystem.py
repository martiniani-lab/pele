...
class AMBERSystem(BaseSystem):
    def __init__(self, prmtopFname, inpcrdFname):
        ...
        if self.sanitycheck:
            ...
            self.params.basinhopping.confCheck = [
                self.check_CAchirality_wrapper
            ]
    ...
    def set_params(self, params):
        ...
        tsSearchParams = (
            params.double_ended_connect.local_connect_params.tsSearchParams
        )
    ...
    def get_permlist(self):
        ...
        if os.path.exists("coordsModTerm.pdb"):
            ...
            print(
                "amberSystem> coordsModTerm.pdb not found. permlist could not be created."
            )
            return []
    ...
    def get_mindist(self):
        ...
        return MinPermDistAtomicCluster(
            permlist=permlist, niter=10, can_invert=False
        )
    ...
    def get_optim_spawner(self, coords1, coords2):
        ...
        return AmberSpawnOPTIM(
            coords1, coords2, self, OPTIM=optim, tempdir=False
        )
    ...
    def check_cistrans(self, coords):
        ...
        for i in self.peptideBondAtoms:
            ...
            if deg < 90 or deg > 270:
                isTrans = False
                print(
                    "CIS peptide bond between atoms ",
                    i,
                    " torsion (deg) = ",
                    deg,
                )
    ...
    def test_BH(self, db, nsteps):
        ...
        from pele.takestep import (
            RandomDisplacement,
            AdaptiveStepsizeTemperature,
        )

        takeStepRnd = RandomDisplacement(stepsize=2)
        tsAdaptive = AdaptiveStepsizeTemperature(
            takeStepRnd, interval=10, verbose=True
        )
