...
# pele
from pele.systems import BaseParameters, BaseSystem
...
class AMBERSystem(BaseSystem):
    def __init__(self, prmtopFname, inpcrdFname):
        ...
        self.params.takestep_random_displacement.stepsize = 2.0
        ...
        if self.sanitycheck:
            ...
            self.params.double_ended_connect.conf_checks = [
                self.check_cistrans_wrapper_kwargs,
    ...
    def parse_prmtop(self):
        ...
        self.bonds = [
            (a1.index, a2.index) for a1, a2 in self.prmtop_parsed.atoms.edges_iter()
        ]
    ...
    def get_ndof(self):
        return 3.0 * len(self.atom_names)
    ...
    def set_params(self, params):
        ...
        if False:  # use fire
            ...
            NEBparams.NEBquenchParams.maxstep = 0.1
    ...
    def get_potential(self):
        ...
        if os.path.exists("min.in") and os.path.exists("data"):
            ...
            try:
                ...
                self.potential = gmin_potential.GMINAmberPotential(
                    self.prmtopFname, self.inpcrdFname
                )
        ...
        try:
            ...
            self.potential = openmm_potential.OpenMMAmberPotential(
                self.prmtopFname, self.inpcrdFname
            )
        ...
        if self.potenial == None:
            print(
                "\namberSystem> potential not set. Could not import GMIN or OpenMM"
                " potential."
            )
    ...
    def get_permlist(self):
        ...
        if os.path.exists("coordsModTerm.pdb"):
            ...
            print(
                "amberSystem> coordsModTerm.pdb not found. permlist could not be"
                " created."
            )
            return []
    ...
    def drawCylinder(self, X1, X2):
        ...
        g = GLU.gluNewQuadric()
        GLU.gluCylinder(g, 0.1, 0.1, r, 30, 30)  # I can't seem to draw a cylinder
        GL.glPopMatrix()
    ...
    def draw(self, coordsl, index):
        ...
        for i, name in enumerate(
            self.atom_names
        ):  # in self.potential.prmtop.topology.atoms():
            x = coords[i, :] - com
    ...
    def load_coords_pymol(self, coordslist, oname, index=1):
        ...
        for coords in coordslist:
            ...
            openmmpdb.PDBFile.writeModel(
                self.potential.prmtop.topology,
                self.potential.localCoords * openmm_angstrom,
                file=f,
                modelIndex=ct,
            )
    ...
    def populate_peptideAtomList(self):
        listofC = [
            i.index for i in self.potential.prmtop.topology.atoms() if i.name == "C"
        ]
        listofO = [
            i.index for i in self.potential.prmtop.topology.atoms() if i.name == "O"
        ]
        listofN = [
            i.index for i in self.potential.prmtop.topology.atoms() if i.name == "N"
        ]
        listofH = [
            i.index for i in self.potential.prmtop.topology.atoms() if i.name == "H"
        ]
        ...
        for i in listofC:
            if (
                listofO.__contains__(i + 1)
                and listofN.__contains__(i + 2)
                and listofH.__contains__(i + 3)
            ):
    ...
    def populate_CAneighborList(self):
        listofCA = [
            i.index for i in self.potential.prmtop.topology.atoms() if i.name == "CA"
        ]
        listofC = [
            i.index for i in self.potential.prmtop.topology.atoms() if i.name == "C"
        ]
        ...
        listofCB = [
            i.index for i in self.potential.prmtop.topology.atoms() if i.name == "CB"
        ]
    ...
    def check_cistrans(self, coords):
        ...
        for i in self.peptideBondAtoms:
            atNum = i[0]
            rC = np.array(
                [coords[3 * atNum], coords[3 * atNum + 1], coords[3 * atNum + 2]]
            )
            atNum = i[1]
            rO = np.array(
                [coords[3 * atNum], coords[3 * atNum + 1], coords[3 * atNum + 2]]
    ...
    def test_BH(self, db, nsteps):
        ...
        from pele.takestep import (AdaptiveStepsizeTemperature,
                                   RandomDisplacement)
...
if __name__ == "__main__":
    ...
    sysAmb = AMBERSystem(
        "../../examples/amber/aladipep/coords.prmtop",
