...
class Molecule(object):
    ...
    def read_coords(self, inpcrd_filename):
        ...
        if len(coords) != self.num_atoms():
            raise ex.RuntimeError(
                "Incorrect number of coordinates read into molecule."
            )
        # Now run through the list of atoms (sorted by index) and coordinates
        for atom, xyz in zip(
            sorted(self.atoms.nodes(), key=op.attrgetter("index")), coords
        ):
            atom.coords = xyz
    ...
    for input_line in prmtop_input_lines:
        ...
        data = list(
            map(
                data_cast,
                split_string,
                itertools.repeat(data_type, len(split_string)),
            )
        )
...
def create_molecule(topology_data):
    ...
    bond_list = list(zip(first_atoms, second_atoms))
    ...
    for bond in bond_list:
        ...
        if bonded_atoms[0].residue != bonded_atoms[1].residue:
            molecule.residues.add_edge(
                bonded_atoms[0].residue, bonded_atoms[1].residue
            )
    return molecule
    ...
    for param in list(params.keys()):
        for residue in molecule.residues.nodes():
            if residue.name == param[0]:
                ...
                groups[group_name]["group_atoms"] = [
                    atom.index for atom in rotated_atoms[2]
                ]
...
def read_amber_coords(filename):
        ...
        for line in coords_file:
            ...
            coords += list(
                map(
                    float,
                    [
                        line[i : i + field_length]
                        for i in range(0, len(line), field_length)
                    ],
                )
            )
    ...
    if len(coords) != number_of_atoms * 3:
        raise ex.RuntimeError(
            "Number of coordinates in coords file and number of atoms are inconsistent."
        )
    return coords
