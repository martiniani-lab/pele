...
class TestDistanceGraph(unittest.TestCase):
    def setUp(self):
        ...
        self.db = db
        self.natoms = natoms
        self.connect = DoubleEndedConnect(
            min1,
            min2,
            pot,
            mindist,
    ...
    def test_merge_minima(self):
        ...
        print(
            ...
            min4.id(),
            "are connected",
            self.connect.graph.areConnected(min3, min4),
        )
        print(
            min3.id(), "number of edges", self.connect.graph.graph.degree(min3)
        )
        print(
            min4.id(), "number of edges", self.connect.graph.graph.degree(min4)
        )
    ...
    def test_add_TS_existing_minima(self):
        ...
        allok = self.connect.dist_graph.checkGraph()
        self.assertTrue(
            allok, "adding a transition state broke the distance graph"
        )
    ...
    def test_add_TS_multiple(self):
        ...
        allok = self.connect.dist_graph.checkGraph()
        self.assertTrue(
            allok, "adding multiple transition states broke the distance graph"
        )
