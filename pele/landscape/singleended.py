...
def find_escape_paths(
    minimum, potential, graph, ntries=1, push=1.0e-2, push_minrms=1.0e-2
):
    raise Exception(
        "js850> this function doesn't work anymore since changing graph.addMinimum and"
        " addTransitionState.  It needs to be overhauled"
    )
    ...
    for i in range(ntries):
        x_ts, energy_ts, rms, tmp = _uphill_search(
            minimum.coords, search, push, push_minrms
        )
        ...
        if not min1 is minimum and not min2 is minimum:
            print(
                "Warning in single ended search: did not find initial minimum during"
                " quench from transition state"
            )

        if min1 is min2:
            print(
                "Warning in single ended search: downhill search from transistion state"
                " ended in same minimum"
            )
