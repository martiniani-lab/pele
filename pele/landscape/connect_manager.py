...
class ConnectManagerRandom(BaseConnectManager):
    ...
    def get_connect_job(self):
        ...
        itermax = (
            self.database.number_of_minima()
        )  # so we don't iterate forever
    ...
    def get_connect_job(self, strategy=None):
        ...
        if strategy == "gmin":
            ...
            if min1 is None or min2 is None:
                if self.verbosity > 0:
                    print(
                        "couldn't find any minima not connected to the global minimum.  Doing",
                        self.backup_strategy,
                        "strategy instead",
                    )
                strategy = self.backup_strategy
                ...
                if self.verbosity > 0:
                    print(
                        "sending a connect job to connect all minima with the global minimum",
                        min1.id(),
                        min2.id(),
                    )
