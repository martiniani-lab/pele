...
from pele.landscape import LocalConnect, TSGraph
...
class DoubleEndedConnect(object):
    ...
    def __init__(
        ...
        if local_connect_params is None:
            local_connect_params = dict()
        self.local_connect_params = dict(
            [("verbosity", verbosity)] + list(local_connect_params.items())
        )
        self.database = database
        self.fresh_connect = fresh_connect
        if self.fresh_connect:
            self.graph = TSGraph(
                self.database, minima=[self.minstart, self.minend], no_edges=True
            )
        else:
        ...
        self.dist_graph = _DistanceGraph(
            self.database, self.graph, self.mindist, self.verbosity
        )
        ...
        if self.verbosity > 0:
            ...
            logger.info(
                "        minimum 1: id %d energy %f"
                % (self.minstart.id(), self.minstart.energy)
            )
            logger.info(
                "        minimum 2: id %d energy %f"
    ...
    def mergeMinima(self, min1, min2):
        ...
        logger.info(
            "merging minima %s %s %s %s %s",
            min1.id(),
            min2.id(),
            dist,
            "E1-E2",
            min1.energy - min2.energy,
        )
        ...
        if debug:
            ...
            logger.debug(
                "self.graph.graph. nnodes %s", self.graph.graph.number_of_nodes()
            )
    ...
    def _addTransitionState(self, ts_ret, min_ret1, min_ret2):
        ...
        if ts_ret.energy < me1 or ts_ret.energy < me2:
            logger.warning(
                "trying to add a transition state that has energy lower than its"
                " minima."
            )
            logger.warning(
                "    TS energy %s %s %s %s", ts_ret.energy, "minima energy", me1, me2
            )
        ...
        if min1 == min2:
            logger.warning(
                "stepping off the transition state resulted in twice the same"
                " minima %s",
                min1.id(),
            )
            return False
        ...
        ts = self.database.addTransitionState(
            ts_ret.energy, ts_ret.coords, min1, min2, eigenval=ts_ret.eigenval
        )
        ...
        if self.verbosity > 1:
            ...
            if min1 != self.minstart and min1 != self.minend:
                ...
                logger.info(
                    "    distances: %4d -> %4d = %f    %4d -> %4d = %f    %4d -> %4d ="
                    " %f  %s"
            ...
            if min2 != self.minstart and min2 != self.minend:
                ...
                logger.info(
                    "    distances: %4d -> %4d = %f    %4d -> %4d = %f    %4d -> %4d"
                    " = %f" % (msid, m2id, ds, m2id, meid, de, m2id, m2id, dse)
                )
    ...
    def _localConnect(self, min1, min2):
        ...
        if (min1, min2) in self.pairsNEB:
            logger.warning(
                "WARNING: redoing NEB for minima %s %s", min1.id(), min2.id()
            )
        ...
        if self.graph.areConnected(min1, min2):
            logger.warning(
                "in _local_connect, but minima are already connected. aborting %s"
                " %s %s",
                min1.id(),
                min2.id(),
        ...
        if nsuccess == 0:
            ...
            if dist < self.max_dist_merge:
                logger.warning(
                    "local connect failed and the minima are close. Are the minima"
                    " really the same?"
                )
                logger.warning(
                    "         energies: %s %s %s %s",
    ...
    def _getNextPair(self):
        ...
        if True:
            logger.debug(
                "Gdist has %s %s %s %s",
                self.dist_graph.Gdist.number_of_nodes(),
                "nodes and",
                self.dist_graph.Gdist.number_of_edges(),
                "edges",
            )
    ...
    def connect(self):
        ...
        logger.info(
            "failed to find connection between %s %s",
            self.minstart.id(),
            self.minend.id(),
        )
