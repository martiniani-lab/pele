...
class DoubleEndedConnect(object):
    ...
    def __init__(
        ...
        assert (
            min1.id() == min1
        ), "minima must compare equal with their id %d %s %s" % (
            min1.id(),
            str(min1),
            str(min1.__hash__()),
        )
        ...
        if self.verbosity > 0:
            ...
            logger.info(
                "        dist %f" % self.getDist(self.minstart, self.minend)
            )
            logger.info(
                "************************************************************"
            )
    ...
    def mergeMinima(self, min1, min2):
        ...
        if dist > self.max_dist_merge:
            logger.info(
                "    minima merge aborted.  distance is too large %s", dist
            )
            return
        ...
        if True:
            ...
            for w, min1, min2 in weightlist:
                if w > 1e-6:
                    ...
                    dist = w
                logger.info(
                    "    path guess %s %s %s", min1.id(), min2.id(), dist
                )
