...
from pele.angleaxis import RBPotentialWrapper, RBTopology, RigidFragment
from pele.angleaxis._aa_utils import (_rot_mat_derivative, _sitedist,
                                      _sitedist_grad)
from pele.angleaxis._otp_cluster import OTPCluster, make_otp
...
class TestCppRBPotentialWrapper(TestOTPExplicit):
    def test_pot_wrapper(self):
        ...
        self.assertAlmostEqual(
            rbpot_cpp.getEnergy(self.x0), rbpot.getEnergy(self.x0), 4
        )
...
class TestOTPCluster(unittest.TestCase):
    ...
    def test1(self):
        pot = self.system.get_potential()
        self.assertLess(np.linalg.norm(pot.getGradient(self.m1.coords)), 0.1)
        self.assertLess(np.linalg.norm(pot.getGradient(self.m2.coords)), 0.1)
    ...
    def test_thermodynamics(self):
        get_thermodynamic_information(
            self.system, self.db, nproc=None, recalculate=True
        )
