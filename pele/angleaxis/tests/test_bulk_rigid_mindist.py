...
class TestExactMatchPeriodicRigid(unittest.TestCase):
        ...
        for i in range(self.nrigid):
            for j in range(3):
                x[i][j] = np.random.uniform(
                    -self.boxl[j] / 2.0, self.boxl[j] / 2.0
                )
    ...
    def test_align_improvement(self, verbose=True):
        ...
        for i in range(max_step):
            ...
            if dist2 > dist:
                ...
                try:
                    ...
                    pym.draw_rigid(
                        x1, "A", 0.1, (1, 0, 0), self.system.draw_bonds
                    )
                    pym.draw_rigid(
                        self.x2diff, "B", 0.1, (0, 1, 0), self.system.draw_bonds
                    )
                    pym.draw_rigid(
                        x2, "C", 0.1, (0, 0, 1), self.system.draw_bonds
                    )
        ...
        self.assertFalse(
            fail_counter, "alignment failed %d times" % fail_counter
        )
