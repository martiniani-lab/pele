...
from pele.angleaxis import (MeasureAngleAxisCluster, RBTopology,
...
from pele.utils.rotations import aa2mx, random_aa
    ...
    def test_distance(self):
        for i in range(100):
            ...
            self.assertAlmostEqual(
                measure1.get_dist(coords1, coords2), measure2.get_dist(coords1, coords2)
            )
...
class TestAAMeasure(unittest.TestCase):
    ...
    def test_cpp_align(self):
        ...
        assert_arrays_almost_equal(
            self, x2_python[: 3 * self.nrigid], x2[: 3 * self.nrigid]
        )
        assert_arrays_almost_equal(
            self, x2_cpp[: 3 * self.nrigid], x2[: 3 * self.nrigid]
        )
