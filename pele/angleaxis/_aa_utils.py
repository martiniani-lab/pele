...
def _sitedist(drij, p1, p2, S, W, cog):
    ...
    DR1 = np.dot(
        dR, np.dot(S, np.transpose(dR))
    )  # we only need the trace, so this can be sped up
    d_P = np.trace(DR1)
    d_mix = 2.0 * W * np.dot(drij, np.dot(dR, cog))
...
def _sitedist_grad(drij, p1, p2, S, W, cog):
    ...
    g_P = np.zeros(3)
    g_P[0] = -2.0 * np.trace(np.dot(R11, np.dot(S, np.transpose(dR))))
    g_P[1] = -2.0 * np.trace(np.dot(R12, np.dot(S, np.transpose(dR))))
    g_P[2] = -2.0 * np.trace(np.dot(R13, np.dot(S, np.transpose(dR))))

    g_M -= 2.0 * W * np.dot(dR, cog)
    g_P[0] -= 2.0 * W * np.dot(drij, np.dot(R11, cog))
    g_P[1] -= 2.0 * W * np.dot(drij, np.dot(R12, cog))
    g_P[2] -= 2.0 * W * np.dot(drij, np.dot(R13, cog))
