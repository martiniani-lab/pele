...
from pele.angleaxis import (ExactMatchAACluster, MinPermDistAACluster,
                            TakestepAA)
...
class AASystem(BaseSystem):
    def __init__(self):
        ...
        self.params.basinhopping["temperature"] = 8.0
        ...
        tsSearchParams = (
            self.params.double_ended_connect.local_connect_params.tsSearchParams
        )
    ...
    def get_random_configuration(self):
        coords = 5.0 * np.random.uniform(-1, 1, 6 * self.aatopology.get_nrigid())
    ...
    def smooth_path(self, path, **kwargs):
        ...
        return smooth_path(
            path, mindist, interpolator=self.aasystem.interpolate, **kwargs
        )
...
class RBSystem(AASystem):
    def draw(self, rbcoords, index, shift_com=True):  # pragma: no cover
        from pele.systems._opengl_tools import draw_cylinder, draw_sphere
    ...
    def load_coords_pymol(self, coordslist, oname, index=1):  # pragma: no cover
        ...
        if hasattr(self, "draw_bonds"):
            ...
            for i1, i2 in self.draw_bonds:
                pymol.cmd.bond(
                    "id %d and %s" % (i1 + 1, oname), "id %d and %s" % (i2 + 1, oname)
                )
