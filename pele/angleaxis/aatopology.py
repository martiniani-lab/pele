...
def interpolate_angleaxis(initial, final, t):
    ...
    for i in range(conf.shape[0]):
        conf[i] = rotations.q2aa(
            rotations.q_slerp(
                rotations.aa2q(initial[i]), rotations.aa2q(final[i]), t
            )
        )
    return conf
...
class AATopology(object):
    ...
    def interpolate(self, initial, final, t):
        ...
        cnew.posRigid[:] = interpolate_linear(
            cinitial.posRigid, cfinal.posRigid, t
        )
        cnew.rotRigid[:] = interpolate_angleaxis(
            cinitial.rotRigid, cfinal.rotRigid, t
        )
        return cnew.coords
...
class AATopologyBulk(AATopology):
    ...
    def distance_squared(self, coords1, coords2):
        ...
        if self.cpp_topology is not None:
            ...
            print(
                "Warning: used Python version of AATopologyBulk.distance_squared"
            )
            return self._distance_squared_python(coords1, coords2)
