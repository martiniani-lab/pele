...
class LJClusterFrozen(LJCluster):
    def __init__(self, natoms, frozen_atoms, reference_coords):
        ...
        if self.nfrozen <= 2:
            print(
                "warning: Dealing properly with the rotational degrees of freedom and reflection symmetry in clusters with 1 or 2 frozen atoms is not implemented yet"
            )
        if self.nfrozen == 3:
            print(
                "warning: with three frozen atoms there is still reflection symmetry through the plane of the atoms.  This will not be accounted for."
            )

    def get_potential(self):
        pot = LJCluster.get_potential(self)
        frozen_pot = FrozenPotentialWrapper(
            pot, self.reference_coords, self.frozen_dof
        )
        return frozen_pot
