...
class LJClusterFrozen(LJCluster):
    def __init__(self, natoms, frozen_atoms, reference_coords):
        ...
        self.frozen_dof = np.array(
            [list(range(3 * i, 3 * i + 3)) for i in self.frozen_atoms]
        ).flatten()
        ...
        self.mobile_atoms = np.array(
            [i for i in range(self.natoms) if i not in self.frozen_atoms], np.integer
        )
        ...
        if self.nfrozen <= 2:
            print(
                "warning: Dealing properly with the rotational degrees of freedom and"
                " reflection symmetry in clusters with 1 or 2 frozen atoms is not"
                " implemented yet"
            )
        if self.nfrozen == 3:
            print(
                "warning: with three frozen atoms there is still reflection symmetry"
                " through the plane of the atoms.  This will not be accounted for."
            )
    ...
    def get_mindist(self):
        if self.get_nzero_modes() != 0:
            raise RuntimeError(
                "Dealing properly with the rotational degrees of freedom in clusters"
                " with 1 or 2 frozen atoms is not implemented yet"
            )
    ...
    def get_random_configuration(self):
        ...
        boxl = 0.7 * float(self.nmobile) ** (1.0 / 3)
