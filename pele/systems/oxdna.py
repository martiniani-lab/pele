    ...
    def takeStep(self, coords, **kwargs):
        ...
        for com, p in zip(ca.posRigid, ca.rotRigid):
            ...
            if self.rotate_around_backbone:
                a1 = np.dot(rotations.aa2mx(p), np.array([1.0, 0.0, 0.0]))
                x1 = com - 0.4 * a1
...
class OXDNAScrewStep(takestep.TakestepInterface):
    def __init__(
        self, rotate_backbone=0.5 * pi, rotate_base=0.0, ntorsionmoves=1, P_mid=1.0
    ):
    ...
    def takeStep(self, coords, **kwargs):
        ...
        for i in range(ca.nrigid):
            ...
            a *= 2.0 * (np.random.random() - 0.5) * self.rotate_base
            ca.rotRigid[i] = rotations.rotate_aa(ca.rotRigid[i], a)

            # random rotation for angle-axis vectors
        if self.rotate_backbone != 0.0:
            for j in range(self.ntorsionmoves):
                ...
                # determine backbone beads
                a1 = np.dot(
                    rotations.aa2mx(ca.rotRigid[index - 1]), np.array([1.0, 0.0, 0.0])
                )
                a2 = np.dot(
                    rotations.aa2mx(ca.rotRigid[index]), np.array([1.0, 0.0, 0.0])
                )
                ...
                p = x2 - x1
                p /= np.linalg.norm(p)
                p *= 2.0 * (np.random.random() - 0.5) * self.rotate_backbone
                ...
                for i in range(index, ca.nrigid):
                    a = np.dot(
                        rotations.aa2mx(ca.rotRigid[i]), np.array([1.0, 0.0, 0.0])
                    )
...
class OXDNAReseed(takestep.TakestepInterface):
    ...
    def takeStep(self, coords, **kwargs):
        ...
        # random displacement for positions
        ca.posRigid[:] = 2.0 * self.radius * (np.random.random(ca.posRigid.shape) - 0.5)
...
class OXDNAReseedRandomwalk(takestep.TakestepInterface):
    ...
    def takeStep(self, coords, **kwargs):
        ...
        for pos, rot in zip(ca.posRigid, ca.rotRigid):
            ...
            a1 = np.dot(rotations.aa2mx(rot), np.array([1.0, 0.0, 0.0]))
            pos[:] = backbone + 0.4 * a1
