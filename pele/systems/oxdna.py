...
class OXDNATakestep(takestep.TakestepInterface):
    def __init__(
        self, displace=1.0, rotate=0.5 * pi, rotate_around_backbone=False
    ):
    ...
    def takeStep(self, coords, **kwargs):
        ...
        if self.rotate_backbone != 0.0:
            for j in range(self.ntorsionmoves):
                ...
                # determine backbone beads
                a1 = np.dot(
                    rotations.aa2mx(ca.rotRigid[index - 1]),
                    np.array([1.0, 0.0, 0.0]),
                )
                a2 = np.dot(
                    rotations.aa2mx(ca.rotRigid[index]),
                    np.array([1.0, 0.0, 0.0]),
                )
...
class OXDNAReseed(takestep.TakestepInterface):
    ...
    def takeStep(self, coords, **kwargs):
        ...
        # random displacement for positions
        ca.posRigid[:] = (
            2.0 * self.radius * (np.random.random(ca.posRigid.shape) - 0.5)
        )
