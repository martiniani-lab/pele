...
def dos_to_cv(energies, ldos, T, K=1.0):
...
def minima_to_cv(minima, kT, k):
    ...
    try:
        lZterms = np.array(
            [-beta * m.energy - 0.5 * m.fvib - np.log(m.pgorder) for m in minima]
        )
    except TypeError or AttributeError:
        print(
            "Error reading thermodynamic data from minima.  Have you computed the"
            " normal mode frequencies and point group order for all the minima?  See"
            " pele.thermodynamics  for more information"
        )
        raise
