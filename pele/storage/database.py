...
from sqlalchemy import (Column, Float, ForeignKey, Integer, PickleType, String,
                        and_, create_engine, or_)
from sqlalchemy.ext.declarative import declarative_base
from sqlalchemy.orm import deferred, relationship, sessionmaker, undefer
...
class SystemProperty(Base):
    ...
    def value(self):
        ...
        actual_values = [v for v in list(self._values().values()) if v is not None]
...
Index(
    "idx_transition_states",
    TransitionState.__table__.c._minimum1_id,
...
class MinimumAdder(object):
    ...
    def __call__(self, E, coords):
        ...
        self.count += 1
        return self.db.addMinimum(
            E, coords, max_n_minima=self.max_n_minima, commit=commit
        )
...
class Database(object):
    ...
    def _highest_energy_minimum(self):
        ...
        candidates = (
            self.session.query(Minimum).order_by(Minimum.energy.desc()).limit(1).all()
        )
        return candidates[0]

    def get_lowest_energy_minimum(self):
        """return the minimum with the lowest energy"""
        candidates = self.session.query(Minimum).order_by(Minimum.energy).limit(1).all()
        return candidates[0]

    def findMinimum(self, E, coords):
        candidates = (
            self.session.query(Minimum)
    ...
    def addMinimum(
        self, E, coords, commit=True, max_n_minima=-1, pgorder=None, fvib=None
    ):
    ...
    def addTransitionState(
        ...
        candidates = (
            ...
            .filter(
                or_(
                    and_(
                        TransitionState.minimum1 == m1, TransitionState.minimum2 == m2
                    ),
                    and_(
                        TransitionState.minimum1 == m2, TransitionState.minimum2 == m1
                    ),
                )
            )
            .filter(
                TransitionState.energy.between(
                    energy - self.accuracy, energy + self.accuracy
                )
            )
        )
        ...
        new = TransitionState(
            energy, coords, m1, m2, eigenval=eigenval, eigenvec=eigenvec
        )
    ...
    def getTransitionState(self, min1, min2):
        ...
        candidates = self.session.query(TransitionState).filter(
            or_(
                and_(TransitionState.minimum1 == m1, TransitionState.minimum2 == m2),
                and_(TransitionState.minimum1 == m2, TransitionState.minimum2 == m1),
            )
        )
    ...
    def getTransitionStatesMinimum(self, min1):
        ...
        """
        candidates = self.session.query(TransitionState).filter(
            or_(TransitionState.minimum1 == min1, TransitionState.minimum2 == min1)
        )
    ...
    def minimum_adder(self, Ecut=None, max_n_minima=None, commit_interval=1):
        ...
        """
        return MinimumAdder(
            self, Ecut=Ecut, max_n_minima=max_n_minima, commit_interval=commit_interval
        )
    ...
    def removeMinimum(self, m, commit=True):
        ...
        candidates = self.session.query(TransitionState).filter(
            or_(TransitionState.minimum1 == m, TransitionState.minimum2 == m)
        )
    ...
    def get_property(self, property_name):
        ...
        candidates = self.session.query(SystemProperty).filter(
            SystemProperty.property_name == property_name
        )
    ...
    def add_property(self, name, value, dtype=None, commit=True, overwrite=True):
        ...
        if new is None:
            ...
            if not same:
                if not overwrite:
                    raise RuntimeError(
                        "property %s already exists and the value %s does not compare"
                        " equal to the new value." % (new.item(), value)
                    )
        ...
        if dtype == "string":
            ...
            raise ValueError(
                'dtype must be one of "int", "float", "string", "pickle", or None'
            )
...
def test_fast_insert():  # pragma: no cover
    ...
    db.engine.execute(
        ...
        [
            dict(energy=0.01, coords=np.array([0.0, 1.0]), invalid=False),
            dict(energy=0.02, coords=np.array([0.0, 2.0]), invalid=False),
        ],
    )
    m1, m2 = db.minima()[:2]
    db.engine.execute(
        TransitionState.__table__.insert(),
        [
            dict(
                energy=1.0,
