...
class SystemProperty(Base):
    ...
    def value(self):
        ...
        actual_values = [
            v for v in list(self._values().values()) if v is not None
        ]
...
class Database(object):
    ...
    def addMinimum(
        ...
        candidates = (
            ...
            .options(undefer("coords"))
            .filter(
                Minimum.energy.between(E - self.accuracy, E + self.accuracy)
            )
        )
    ...
    def add_property(
        self, name, value, dtype=None, commit=True, overwrite=True
    ):
        ...
        if new is None:
            ...
            if not same:
                if not overwrite:
                    raise RuntimeError(
                        "property %s already exists and the value %s does not compare equal to the new value."
                        % (new.item(), value)
                    )
