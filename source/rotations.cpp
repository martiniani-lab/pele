...
pele::VecN<4> quaternion_multiply(pele::VecN<4> const &q0,
...
/**
 * compute the rotation matrix and it's derivatives from an angle axis vector if
 * the rotation angle is very small
 */
void rot_mat_derivatives_small_theta(pele::VecN<3> const &p,
                                     MatrixNM<3, 3> &rmat, MatrixNM<3, 3> &drm1,
                                     MatrixNM<3, 3> &drm2, MatrixNM<3, 3> &drm3,
                                     bool with_grad) {
  double theta2 = pele::dot<3>(p, p);
  if (theta2 > 1e-2) {
 ...
 */
void rot_mat_derivatives(pele::VecN<3> const &p, MatrixNM<3, 3> &rmat,
                         MatrixNM<3, 3> &drm1, MatrixNM<3, 3> &drm2,
                         MatrixNM<3, 3> &drm3) {
  assert(p.size() == 3);
  ...
  for (size_t i = 0; i < 3; ++i) {
    for (size_t j = 0; j < 3; ++j) {
      drm1(i, j) =
          (st * pn[0] * esq(i, j) + (1. - ct) * (de_e(i, j) + ede_(i, j)) +
           ct * pn[0] * e(i, j) + st * de1(i, j));
    }
  }
  ...
  for (size_t i = 0; i < 3; ++i) {
    for (size_t j = 0; j < 3; ++j) {
      drm2(i, j) =
          (st * pn[1] * esq(i, j) + (1. - ct) * (de_e(i, j) + ede_(i, j)) +
           ct * pn[1] * e(i, j) + st * de2(i, j));
    }
  }
  ...
  for (size_t i = 0; i < 3; ++i) {
    for (size_t j = 0; j < 3; ++j) {
      drm3(i, j) =
          (st * pn[2] * esq(i, j) + (1. - ct) * (de_e(i, j) + ede_(i, j)) +
           ct * pn[2] * e(i, j) + st * de3(i, j));
    }
  }
}
