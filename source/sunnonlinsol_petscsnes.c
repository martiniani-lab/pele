/* IMPORTANT: This is a temporary file for development purposes there is an MN suffix to emphasize that  */
/* -----------------------------------------------------------------------------
...
SUNNonlinearSolver SUNNonlinSol_PetscSNES(N_Vector y, SNES snes, CVMNPETScMem ctx)
{
  ...
  ierr = SNESSetFunction(SUNNLS_SNESOBJ(NLS), SUNNLS_SNES_CONTENT(NLS)->r, PetscSysFnMN, NLS);
  if (ierr != 0) {
    SUNNonlinSolFree(NLS);
    return NULL;
  }
...
int SUNNonlinSolInitialize_PetscSNES(SUNNonlinearSolver NLS)
{
  ...
  ptcerr = SNESSetFunction(SUNNLS_SNESOBJ(NLS), SUNNLS_SNES_CONTENT(NLS)->r, PetscSysFnMN, NLS);
  if (ptcerr != 0) {
