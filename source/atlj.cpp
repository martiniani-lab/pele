...
double
ATLJ::get_energy_gradient(const Array<double> &x,
...
double ATLJ::get_energy_gradient(const Array<double> &x, Array<double> &grad) {
...
double ATLJ::get_energy_gradient_hessian(const Array<double> &x,
                                         Array<double> &grad,
                                         Array<double> &hess) {
  // double lj_energy = ljpot.get_energy_gradient_hessian(x, grad, hess);
  // // AT_Energy_gradient_hessian accumulates without resetting the gradient
  // and hessian double at_energy = get_AT_energy_gradient_hessian(x, grad,
  // hess);
...
double ATLJ::get_AT_energy_gradient_hessian(const Array<double> &x,
                                            Array<double> &grad,
                                            Array<double> &hess) {
...
var ATLJ::Atenergy(const Eigen::VectorXvar &x) {
  ...
  var rki = drki.norm();
  // note that this is slightly differently written, the - sign is taken outside
  var costhetaprod = -(3 * drij.dot(drjk) * drjk.dot(drki) * drki.dot(drij)) /
                     pow((rij * rjk * rki), 2);
  energy += m_Z * (1 + costhetaprod) / (pow((rij * rjk * rki), 3));
  return energy;
}
...
double
ATLJ::get_AT_energy_gradient(const Array<double> &x,
                             std::vector<xsum_small_accumulator> &exact_grad) {
  double energy = 0;
  ...
  // gradient done with chain rule and terms directly accumulated with
  // prefactors into the exact gradient grad = -d(c)/bd +(c/bd^2)* d(d) -
  // (a/b^2)* d(b)
...
#pragma simd
  ...
  piecewise_derivative(rij, rjk, rki, drij, drjk, drki, -3 * m_Z / (b * d),
                       exact_grad);
  prod_derivative(rij, rjk, rki, drij, drjk, drki, 2, 3 * c * m_Z / (b * d * d),
                  exact_grad);
  prod_derivative(rij, rjk, rki, drij, drjk, drki, 3, -a * m_Z / (b * b),
                  exact_grad);
  return energy;
}

void ATLJ::piecewise_derivative(
    double rij, double rjk, double rki, Array<double> drij, Array<double> drjk,
    Array<double> drki, double prefactor,
    std::vector<xsum_small_accumulator> &exact_grad) {
  ...
  for (size_t j = 0; j < m_ndim; ++j) {
    // atom i
    xsum_small_add1(&(exact_grad[j]),
                    prefactor * drjk[j] * drjk_dot_drki * drki_dot_drij);
    xsum_small_add1(&(exact_grad[j]),
                    -prefactor * drij_dot_drjk * drjk[j] * drki_dot_drij);
    xsum_small_add1(&(exact_grad[j]),
                    prefactor * drij_dot_drjk * drjk_dot_drki * (drki[j]));
    xsum_small_add1(&(exact_grad[j]),
                    prefactor * drij_dot_drjk * drjk_dot_drki * (-drij[j]));
    // atom j
    xsum_small_add1(&(exact_grad[j + m_ndim]),
                    prefactor * (drij[j]) * drjk_dot_drki * drki_dot_drij);
    xsum_small_add1(&(exact_grad[j + m_ndim]),
                    prefactor * (-drjk[j]) * drjk_dot_drki * drki_dot_drij);
    xsum_small_add1(&(exact_grad[j + m_ndim]),
                    prefactor * drij_dot_drjk * drki[j] * drki_dot_drij);
    xsum_small_add1(&(exact_grad[j + m_ndim]),
                    -prefactor * drij_dot_drjk * drjk_dot_drki * (drki[j]));
    // atom k
    xsum_small_add1(&(exact_grad[j + 2 * m_ndim]),
                    -prefactor * drij[j] * drjk_dot_drki * drki_dot_drij);
    xsum_small_add1(&(exact_grad[j + 2 * m_ndim]),
                    prefactor * drij_dot_drjk * (drjk[j]) * drki_dot_drij);
    xsum_small_add1(&(exact_grad[j + 2 * m_ndim]),
...
void ATLJ::prod_derivative(double rij, double rjk, double rki,
                           Array<double> drij, Array<double> drjk,
                           Array<double> drki, double n, double prefactor,
                           std::vector<xsum_small_accumulator> &exact_grad) {
  for (size_t j = 0; j < m_ndim; ++j) {
    // atom i
    xsum_small_add1(&exact_grad[j], n * prefactor * pow(rij * rjk * rki, n) *
                                        (drij[j] / (rij * rij)));
    xsum_small_add1(&exact_grad[j], n * prefactor * pow(rij * rjk * rki, n) *
                                        (-drki[j] / (rki * rki)));
    // atom j
    xsum_small_add1(&exact_grad[j + m_ndim], n * prefactor *
                                                 pow(rij * rjk * rki, n) *
                                                 (drjk[j] / (rjk * rjk)));
    xsum_small_add1(&exact_grad[j + m_ndim], n * prefactor *
                                                 pow(rij * rjk * rki, n) *
                                                 (-drij[j] / (rij * rij)));
    // atom k
    xsum_small_add1(&exact_grad[j + 2 * m_ndim], n * prefactor *
                                                     pow(rij * rjk * rki, n) *
                                                     (drki[j] / (rki * rki)));
    xsum_small_add1(&exact_grad[j + 2 * m_ndim], n * prefactor *
                                                     pow(rij * rjk * rki, n) *
