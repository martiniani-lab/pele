...
double pressure_tensor(std::shared_ptr<pele::BasePotential> pot_,
                       ...
                       const double volume) {
  pele::PairwisePotentialInterface *pot =
      dynamic_cast<pele::PairwisePotentialInterface *>(pot_.get());
  if (pot == NULL) {
  ...
  ptensor /= volume;
  // pressure is the average of the trace of the pressure tensor divided by the
  // volume, here we return only the trace divided by the dimensionality
  double traceP = 0.;
