...
class MeanFieldPSpinSpherical : public BasePotential {
 protected:
  ...
  // this function normalizes the spin vector and orthogonalises the gradient to
  // it
  inline void m_orthogonalize(Array<double> const &spins_,
                              Array<double> &grad) {
 ...
 public:
  ...
  MeanFieldPSpinSpherical(pele::Array<double> interactions_, size_t nspins_,
      ...
      : m_interactions(interactions_.copy()),
        ...
        m_combination_generator_grad(m_indexes.begin(), m_indexes.end(),
                                     m_p - 1),
        m_combination_generator_hess(m_indexes.begin(), m_indexes.end(),
                                     m_p - 2) {
    static_assert(m_p >= 2, "MeanFieldPSpinSpherical: p cannot be less than 2");
    if ((size_t)std::pow(m_N, m_p) != m_interactions.size()) {
      // note: in an ideal implementation interactions should actually be of
      // size (N r)
      throw std::invalid_argument(
          "MeanFieldPSpinSpherical: interactions is of the wrong size");
    }
  ...
  inline double add_energy_gradient(pele::Array<double> const &x,
                                    pele::Array<double> &grad);
  inline double add_energy_gradient_hessian(pele::Array<double> const &x,
                                            pele::Array<double> &grad,
                                            pele::Array<double> &hess);
  virtual inline double get_energy(pele::Array<double> const &x);
  virtual inline double get_energy_gradient(pele::Array<double> const &x,
                                            pele::Array<double> &grad);
...
inline double MeanFieldPSpinSpherical<p>::add_energy_gradient(
  ...
  for (size_t i = 0; i < m_N; ++i) {
    ...
    while (go_on) {
      ...
      bool repeated_idx =
          std::find(combg, combg + m_p - 1, i) != combg + m_p - 1;
      if (!repeated_idx) {
...
template <size_t p>
void MeanFieldPSpinSpherical<p>::numerical_gradient(Array<double> const &x,
                                                    Array<double> &grad,
                                                    double eps) {
  Array<double> xnew = x.copy();
  this->m_normalize_spins(xnew);  // normalize spins vector
...
template <size_t p>
void MeanFieldPSpinSpherical<p>::numerical_hessian(Array<double> const &x,
                                                   Array<double> &hess,
                                                   double eps) {
...
template <size_t p>
inline double MeanFieldPSpinSpherical<p>::add_energy_gradient_hessian(
      ...
      for (size_t j = i; j < m_N; ++j) {
        ...
        if (i != j) {
          ...
          while (go_on) {
            ...
            bool repeated_idx =
                (std::find(combh, combh + m_p - 2, i) != combh + m_p - 2 ||
  ...
  // I can do so by performing svd, orthogonalizing each vector wrt the radial
  // vector and rebuilding it
