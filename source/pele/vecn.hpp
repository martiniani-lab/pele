...
public:
  ...
  VecN(pele::Array<dtype> const &x) {
    if (x.size() != N) {
      throw std::runtime_error(
          "VecN constructor: array must have the same size as vector");
    }
   ...
   */
  void assign(dtype const &d) { std::fill(begin(), end(), d); }

  /**
   * Assignment function: copy the data into the existing array
   */
  void assign(const dtype *const rhs) { std::copy(rhs, rhs + N, begin()); }
...
public:
  ...
  MatrixNM(pele::Array<dtype> const &x) {
    if (x.size() != m_size) {
      std::stringstream ss;
      ss << "MatrixNM constructor: array (size " << x.size()
         << ") must have the same size as matrix " << m_size;
  ...
  inline dtype &operator()(size_t i, size_t j) { return m_data[i * M + j]; }
...
MatrixNM<N, M, dtype> dot(MatrixNM<N, L, dtype> const &A,
                          MatrixNM<L, M, dtype> const &B) {
...
pele::VecN<N, dtype> dot(MatrixNM<N, M, dtype> const &A,
                         pele::VecN<M, dtype> const &v) {
