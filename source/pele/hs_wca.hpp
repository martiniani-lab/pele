 ...
 * V_\text{sfHS-WCA}(r^2) = 0 \text{ if } r \geq r_S V_\text{sfHS-WCA}(r^2) =
 * V_\text{fHS-WCA}(r^2)                           \text{ if } r_\times < r <
 * r_S V_\text{sfHS-WCA}(r^2) = E_\times - (\sqrt{r^2} - r_\times)G_\times
...
double sf_HS_WCA_interaction<exp>::energy(const double r2,
  ...
  const double C_ir_2m = C_ir_m * C_ir_m;
  const double EX =
      std::max<double>(0, 4. * m_eps * (C_ir_2m - C_ir_m) + m_eps);
  const double GX =
      (m_eps * (m_grad_prfac2 * C_ir_2m - m_grad_prfac * C_ir_m) * ir) * (-r_X);
...
double sf_HS_WCA_interaction<exp>::energy_gradient_hessian(
  ...
  if (__builtin_expect(r2 > r_X2, 1)) {
    ...
    *gij = m_eps * (m_grad_prfac2 * C_ir_2m - m_grad_prfac * C_ir_m) * ir;
    *hij = -*gij + m_eps *
                       (m_hessian_prfac2 * C_ir_2m - m_hessian_prfac * C_ir_m) *
                       r2 * ir * ir;
    return std::max<double>(0, 4. * m_eps * (C_ir_2m - C_ir_m) + m_eps);
  }
  // r2 <= r_X2
...
/**
 * Fast pairwise interaction for Hard Sphere + Weeks-Chandler-Andersen (fHS_WCA)
 * potential, refer to S. Martiniani CPGS pp 20 well depth _eps and scaling
 * factor (shell thickness = sca * R, where R is the hard core radius), sca
...
struct HS_WCA_interaction : BaseInteraction {
  ...
  double compute_energy(const double C_ir_6, const double C_ir_12) const {
    return std::min<double>(
        _infty, std::max<double>(0, 4. * _eps * (-C_ir_6 + C_ir_12) + _eps));
  }
...
public:
  HS_WCA(double eps, double sca, Array<double> radii)
      : SimplePairwisePotential<sf_HS_WCA_interaction<exp>,
                                cartesian_distance<ndim>>(
            std::make_shared<sf_HS_WCA_interaction<exp>>(eps, sca), radii,
            std::make_shared<cartesian_distance<ndim>>(), sca) {}
};
...
public:
  HS_WCAPeriodic(double eps, double sca, Array<double> radii,
      ...
      : SimplePairwisePotential<sf_HS_WCA_interaction<exp>,
                                periodic_distance<ndim>>(
            std::make_shared<sf_HS_WCA_interaction<exp>>(eps, sca), radii,
            std::make_shared<periodic_distance<ndim>>(boxvec), sca) {}
};
...
public:
  HS_WCALeesEdwards(double eps, double sca, Array<double> radii,
      ...
      : SimplePairwisePotential<sf_HS_WCA_interaction<exp>,
                                leesedwards_distance<ndim>>(
            std::make_shared<sf_HS_WCA_interaction<exp>>(eps, sca), radii,
            std::make_shared<leesedwards_distance<ndim>>(boxvec, shear), sca) {}
};
...
public:
  ...
  size_t get_nr_unique_pairs() const {
    return CellListPotential<sf_HS_WCA_interaction<exp>,
                             cartesian_distance<ndim>>::m_celliter
        ->get_nr_unique_pairs();
  }
};
...
public:
  ...
  size_t get_nr_unique_pairs() const {
    return CellListPotential<sf_HS_WCA_interaction<exp>,
                             periodic_distance<ndim>>::m_celliter
        ->get_nr_unique_pairs();
  }
};
...
public:
  HS_WCALeesEdwardsCellLists(double eps, double sca, Array<double> radii,
                             ...
                             const double ncellx_scale = 1.0,
                             const bool balance_omp = true)
      : CellListPotential<sf_HS_WCA_interaction<exp>,
                          leesedwards_distance<ndim>>(
            std::make_shared<sf_HS_WCA_interaction<exp>>(eps, sca),
  ...
  size_t get_nr_unique_pairs() const {
    return CellListPotential<sf_HS_WCA_interaction<exp>,
                             leesedwards_distance<ndim>>::m_celliter
        ->get_nr_unique_pairs();
  }
};
...
public:
  HS_WCANeighborList(Array<size_t> &ilist, double eps, double sca,
      ...
      : SimplePairwiseNeighborList<sf_HS_WCA_interaction<exp>>(
            std::make_shared<sf_HS_WCA_interaction<exp>>(eps, sca), ilist,
            radii) {}
};
