...
protected:
  ...
  void setup_cells(const size_t nsubdoms,
                   const std::vector<size_t> subdom_ncells) {
#ifdef _OPENMP
#pragma omp parallel
    {
...
public:
  ...
  std::vector<std::vector<std::array<cell_t *, 2>>>
      m_cell_neighbor_pairs_boundary;
...
#pragma omp parallel
   ...
   */
  inline void remove_atom_from_cell_ind(const size_t index, const size_t icell,
                                        const size_t isubdom) {
  ...
  inline void remove_atom_from_cell(const size_t iatom, const size_t icell,
                                    const size_t isubdom) {
    auto atom_ptr = std::find(m_cell_atoms[isubdom][icell].begin(),
                              m_cell_atoms[isubdom][icell].end(), iatom);
  ...
  void calc_subdom_stats() {
    ...
    if ((m_boxvec[1] / m_rcut) / m_nsubdoms < 2 && omp_get_max_threads() > 1) {
      throw std::runtime_error(
          "Some subdomains are thinner than 2*rcut. The "
...
public:
   ...
   * The cell vector needs to be within range [0, m_ncells_vec]
   */
  size_t cell_vec_to_local_ind(cell_vec_t const &v,
   ...
   */
  cell_vec_t local_ind_to_cell_vec(const size_t icell,
                                   const size_t isubdom) const {
    cell_vec_t v;
    size_t remaining_icell = icell;
    size_t fraction;
    for (size_t idim = 0; idim < ndim - 1; ++idim) {
      if (idim == 1) {
        size_t split_len =
            m_subdom_limits[isubdom + 1] - m_subdom_limits[isubdom];
  ...
  /**
   * return the subdomain and the local index of the cell that contains position
   * x
   */
  void position_to_local_ind(const double *const x, size_t &icell,
                             size_t &isubdom) const {
    // note: the speed of this function is important because it is called
   ...
   */
  void global_ind_to_local_ind(const size_t icell_global, size_t &icell_local,
                               size_t &isubdom) const {
   ...
   */
  size_t local_ind_to_global_ind(const size_t icell_local,
                                 const size_t isubdom) const {
  ...
  size_t get_subdomain(cell_vec_t const &v) const {
    if (m_subdoms_balanced) {
      ...
      return std::upper_bound(m_subdom_limits.begin(), m_subdom_limits.end(),
                              v[1]) -
...
#pragma omp parallel
    {
      size_t isubdom = omp_get_thread_num();
      find_neighbor_pairs_subdom(cell_neighbors_inner[isubdom],
                                 cell_neighbors_boundary[isubdom],
                                 cell_neighbors, cells, isubdom);
    }
#else
    for (size_t isubdom = 0; isubdom < m_nsubdoms; ++isubdom) {
      find_neighbor_pairs_subdom(cell_neighbors_inner[isubdom],
  ...
  /**
   * return a list of all pairs of neighboring cells originating from a
   * subdomain
   */
  void find_neighbor_pairs_subdom(
      ...
      std::vector<std::vector<cell_t *>> &cell_neighbors,
      std::vector<std::vector<cell_t>> &cells, const size_t isubdom) const {
    ...
    if (m_nsubdoms == 1) {
      neighbor_pairs_inner.reserve(m_subdom_ncells[isubdom] *
                                   ((std::pow(3, ndim) + 1) * 0.5));
    } else {
      const size_t subdom_length =
          m_subdom_limits[isubdom + 1] - m_subdom_limits[isubdom];
      const size_t surface_ncells = m_subdom_ncells[isubdom] / subdom_length;
      if (isubdom == 0) {
        // For Lees-Edwards Boundary Conditions
        neighbor_pairs_boundary.reserve(surface_ncells * 4 *
                                        std::pow(3, ndim - 2));
      } else {
      ...
      neighbor_pairs_inner.reserve(m_subdom_ncells[isubdom] *
                                       ((std::pow(3, ndim) + 1) * 0.5) -
                                   surface_ncells * std::pow(3, ndim - 1));
    }

    for (size_t local_icell = 0; local_icell < m_subdom_ncells[isubdom];
         ++local_icell) {
...
public:
   ...
   */
  CellLists(std::shared_ptr<distance_policy> const &dist,
  ...
  pele::Array<size_t> get_order(const size_t natoms) const {
...
CellLists<distance_policy>::CellLists(
    ...
    : m_initialized(false),
      m_lattice_tool(dist, boxv, rcut,
                     get_ncells_vec(boxv, rcut, ncellx_scale, balance_omp)),
  ...
  for (size_t i = 1; i < boxv.size(); ++i) {
    if (boxv[i] < 0) {
      throw std::runtime_error(
          "CellLists::CellLists: illegal input: boxvector");
    }
  }
  if (ncellx_scale < 0) {
    throw std::runtime_error(
        "CellLists::CellLists: illegal input: ncellx_scale");
  }
#ifdef _OPENMP
  ...
  if (m_lattice_tool.m_rcut >
      0.5 * *std::min_element(m_lattice_tool.m_boxvec.begin(),
                              m_lattice_tool.m_boxvec.end())) {
    // an atom can interact with more than just the nearest image of it's
    // neighbor
    std::cerr << "CellLists: warning: rcut > half the box length.  This might "
                 "cause errors with periodic boundaries.\n";
  }
}

template <typename distance_policy>
Array<size_t> CellLists<distance_policy>::get_ncells_vec(
    Array<double> const &boxv, const double rcut, const double ncellx_scale,
    const bool balance_omp) {
...
void CellLists<distance_policy>::print_warnings(const size_t natoms) {
  if (m_lattice_tool.m_ncells > natoms) {
    ...
    std::cout << "CellLists: efficiency warning: the number of cells ("
              << m_lattice_tool.m_ncells << ")"
...
void CellLists<distance_policy>::build_cell_neighbors_list() {
  m_lattice_tool.find_neighbor_pairs(m_container.m_cell_neighbor_pairs_inner,
...
/**
 * remove all atoms and re-add them to the right cells according to the new
 * coordinates
 */
template <typename distance_policy>
void CellLists<distance_policy>::reset_container(
 ...
 */
template <typename distance_policy>
void CellLists<distance_policy>::update_container(
    pele::Array<double> const &coords) {
#ifdef _OPENMP
#pragma omp parallel
  {
    size_t isubdom = omp_get_thread_num();
    for (size_t icell = 0; icell < m_lattice_tool.m_subdom_ncells[isubdom];
         ++icell) {
      size_t atom_nr = 0;
...
void CellLists<distance_policy>::update_container_specific(
  ...
  for (size_t iatom = 0; iatom < iatoms.size(); iatom++) {
    ...
    m_lattice_tool.position_to_local_ind(old_coords.data() + m_ndim * iatom,
                                         old_cell, old_subdom);
    size_t new_cell, new_subdom;
    m_lattice_tool.position_to_local_ind(coords.data() + m_ndim * iatoms[iatom],
                                         new_cell, new_subdom);
