 ...
 *
 * 3*nrigid -- 6*nrigid            : the rotations of the rigid bodies in angle
 * axis coords
 *
 * 6*nrigid -- 6*nrigid + 3*natoms : the positions of the non-rigid atoms (point
 * masses) (not yet supported)
 *
 * ...      -- end                 : the last nlattice spaces are for the
 * lattice degrees of freedom (not yet supported)
 */
class CoordsAdaptor {
 ...
 public:
  CoordsAdaptor(size_t nrigid, size_t natoms, pele::Array<double> coords)
    ...
    if (coords.size() != (6 * _nrigid + 3 * _natoms)) {
      throw std::invalid_argument(std::string("coords has the wrong size ") +
...
class TransformPolicy {
 public:
   ...
   */
  virtual void rotate(pele::Array<double> x,
                      pele::MatrixNM<3, 3> const &mx) = 0;
...
class MeasureAngleAxisCluster {
 public:
  ...
  /**
   * align the rotations so that the atomistic coordinates will be in best
   * alignment
   */
...
class RigidFragment {
 ...
 public:
  RigidFragment(pele::Array<double> atom_positions, Array<double> cog, double M,
    ...
    if (_atom_positions.size() != _natoms * _ndim) {
      throw std::invalid_argument(
          "the length of atom_positions must be divisible by 3");
    }
  }
  ...
  /**
   * convert a center of mass and a angle axis rotation to a set of atomistic
   * coordinates
   */
   ...
   */
  void transform_grad(pele::VecN<3> const &p, pele::Array<double> const g,
                      pele::VecN<3> &g_com, pele::VecN<3> &g_rot);
   ...
   */
  void transform_grad(pele::Array<double> const p, pele::Array<double> const g,
                      pele::Array<double> g_com, pele::Array<double> g_rot) {
    pele::VecN<3> p_vec = p;
   ...
   */
  inline pele::VecN<3> get_smallest_rij(pele::VecN<3> const &com1,
                                        pele::VecN<3> const &com2) const {
...
class RBTopology {
 ...
 public:
  ...
  /**
   * Ensure the angle axis rotations of two structures are aligned with each
   * other.
   *
   ...
   */
  double distance_squared(pele::Array<double> const x1,
                          pele::Array<double> const x2) const {
    double d_sq = 0;
    ...
    for (size_t isite = 0; isite < nrigid(); ++isite) {
      d_sq += _sites[isite].distance_squared(
          ca1.get_rb_position(isite), ca1.get_rb_rotation(isite),
          ca2.get_rb_position(isite), ca2.get_rb_rotation(isite));
    }
    return d_sq;
  }
   ...
   */
  void distance_squared_grad(pele::Array<double> const x1,
                             pele::Array<double> const x2,
                             pele::Array<double> grad) const {
    ...
    for (size_t isite = 0; isite < nrigid(); ++isite) {
      ...
      _sites[isite].distance_squared_grad(
          ca1.get_rb_position(isite), ca1.get_rb_rotation(isite),
          ca2.get_rb_position(isite), ca2.get_rb_rotation(isite), g_M, g_P);
 ...
 *
 * this converts rigid body coords to atomistic coords and passes the atomistic
 * coords to the potential function.  It also converts the atomistic gradient
 * into a gradient in the rigid body coordinate system.
 */
