...
class NGT {
 public:
  ...
  NGT(std::shared_ptr<Graph> graph, Acontainer const &A, Bcontainer const &B)
    ...
    //        "B.size() " << _B.size() << "\n"; std::cout <<
    //        "intermediates.size() " << intermediates.size() << "\n";
    assert(intermediates.size() + _A.size() + _B.size() ==
  ...
  NGT(rate_map_t &rate_constants, Acontainer const &A, Bcontainer const &B)
      : _graph(new Graph()), debug(false) {
    std::set<node_ptr> nodes;

    // add nodes to the graph and sum the rate constants for all out edges for
    // each node.
   ...
   *
   * This is done because it is faster to remove nodes with fewer connections
   * first.
   */
  ...
  /*
   * This returns P for the edge u->u.  This is slow because the edge must first
   * be found.
   */
   ...
   *
   * If P is close to one compute 1.-P directly by summing P over all the out
   * edges of u. This is extremely important for numerical precision.  It is
   * this ability to deal precisely with both P and 1.-P that makes this method
   * more stable then linear algebra methods.
   */
  double get_node_one_minus_P(node_ptr u) {
    ...
    if (Puu < 0.99) {
      ...
      double omPuu = 0.;
      for (auto eiter = u->out_edge_begin(); eiter != u->out_edge_end();
           ++eiter) {
  ...
  void update_node(edge_ptr ux, double omPxx, double tau_x) {
    ...
    if (debug) {
      std::cout << "updating node " << u->id() << " tau " << tau_u << " -> "
                << new_tau_u << "\n";
    }
    set_tau(u, new_tau_u);
  }
  ...
  void update_edge(node_ptr u, node_ptr v, edge_ptr ux, edge_ptr xv,
    ...
    if (debug) {
      std::cout << "updating edge " << u->id() << " -> " << v->id() << " Puv "
                << Puv << " -> " << newPuv << " 1-Pxx " << omPxx << " Pux "
                << Pux << " Pxv " << Pxv << "\n";
    }
    set_P(uv, newPuv);
  }
  ...
  void remove_node(node_ptr x) {
    ...
    //
    for (auto uxiter = x->in_edge_begin(); uxiter != x->in_edge_end();
         ++uxiter) {
      edge_ptr ux = *uxiter;
      node_ptr u = ux->tail();
      if (u == x) continue;
   ...
   */
  void phase_one() { remove_intermediates(); }

  /*
   * Compute final_tau and final_omPxx for each node in to_remove
   *
   * For each node x in to_remove, this involves removing all other nodes in
   * to_remove, and getting the results from this reduced graph.
   */
  void reduce_all_in_group(std::set<node_ptr> &to_remove,
    ...
    if (Aids.size() > 1) {
      ...
      while (Aids.size() > 1) {
        /*
         * Create a new graph and a new NGT object new_ngt.  Pass x as A and
         * Bids as B.  new_ngt will remove all `intermediates`, i.e. everything
         * in Aids except x.  Then save the final value of 1-Pxx and tau_x.
         */
  ...
  /*
   * Phase two, compute final_tau and final_omPxx for each x separately in _A
   * and in _B
   */
  void phase_two() {
   ...
   * Return the rate A->B
   */
  double get_rate_AB() { return _get_rate_final(_A); }

  /*
   * Return the rate B->A
   */
  double get_rate_BA() { return _get_rate_final(_B); }
  ...
  double _get_rate_SS(std::set<node_ptr> &A, std::set<node_ptr> &B) {
    ...
    for (auto a : A) {
      ...
      double PaB = 0.;
      for (auto eiter = a->out_edge_begin(); eiter != a->out_edge_end();
           ++eiter) {
        edge_ptr ab = *eiter;
        node_ptr b = ab->head();
        if (B.find(b) != B.end()) {
   ...
   */
  double get_rate_AB_SS() { return _get_rate_SS(_A, _B); }
   ...
   * this must be called after calling phase_one
   */
  double get_rate_BA_SS() { return _get_rate_SS(_B, _A); }

  /*
   * sum the probabilities of the out edges of x that end in B normalized by
   * 1-Pxx
   */
  double get_PxB(node_ptr x, std::set<node_id> &B) {
    ...
    double Pxx = 0.;
    double omPxx = 0.;
    for (auto eiter = x->out_edge_begin(); eiter != x->out_edge_end();
         ++eiter) {
      edge_ptr xb = *eiter;
      node_ptr b = xb->head();
      double Pxb = get_P(xb);
   ...
   *
   * \param to_remove a list of nodes that will be removed.  Committor values
   * will be computed for these nodes \param to_keep a list of nodes that should
   * not be deleted. \param committor_targets a list of nodes that should not be
   * deleted.  These nodes will be the targets in the committor calucation.
   *
  ...
  void _remove_nodes_and_compute_committors(
      std::list<node_ptr> &to_remove, std::set<node_ptr> &to_keep,
    ...
    while (to_remove_cp.size() > 0) {
      /*
       * Create a new graph and a new NGT object new_ngt.  Pass x as A and Bids
       * as B.  new_ngt will remove all `intermediates`, i.e. everything in
       * to_remove except x.  Then save the final value of 1-Pxx and tau_x.
       */
      // choose an element x and remove it from the list
  ...
  /*
   * Compute the rate from A->B and committor probabilities for all
   * intermediates
   *
