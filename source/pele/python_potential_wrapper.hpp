...
*);
        ...
        PotentialFunction(EnergyCallback *get_energy, EnergyGradientCallback
*get_energy_gradient, void *userdata) :    _get_energy(get_energy),
_get_energy_gradient(get_energy_gradient), _userdata(userdata) {}

        virtual double get_energy(Array<double> const & x) { return
(*_get_energy)(x, _userdata); } ; virtual double
get_energy_gradient(Array<double> const & x, Array<double> & grad) {  return
(*_get_energy_gradient)(x, grad, _userdata); }
...
class PythonPotential : public BasePotential {
 ...
 public:
  ...
  virtual ~PythonPotential() { Py_XDECREF(_potential); }
  ...
  virtual double get_energy(Array<double> const &x) {
    ...
    // otherwise this alone would do PyObject * numpyx =
    // PyArray_SimpleNewFromData(1, &N, NPY_DOUBLE, x.data());
...
// call the function getEnergy
...
#endif
    PyObject *returnval =
        PyObject_CallMethodObjArgs(_potential, name, numpyx, NULL);
    Py_XDECREF(name);
    Py_XDECREF(numpyx);
    if (!returnval) {
      // parse error
    ...
    if (PyErr_Occurred()) {
      ...
      PyErr_SetString(
          PyExc_TypeError,
          "return value of getEnergy could not be converted to float");
...
// call the function getEnergy
    ...
    // note: NPY_CARRAY is for numpy version 1.6, for later version use
    // NPY_ARRAY_CARRAY
    ...
    if (static_cast<size_t>(PyArray_Size(npgrad_safe)) != grad.size()) {
      PyErr_SetString(PyExc_IndexError,
                      "gradient returned by getEnergyGradient has wrong size.");
      Py_XDECREF(returnval);
