...
template <typename dtype> class _ArrayMemory {
  ...
  dtype
      *_data; /** _data will either point to the beginning of _vector, or to the
                 beginning of the block of externally allocated memory.  _data
                 is a simple pointer and will not represent allocated memory. */
...
public:
   ...
   *
   * Note, the input here is a const array, but this constructs a modifiable
   array.  This
   * is a loophole around the const declaration, but i'm not sure there is any
   way around it.
   * The compiler complains if this is not here.  e.g. for constructions like
   Array(x.copy())
   *
  ...
  */
   ...
   *
   * This is commented because it just duplicates the default assignment
   operator
   *
  ...
  */
    ...
    if ((*this) != rhs) {
      if (size() != rhs.size()) {
        throw std::runtime_error(
            "arrays must have the same size during assignment");
      }
  ...
  inline long int reference_count() const { return _memory.use_count(); }
  ...
  dtype sum() const {
    if (empty()) {
      throw std::runtime_error(
          "array::sum(): array is empty, can't sum array elements");
    }
 ...
 */
inline double norm(Array<double> const &v) { return sqrt(dot(v, v)); }
