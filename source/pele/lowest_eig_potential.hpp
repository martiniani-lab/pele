...
class OrthogonalizeTranslational : public Orthogonalize {
 ...
 public:
  ...
  // virtual inline void orthogonalize(Array<double> const & coords,
  // Array<double>& vector);
  virtual inline void orthogonalize(Array<double> const &coords,
                                    Array<double> &vector) {
    bool success = true;
...
class LowestEigPotential : public BasePotential {
 ...
 public:
  ...
  void set_x_opt(pele::Array<double> x) { x_opt = pele::Array<double>(x); }
  ...
  virtual double inline get_energy(pele::Array<double> const &x) {
    if (x_opt != x) {
      throw std::runtime_error(
          "LowestEigPotential::get_energy: x_opt is different from x. Use "
   ...
   * g/|rij|, r0 is the hard core distance, r is the distance between the
   * centres */
  virtual double inline get_energy_gradient(pele::Array<double> const &x,
                                            pele::Array<double> &grad) {
    if (x_opt != x) {
      throw std::runtime_error(
          "LowestEigPotential::get_energy_gradient: x_opt is different from x. "
