...
struct InversePowerStillingerQuadCutInteraction : BaseInteraction {
  ...
  InversePowerStillingerQuadCutInteraction(const size_t pow, const double v0,
      ...
      : m_pow(pow), m_pow_by_2(pow/2), m_v0(v0), m_cutoff_factor(cutoff_factor),
        ...
        c4(-(1.0 / 8.0) * std::pow(m_cutoff_factor, -pow - 4) *
           (pow * pow + 2 * pow)) {
            if (pow%2 != 0) {
                ...
                throw std::runtime_error("InversePowerStillingerQuadCutInteraction: only implemented for even powers");
            }
           }
...
public:
  InversePowerStillingerCutQuad(const size_t pow, const pele::Array<double> radii,
                            double rcut)
...
public:
  InversePowerStillingerCutQuadPeriodic(const size_t pow,
...
class InversePowerStillingerCutQuadPeriodicCellLists
    : public CellListPotential<InversePowerStillingerQuadCutInteraction,
                               periodic_distance<ndim>> {
public:
  InversePowerStillingerCutQuadPeriodicCellLists(const size_t pow,
                                             const pele::Array<double> radii,
                                             double rcut,
                                             const pele::Array<double> boxvec,
                                             double ncellx_scale)
      : CellListPotential<InversePowerStillingerQuadCutInteraction,
            ...
            std::make_shared<InversePowerStillingerQuadCutInteraction>(pow,
                                                                       rcut),
            std::make_shared<periodic_distance<ndim>>(boxvec), boxvec, rcut,
            ncellx_scale, radii) {}
};
