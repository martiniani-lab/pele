/**
 * LineSearch parameters given in as taken from
 * https://github.com/yixuan/LBFGSpp/ wrapped around in nwpele
 */
...
public:
  ...
  /// where \f$||\cdot||\f$ denotes the Euclidean (L2) norm. The default value
  /// is \c 1e-5.
  ///
  Scalar epsilon;
  ///
  ...
  /// where \f$f_k(x)\f$ is the current function value, and \f$f_{k-d}(x)\f$ is
  /// the function value \f$d\f$ iterations ago (specified by the \ref past
  /// parameter). The default value is \c 0.
  ///
  Scalar delta;
  ///
...
public:
  ...
  ///
  ///
  LBFGSParam(int m_ = 6, Scalar epsilon_ = Scalar(1e-5),
             Scalar epsilon_rel_ = Scalar(1e-5), int past_ = 0,
             Scalar delta_ = Scalar(0), int max_linesearch_ = 200,
