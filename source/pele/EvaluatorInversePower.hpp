...
/*! \file EvaluatorPairForceShiftedLJ.h
   ...
   serves as the primary documentation and base reference for the implementation
   of pair evaluators.
*/
...
namespace hoomd_pele {
namespace md {
//! Class for evaluating the force shifted LJ pair potential
/*! This evaluator is a variant of the Lennard-Jones pair potential, for which
   the force goes smoothly to zero at \f$ r = r_{\mathrm{cut}} \f$.
    EvaluatorPairForceShiftedLJ evaluates the function:
    \f[ V(r) = V_{LJ}(r) + \Delta V(r) \f], where
    \f[ V_{\mathrm{LJ}}(r) = 4 \varepsilon \left[ \left( \frac{\sigma}{r}
   \right)^{12} - \left( \frac{\sigma}{r} \right)^{6} \right] \f] is the
   standard Lennard-Jones pair potential and \f[ \Delta V(r) = -(r -
   r_{\mathrm{cut}}) \frac{\partial V_{\mathrm{LJ}}}{\partial
   r}(r_{\mathrm{cut}}) \f]. The constant value \f$ -\frac{1}{r} \frac{\partial
   V}{\partial r}(r_{\mathrm{cut}}) \f$ is subtracted from \a force_divr . The
   two parameters \a lj1 and \a lj2 are the same as for the (non-force-shifted)
   LJ potential.
*/
class EvaluatorPairForceShiftedLJ {
 public:
  ...
  /*! \param _rsq Squared distance between the particles
      \param _rcutsq Squared distance at which the potential and the force go to
     0 \param _params Per type pair parameters of this potential
  */
  ...
  */
  ...
  /*! \param force_divr Output parameter to write the computed force divided by
     r. \param pair_eng Output parameter to write the computed pair energy
      \param energy_shift If true, the potential must be shifted so that V(r) is
     continuous at the cutoff \note There is no need to check if rsq < rcutsq in
     this method. Cutoff tests are performed in PotentialPair. \return True if
     they are evaluated or false if they are not because we are beyond the
     cutoff
  */
  DEVICE bool evalForceAndEnergy(Scalar &force_divr, Scalar &pair_eng,
    ...
    if (rsq < rcutsq && lj1 != 0) {
      ...
      force_divr =
          r2inv * r6inv * (Scalar(12.0) * lj1 * r6inv - Scalar(6.0) * lj2);
   ...
   */
  static std::string getName() { return std::string("force_shift_lj"); }
