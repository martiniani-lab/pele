/**
 * backtracking LineSearch given in as taken from
...
public:
  BacktrackingLineSearch(GradientOptimizer *opt,
                         ...
                         Scalar delta_ = Scalar(0), int max_linesearch_ = 40,
                         Scalar min_step_ = 0, Scalar ftol_ = Scalar(1e-4),
      ...
      : LineSearch(opt, max_step_),
        ...
        step_direction(xsize), xoldvec(xsize), step_moving_to_min_(false) {
  ...
  bool get_step_moving_to_min() { return step_moving_to_min_; };
...
private:
  ...
  Vector xoldvec;
  bool step_moving_to_min_; // flag to inform the optimizer whether the step is
