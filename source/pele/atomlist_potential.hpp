...
class AtomListPotential : public PairwisePotentialInterface {
 ...
 public:
  virtual inline double get_energy(Array<double> const &x) {
    ...
    for (size_t i = 0; i < _atoms1.size(); ++i) {
      ...
      for (size_t j = jstart; j < _atoms2.size(); ++j) {
        ...
        e += _interaction->energy(r2, get_non_additive_cutoff(atom1, atom2));
      }
    }
  ...
  virtual inline double add_energy_gradient(Array<double> const &x,
                                            Array<double> &grad) {
    if (x.size() != grad.size()) {
  ...
  virtual inline double add_energy_gradient_hessian(Array<double> const &x,
                                                    Array<double> &grad,
                                                    Array<double> &hess) {
    if (x.size() != grad.size()) {
      throw std::invalid_argument("the gradient has the wrong size");
