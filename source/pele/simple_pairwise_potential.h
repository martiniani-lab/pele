...
#pragma unroll
  ...
  template<typename pairwise_interaction, typename distance_policy>
  double SimplePairwisePotential<pairwise_interaction, distance_policy>::add_energy_gradient(
                                                                                             Array<double> const & x, std::vector<ExactSum> & grad)
  {
    const size_t natoms = x.size() / m_ndim;
    if (m_ndim * natoms != x.size()) {
      throw std::runtime_error("x.size() is not divisible by the number of dimensions");
...
#pragma unroll
          for (size_t k=0; k<m_ndim; ++k) {
            dr[k] *= gij;
            grad[i1+k].AddNumber(-dr[k]);
            grad[j1+k].AddNumber(dr[k]);          
