...
public:
    ...
    if (_frozen_dof.size() > 0) {
      if (_frozen_dof[_frozen_dof.size() - 1] >= (size_t)ndof()) {
        throw std::runtime_error(
            "index of frozen degree of freedom is out of bounds");
      }
    }
  ...
  pele::Array<size_t> get_frozen_dof() {
    return pele::Array<size_t>(_frozen_dof);
  }
  pele::Array<size_t> get_mobile_dof() {
    return pele::Array<size_t>(_mobile_dof);
  }
  ...
  Array<double> get_full_coords(Array<double> const &reduced_coords) {
    ...
    // Array<double> const a(_reference_coords); //wrap _reference_coords in an
    // Array (returns error due to _reference_coords being const)
    ////Array<double> full_coords(a.copy());
    Array<double> full_coords(ndof());
    std::copy(_reference_coords.begin(), _reference_coords.end(),
              full_coords.begin());
...
public:
  ...
  //    inline size_t ndof_frozen() const { return
  //    coords_converter.ndof_frozen(); } inline size_t ndof_mobile() const {
  //    return coords_converter.ndof_mobile(); }
  inline pele::Array<size_t> get_mobile_dof() {
    return coords_converter.get_mobile_dof();
  }
  inline pele::Array<size_t> get_frozen_dof() {
    return coords_converter.get_frozen_dof();
  }
  ...
  inline double get_energy_gradient(Array<double> const &reduced_coords,
                                    Array<double> &reduced_grad) {
    if (reduced_coords.size() != coords_converter.ndof_mobile()) {
      throw std::runtime_error("reduced coords does not have the right size");
    }
    ...
    double const energy =
        _underlying_potential->get_energy_gradient(full_coords, gfull);
    // reduced_coords.assign(coords_converter.get_reduced_coords(full_coords));
  ...
  inline double get_energy_gradient_hessian(Array<double> const &reduced_coords,
    ...
    if (reduced_hess.size() !=
        coords_converter.ndof_mobile() * coords_converter.ndof_mobile()) {
      throw std::invalid_argument("reduced_hess has the wrong size");
    }
    Array<double> full_coords(coords_converter.get_full_coords(reduced_coords));
    Array<double> gfull(coords_converter.ndof());
    Array<double> hfull(coords_converter.ndof() * coords_converter.ndof());
    const double energy = _underlying_potential->get_energy_gradient_hessian(
        full_coords, gfull, hfull);
    // reduced_coords.assign(coords_converter.get_reduced_coords(full_coords));
    reduced_grad.assign(coords_converter.get_reduced_coords(gfull));
