 ...
 *
 * References:
 * Used here: general reference on template meta-programming and recursive
 * template functions:
...
public:
  ...
  periodic_distance() {
    ...
    throw std::runtime_error(
        "The empty constructor is not available for periodic boundaries.");
  }
  ...
  inline double *get_box() const { return m_box; }

  inline double get_ndim() const { return ndim; }
  inline void put_atom_in_box(double *const xnew, const double *x) const {
 ...
 * (this should reduce the need for templating where best performance is not
 * essential)
 */
