...
/*
 * This implements a lightweight directed graph.  This was primarily designed
 * for use in the New Graph Transformation (NGT) method.  This graph was
 * optimized for the following actions to be as fast as possible
 *
 *     * removing nodes
 ...
 * an index and use std::vector's.  This is at odds with fast access to the edge
 * u->v. The solution here is to use a std::map Node.successor_edge_
 */
...
class Graph {
 public:
   ...
   */
  void remove_node(node_id nodeid) { return _remove_node(get_node(nodeid)); }
   ...
   * copy constructor
   */
  Graph(Graph &graph) : next_node_id_(0) {
    ...
    for (auto eiter = graph.edge_list_.begin(); eiter != graph.edge_list_.end();
         ++eiter) {
      edge_ptr edge = *eiter;
