...
void pele::MeasureAngleAxisCluster::align(pele::Array<double> const x1,
  ...
  for (size_t isite = 0; isite < m_topology->nrigid(); ++isite) {
    auto const &rotations =
        m_topology->get_sites()[isite].get_symmetry_rotations();
  ...
  for (size_t isite = 0; isite < nrigid; ++isite) {
    ...
    if (atom_indices.size() != _sites[isite].natoms()) {
      throw std::runtime_error(
          "atom_indices is the wrong size.  Did you forget to set it?");
    }
    auto site_atom_positions =
        _sites[isite].to_atomistic(ca.get_rb_position(isite), psite);
    ...
    //        Array<double> atomistic_view(atomistic.view(istart, istart +
    //        site_atom_positions.size()));
...
void pele::RBTopology::transform_gradient(pele::Array<double> rbcoords,
  ...
  for (size_t isite = 0; isite < nrigid(); ++isite) {
    ...
    for (size_t i = 0; i < atom_indices.size(); ++i) {
      size_t const atom_i = atom_indices[i];
      std::copy(grad.begin() + 3 * atom_i, grad.begin() + 3 * atom_i + 3,
                grad_permuted.begin() + 3 * i);
    }
    _sites[isite].transform_grad(ca.get_rb_rotation(isite), grad_permuted,
