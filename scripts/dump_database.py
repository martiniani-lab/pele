...
def long_summary(db):
    ...
    for n, count in counts:
        if n == 1:
            print(
                "%7d unconnected minima                : minimum energy = %s"
                % (count, minimum_energy[n])
            )
        else:
            print(
                "%7d connected clusters of size %7d: minimum energy = %s"
                % (count, n, minimum_energy[n])
            )
...
def main():
    ...
    parser.add_argument(
        "--write-pathsample-db",
        dest="write_pathsample",
        action="store_true",
        help=(
            "generate a pathsample database by writing files min.data, ts.data,"
            " points.min, and points.ts"
        ),
    )
    parser.add_argument(
        "-m", dest="writeMinima", action="store_true", help="dump minima to screen"
    )
    parser.add_argument(
        "-t",
        dest="writeTS",
    ...
    parser.add_argument(
        "-p", dest="properties", action="store_true", help="print system properties"
    )
    parser.add_argument("-s", dest="summary", action="store_true", help="print summary")
    parser.add_argument(
        "-S", dest="summary_long", action="store_true", help="print long summary"
    )
