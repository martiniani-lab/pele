...
from pele.concurrent import BasinhoppingWorker, ConnectServer, ConnectWorker
...
def start_server(create_system, dbname, server_name=None, host=None, port=None):
    ...
    connect_manager = ConnectServer(
        system, db, server_name=server_name, host=host, port=port
    )

    if db.number_of_minima() == 0:
        print(
            "there are no minima in the database.  Start a basinhopping run to generate"
            " minima"
        )
    else:
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "file containing the function `create_system()`.  "
    ...
    parser.add_argument(
        "--database", type=str, default="db.sqlite", help="database file name"
    )
    parser.add_argument(
        "--basinhopping", action="store_true", help="start a basinhopping run"
    )
    parser.add_argument("--connect", action="store_true", help="start a connect run")
    parser.add_argument("--uri", type=str, default=None, help="uri of the server")
    parser.add_argument(
        "--host",
        type=str,
        default="localhost",
    ...
    parser.add_argument(
        "--niter", type=int, default=5000, help="number of basinhopping iterations"
    )
