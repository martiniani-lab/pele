...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="compute the heat capacity from a database of minima using the harmonic"
    ...
    parser.add_argument(
        "-k",
        type=int,
        help="Number of vibrational degrees freedom.  "
        "If this is not specified the heat capacity will be off by an additive constant",
        default=0,
    )
    parser.add_argument(
        "-o",
        metavar="output",
        type=str,
        help="output file name. The heat capacity will be written to output and output.pdf",
        default="Cv",
    )
    parser.add_argument(
        "--Tmin",
    ...
    if args.OPTIM:
        ...
        if len(minima) == 0:
            print(
                "There are not minima with the necessary thermodynamic information in the database.  Have you computed the normal mode"
