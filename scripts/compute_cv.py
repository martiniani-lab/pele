...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "compute the heat capacity from a database of minima using the"
            " harmonicsuperposition approximation.  The output will be written to Cv"
            " and Cv.pdf unlessspecified differently"
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Number of vibrational degrees freedom.  If this is not specified the heat"
            " capacity will be off by an additive constant"
        ),
        default=0,
    )
    parser.add_argument(
        ...
        type=str,
        help=(
            "output file name. The heat capacity will be written to output and"
            " output.pdf"
        ),
        default="Cv",
    )
    parser.add_argument(
        "--Tmin",
        type=float,
        help="Minimum temperature for the calculation.",
        default=0.02,
    )
    parser.add_argument(
        "--Tmax",
    ...
    parser.add_argument(
        "--Tcount",
        type=int,
        help="Number of temperature points for the calculation.",
        default=300,
    )
    parser.add_argument(
        "--OPTIM",
        action="store_true",
        help=(
            "read data from a min.data file instead."
    ...
    if args.OPTIM:
        ...
        minima = [
            m for m in db.minima() if m.fvib is not None and m.pgorder is not None
        ]
        if len(minima) == 0:
            print(
                "There are not minima with the necessary thermodynamic information in"
                " the database.  Have you computed the normal mode frequencies and"
                " point group order for all the minima?  See pele.thermodynamics  for"
                " more information"
            )
            exit(1)
