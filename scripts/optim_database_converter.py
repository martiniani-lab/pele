...
binary data.  If your coordinates are garbage, try changing the endianness.
    ...
    parser.add_argument(
        "--ndof",
        help=(
            "Number of total degrees of freedom (e.g. 3*number of atoms).  This is"
            " simply the length of a coordinates vector."
        ),
        type=int,
        default=None,
    )
    ...
    parser.add_argument(
        "--Mindata", "-m", help="Name of min.data file", type=str, default="min.data"
    )
    parser.add_argument(
        "--Tsdata", "-t", help="Name of ts.data file", type=str, default="ts.data"
    )
    parser.add_argument(
        "--Pointsmin",
        "-p",
    ...
    parser.add_argument(
        "--Pointsts", "-q", help="Name of points.ts file", type=str, default="points.ts"
    )
    parser.add_argument(
        "--endianness",
        help=(
            'set the endianness of the binary data.  Can be "<" for little-endian or'
            ' ">" for big-endian'
        ),
        type=str,
        default="=",
